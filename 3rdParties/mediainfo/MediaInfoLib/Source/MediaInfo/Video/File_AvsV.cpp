        ...
        if (aspect_ratio == 0);//Forbidden
            ...
            if (vertical_size && AvsV_aspect_ratio[aspect_ratio])
                Fill(Stream_Video, StreamPos_Last, Video_DisplayAspectRatio, (float) horizontal_size / vertical_size
                                                                             * AvsV_aspect_ratio[aspect_ratio] /
                                                                             ((float) display_horizontal_size /
                                                                              display_vertical_size), 3, true);
...
//---------------------------------------------------------------------------
    void File_AvsV::Data_Parse() {
        ...
        if (File_Offset + Buffer_Offset + Element_Size == File_Size && Frame_Count > 0 &&
            Count_Get(Stream_Video) == 0) //Finalize frames in case of there are less than Frame_Count_Valid frames
        {
            //No need of more
            Accept("AVS Video");
            Finish("AVS Video");
...
// Packet "B0"
    void File_AvsV::video_sequence_start() {
        ...
        Get_S1(4, aspect_ratio, "aspect_ratio");
        Param_Info1(AvsV_aspect_ratio[aspect_ratio]);
        Get_S1(4, frame_rate_code, "frame_rate_code");
        Param_Info1(AvsV_frame_rate[frame_rate_code]);
        Get_S3(18, bit_rate_lower, "bit_rate_lower");
        Mark_1();
        Get_S3(12, bit_rate_upper, "bit_rate_upper");
        bit_rate = (bit_rate_upper << 18) + bit_rate_lower;
        Param_Info2(bit_rate * 8, " bps");
...
// Packet "B2", User defined size, this is often used of library name
    void File_AvsV::user_data_start() {
        ...
        while (Library_Start_Offset > 0 && (Buffer[Buffer_Offset + Library_Start_Offset - 1] >= 0x20 &&
                                            Buffer[Buffer_Offset + Library_Start_Offset - 1] <= 0x7D))
            Library_Start_Offset--;

        //But don't accept non-alpha caracters at the beginning (except for "3ivx")
        if (Library_End_Offset - Library_Start_Offset != 4 ||
            CC4(Buffer + Buffer_Offset + Library_Start_Offset) != 0x33697678) //3ivx
...
// Packet "B5"
    void File_AvsV::extension_start() {
        ...
        Param_Info1(AvsV_extension_start_code_identifier[extension_start_code_identifier]);
        ...
        switch (extension_start_code_identifier) {
            case 2  : //sequence_display
            {
                ...
                Param_Info1(AvsV_video_format[video_format]);
            ...
            case 4  : //copyright
            {
                ...
                Mark_1();
                Info_S3(22, copyright_number_3, "copyright_number_3");
                Param_Info1(Ztring::ToZtring(((int64u) copyright_number_1 << 44) + ((int64u) copyright_number_2 << 22) +
                                             (int64u) copyright_number_3, 16));
                BS_End();
            }
                break;
...
// Packet "B3" or "B6"
    void File_AvsV::picture_start() {
        ...
        if (Element_Code == 0xB6) //Only P or B
        {
            ...
            Element_Info1(AvsV_picture_coding_type[picture_coding_type]);
        }
