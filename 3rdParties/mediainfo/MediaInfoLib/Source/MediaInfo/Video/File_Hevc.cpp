...
//---------------------------------------------------------------------------
    File_Hevc::File_Hevc() {
        ...
        //In
        Frame_Count_Valid = MediaInfoLib::Config.ParseSpeed_Get() >= 0.3 ? 16
                                                                         : 16; //Note: should be replaced by "512:2" when I-frame/GOP detection is OK
...
//---------------------------------------------------------------------------
    void File_Hevc::Streams_Fill() {
        ...
        if (chroma_sample_loc_type_top_field != (int32u) -1) {
            Fill(Stream_Video, 0, "ChromaSubsampling_Position",
                 __T("Type ") + Ztring::ToZtring(chroma_sample_loc_type_top_field));
            if (chroma_sample_loc_type_bottom_field != (int32u) -1 &&
                chroma_sample_loc_type_bottom_field != chroma_sample_loc_type_top_field)
                Fill(Stream_Video, 0, "ChromaSubsampling_Position",
                     __T("Type ") + Ztring::ToZtring(chroma_sample_loc_type_bottom_field));
        }
    }
...
//---------------------------------------------------------------------------
    void File_Hevc::Streams_Fill(std::vector<seq_parameter_set_struct *>::iterator seq_parameter_set_Item) {
        ...
        Width -= ((*seq_parameter_set_Item)->conf_win_left_offset + (*seq_parameter_set_Item)->conf_win_right_offset) *
                 CropUnitX;
        Height -= ((*seq_parameter_set_Item)->conf_win_top_offset + (*seq_parameter_set_Item)->conf_win_bottom_offset) *
                  CropUnitY;
        ...
        if ((*seq_parameter_set_Item)->profile_space == 0) {
            ...
            if ((*seq_parameter_set_Item)->level_idc) {
                if ((*seq_parameter_set_Item)->profile_idc)
                    Profile += __T('@');
                Profile += __T('L') + Ztring().From_Number(((float) (*seq_parameter_set_Item)->level_idc) / 30,
        ...
        if ((*seq_parameter_set_Item)->conf_win_left_offset || (*seq_parameter_set_Item)->conf_win_right_offset)
            Fill(Stream_Video, StreamPos_Last, Video_Stored_Width,
                 (*seq_parameter_set_Item)->pic_width_in_luma_samples);
        if ((*seq_parameter_set_Item)->conf_win_top_offset || (*seq_parameter_set_Item)->conf_win_bottom_offset)
            Fill(Stream_Video, StreamPos_Last, Video_Stored_Height,
                 (*seq_parameter_set_Item)->pic_height_in_luma_samples);
        ...
        if ((*seq_parameter_set_Item)->vui_parameters) {
            if ((*seq_parameter_set_Item)->vui_parameters->timing_info_present_flag) {
                if ((*seq_parameter_set_Item)->vui_parameters->time_scale &&
            ...
            if ((*seq_parameter_set_Item)->vui_parameters->aspect_ratio_info_present_flag) {
                ...
                Fill(Stream_Video, 0, Video_DisplayAspectRatio, Width * PixelAspectRatio / Height, 3,
                     true); //More precise
            }
    ...
    bool File_Hevc::Demux_UnpacketizeContainer_Test() {
        ...
        if ((MustParse_VPS_SPS_PPS || SizedBlocks) && Demux_Transcode_Iso14496_15_to_AnnexB) {
            ...
            while (Buffer_Offset + lengthSizeMinusOne + 1 + 1 <= Buffer_Size) {
                ...
                //Coherency checking
                if (Size == 0 || Buffer_Offset + Size > Buffer_Size || (Buffer_Offset + Size != Buffer_Size &&
                                                                        Buffer_Offset + Size + lengthSizeMinusOne + 1 >
                                                                        Buffer_Size))
                    Size = Buffer_Size - Buffer_Offset;

                //Random access check
                if (!RandomAccess && Buffer_Offset + lengthSizeMinusOne + 1 < Buffer_Size &&
                    (Buffer[Buffer_Offset + lengthSizeMinusOne + 1] & 0x40) == 0) //Is a slice
                {
                    ...
                    if (first_slice_segment_in_pic_flag) {
                        ...
                        else if (!(slice_pic_parameter_set_id >= pic_parameter_sets.size() ||
                                   (*(pic_parameter_set_Item =
                                              pic_parameter_sets.begin() + slice_pic_parameter_set_id)) == NULL)) {
            ...
            if (RandomAccess) {
                for (video_parameter_set_structs::iterator Data_Item = video_parameter_sets.begin();
                     Data_Item != video_parameter_sets.end(); ++Data_Item)
            ...
            if (RandomAccess) {
                for (video_parameter_set_structs::iterator Data_Item = video_parameter_sets.begin();
                     Data_Item != video_parameter_sets.end(); ++Data_Item) {
                    std::memcpy(TranscodedBuffer + TranscodedBuffer_Pos, (*Data_Item)->AnnexB_Buffer,
                                (*Data_Item)->AnnexB_Buffer_Size);
            ...
            if (!(((Buffer[Buffer_Offset + (zero_byte ? 4 : 3)] & 0x40) == 0 &&
                   (Buffer[Buffer_Offset + (zero_byte ? 6 : 5)] & 0x80) != 0x80)
                ...
                RandomAccess = Buffer_Offset_Random + 6 <= Buffer_Size &&
                               (Buffer[Buffer_Offset_Random + (zero_byte ? 4 : 3)] & 0x7E) ==
                               (32 << 1); //video_parameter_set
            }
        }
...
//---------------------------------------------------------------------------
    void File_Hevc::Header_Parse() {
        ...
        if (!SizedBlocks) {
            ...
            FILLING_BEGIN() ;
                Header_Fill_Size(Size ? (Element_Offset - 2 + Size) : (Buffer_Size -
...
// Packet "32"
    void File_Hevc::video_parameter_set() {
        ...
        if (MustParse_VPS_SPS_PPS_FromFlv) {
            ...
            std::vector<video_parameter_set_struct *>::iterator Data_Item =
                    video_parameter_sets.begin() + vps_video_parameter_set_id;
            delete *Data_Item;
            *Data_Item = new video_parameter_set_struct(
        ...
        for (int32u SubLayerPos = (vps_sub_layer_ordering_info_present_flag ? 0 : vps_max_sub_layers_minus1);
             SubLayerPos <= vps_max_sub_layers_minus1; SubLayerPos++) {
        ...
        TESTELSE_SB_SKIP("vps_extension_flag");
                ...
                Trusted_IsNot(
                        "(Not supported)"); // Should be skipped, but the detection is too sensible in that case, do not remove it until the detection is adapted
...
// Packet "33"
    void File_Hevc::seq_parameter_set() {
        ...
        if (video_parameter_set_id >= video_parameter_sets.size() ||
            (*(video_parameter_set_Item = video_parameter_sets.begin() + video_parameter_set_id)) == NULL) {
            //Not yet present
        ...
        if (MustParse_VPS_SPS_PPS_FromFlv) {
            ...
            std::vector<seq_parameter_set_struct *>::iterator Data_Item =
                    seq_parameter_sets.begin() + sps_seq_parameter_set_id;
            delete *Data_Item;
            *Data_Item = new seq_parameter_set_struct(
        ...
        Param_Info1(Hevc_chroma_format_idc((int8u) chroma_format_idc));
        ...
        for (int32u SubLayerPos = (sps_sub_layer_ordering_info_present_flag ? 0 : max_sub_layers_minus1);
             SubLayerPos <= max_sub_layers_minus1; SubLayerPos++) {
            Element_Begin1("SubLayer");
        ...
        TEST_SB_SKIP("long_term_ref_pics_present_flag");
                ...
                for (int32u long_term_ref_pics_sps_pos = 0;
                     long_term_ref_pics_sps_pos < num_long_term_ref_pics_sps; long_term_ref_pics_sps_pos++) {
...
// Packet "34"
    void File_Hevc::pic_parameter_set() {
        ...
        if (pps_seq_parameter_set_id >= seq_parameter_sets.size() ||
            (*(seq_parameter_sets.begin() + pps_seq_parameter_set_id)) ==
            NULL) //(seq_parameter_set_Item=seq_parameter_sets.begin()+pps_seq_parameter_set_id) not usd for the moment
        {
            //Not yet present
        ...
        if (MustParse_VPS_SPS_PPS_FromFlv) {
            ...
            std::vector<pic_parameter_set_struct *>::iterator pic_parameter_sets_Item =
                    pic_parameter_sets.begin() + pps_pic_parameter_set_id;
...
// Packet "35"
    void File_Hevc::access_unit_delimiter() {
        ...
        //Parsing
        BS_Begin();
        Info_S1(3, pic_type, "pic_type");
        Param_Info1(Hevc_pic_type[pic_type]);
        BS_End();
    }
...
//---------------------------------------------------------------------------
    void File_Hevc::sei_message(int32u &seq_parameter_set_id) {
        ...
        switch (payloadType) {
            case 0 :
                sei_message_buffering_period(seq_parameter_set_id, payloadSize);
                break;
            case 1 :
                sei_message_pic_timing(seq_parameter_set_id, payloadSize);
                break;
            ...
            case 129 :
                sei_message_active_parameter_sets();
                break;
            case 132 :
                sei_message_decoded_picture_hash(payloadSize);
                break;
            case 137 :
                sei_message_mastering_display_colour_volume();
                break;
            case 144 :
                sei_message_light_level();
                break;
            default :
...
// SEI - 0
    void File_Hevc::sei_message_buffering_period(int32u &seq_parameter_set_id, int32u payloadSize) {
        ...
        bool irap_cpb_params_present_flag = ((*seq_parameter_set_Item)->vui_parameters &&
                                             (*seq_parameter_set_Item)->vui_parameters->xxL_Common)
                                            ? (*seq_parameter_set_Item)->vui_parameters->xxL_Common->sub_pic_hrd_params_present_flag
                                            : false;
        if (!sub_pic_hrd_params_present_flag)
            Get_SB(irap_cpb_params_present_flag, "irap_cpb_params_present_flag");
        int8u au_cpb_removal_delay_length_minus1 = ((*seq_parameter_set_Item)->vui_parameters &&
                                                    (*seq_parameter_set_Item)->vui_parameters->xxL_Common)
                                                   ? (*seq_parameter_set_Item)->vui_parameters->xxL_Common->au_cpb_removal_delay_length_minus1
                                                   : 23;
        int8u dpb_output_delay_length_minus1 = ((*seq_parameter_set_Item)->vui_parameters &&
                                                (*seq_parameter_set_Item)->vui_parameters->xxL_Common)
                                               ? (*seq_parameter_set_Item)->vui_parameters->xxL_Common->dpb_output_delay_length_minus1
                                               : 23;
        ...
        if ((*seq_parameter_set_Item)->NalHrdBpPresentFlag())
            sei_message_buffering_period_xxl(
                    (*seq_parameter_set_Item)->vui_parameters ? (*seq_parameter_set_Item)->vui_parameters->xxL_Common
                                                              : NULL, irap_cpb_params_present_flag,
                    (*seq_parameter_set_Item)->vui_parameters->NAL);
        if ((*seq_parameter_set_Item)->VclHrdBpPresentFlag())
            sei_message_buffering_period_xxl(
                    (*seq_parameter_set_Item)->vui_parameters ? (*seq_parameter_set_Item)->vui_parameters->xxL_Common
                                                              : NULL, irap_cpb_params_present_flag,
                    (*seq_parameter_set_Item)->vui_parameters->VCL);
        BS_End();
    }

    void
    File_Hevc::sei_message_buffering_period_xxl(seq_parameter_set_struct::vui_parameters_struct::xxl_common *xxL_Common,
                                                bool irap_cpb_params_present_flag,
                                                seq_parameter_set_struct::vui_parameters_struct::xxl *xxl) {
        ...
        for (int32u SchedSelIdx = 0; SchedSelIdx < xxl->SchedSel.size(); SchedSelIdx++) {
            ...
            Info_S4 (xxL_Common->initial_cpb_removal_delay_length_minus1 + 1, initial_cpb_removal_delay,
                     "initial_cpb_removal_delay");
            Param_Info2(initial_cpb_removal_delay / 90, " ms");
            Info_S4 (xxL_Common->initial_cpb_removal_delay_length_minus1 + 1, initial_cpb_removal_delay_offset,
            ...
            if (xxL_Common->sub_pic_hrd_params_present_flag || irap_cpb_params_present_flag) {
                Info_S4 (xxL_Common->initial_cpb_removal_delay_length_minus1 + 1, initial_alt_cpb_removal_delay,
                         "initial_alt_cpb_removal_delay");
                Param_Info2(initial_alt_cpb_removal_delay / 90, " ms");
                Info_S4 (xxL_Common->initial_cpb_removal_delay_length_minus1 + 1, initial_alt_cpb_removal_delay_offset,
                         "initial_alt_cpb_removal_delay_offset");
                Param_Info2(initial_alt_cpb_removal_delay_offset / 90, " ms");
            }
        }
    }
...
// SEI - 1
    void File_Hevc::sei_message_pic_timing(int32u &seq_parameter_set_id, int32u payloadSize) {
        ...
        BS_Begin();
        if ((*seq_parameter_set_Item)->vui_parameters
            ? (*seq_parameter_set_Item)->vui_parameters->frame_field_info_present_flag : (
                    (*seq_parameter_set_Item)->general_progressive_source_flag &&
                    (*seq_parameter_set_Item)->general_interlaced_source_flag)) {
...
// SEI - 5
    void File_Hevc::sei_message_user_data_unregistered(int32u payloadSize) {
        ...
        switch (uuid_iso_iec_11578.hi) {
            case 0x214892b89bCC7f42LL :
                Element_Info1("Ateme");
                sei_message_user_data_unregistered_Ateme(payloadSize - 16);
                break;
            case 0xDB4717b509DEA22CLL :
                Element_Info1("x265");
                sei_message_user_data_unregistered_x265(payloadSize - 16);
                break;
            default :
                Element_Info1("unknown");
            ...
            if (Data.find(__T("options: "), Data_Pos_Before) == Data_Pos_Before) {
                ...
                while (Options_Pos_Before != Data.size()) {
                    ...
                    if (option != __T("options:") &&
                        !(!option.empty() && option[0] >= __T('0') && option[0] <= __T('9')) &&
                        option.find(__T("fps=")) != 0 && option.find(__T("bitdepth=")) !=
                                                         0) //Ignoring redundant information e.g. width, height, frame rate, bit depth
                    {
...
//---------------------------------------------------------------------------
    void File_Hevc::slice_segment_header() {
        ...
        Get_UE(slice_pic_parameter_set_id, "slice_pic_parameter_set_id");
        std::vector<pic_parameter_set_struct *>::iterator pic_parameter_set_Item;
        if (slice_pic_parameter_set_id >= pic_parameter_sets.size() ||
            (*(pic_parameter_set_Item = pic_parameter_sets.begin() + slice_pic_parameter_set_id)) == NULL) {
            //Not yet present
        ...
        if (!dependent_slice_segment_flag) {
            ...
            Get_UE(slice_type, "slice_type");
            Param_Info1(Hevc_slice_type(slice_type));
        }
        //TODO...
        Skip_BS(Data_BS_Remain(), "(ToDo)");
...
//---------------------------------------------------------------------------
    void File_Hevc::vui_parameters(std::vector<video_parameter_set_struct *>::iterator video_parameter_set_Item,
                                   seq_parameter_set_struct::vui_parameters_struct *&vui_parameters_Item_) {
        //Parsing
        ...
        TEST_SB_GET (timing_info_present_flag, "timing_info_present_flag");
                ...
                TEST_SB_SKIP("hrd_parameters_present_flag");
                        hrd_parameters(true, (*video_parameter_set_Item)->vps_max_sub_layers_minus1, xxL_Common, NAL,
                                       VCL);
                TEST_SB_END();
        TEST_SB_END();
...
//---------------------------------------------------------------------------
    void File_Hevc::hrd_parameters(bool commonInfPresentFlag, int8u maxNumSubLayersMinus1,
                                   seq_parameter_set_struct::vui_parameters_struct::xxl_common *&xxL_Common,
                                   seq_parameter_set_struct::vui_parameters_struct::xxl *&NAL,
                                   seq_parameter_set_struct::vui_parameters_struct::xxl *&VCL) {
        //Parsing
        ...
        if (commonInfPresentFlag) {
            ...
            if (nal_hrd_parameters_present_flag || vcl_hrd_parameters_present_flag) {
                TEST_SB_GET (sub_pic_hrd_params_present_flag, "sub_pic_hrd_params_present_flag");
                        ...
                        Get_S1(5, du_cpb_removal_delay_increment_length_minus1,
        ...
        for (int8u NumSubLayer = 0; NumSubLayer <= maxNumSubLayersMinus1; NumSubLayer++) {
            ...
            if (nal_hrd_parameters_present_flag)
                sub_layer_hrd_parameters(xxL_Common, bit_rate_scale, cpb_size_scale, cpb_cnt_minus1,
                                         NAL); //TODO: save HRD per NumSubLayer
            if (vcl_hrd_parameters_present_flag)
                sub_layer_hrd_parameters(xxL_Common, bit_rate_scale, cpb_size_scale, cpb_cnt_minus1,
                                         VCL); //TODO: save HRD per NumSubLayer
        }
    }

//---------------------------------------------------------------------------
    void File_Hevc::sub_layer_hrd_parameters(seq_parameter_set_struct::vui_parameters_struct::xxl_common *xxL_Common,
                                             int8u bit_rate_scale, int8u cpb_size_scale, int32u cpb_cnt_minus1,
                                             seq_parameter_set_struct::vui_parameters_struct::xxl *&hrd_parameters_Item_) {
        //Parsing
        ...
        if (MustParse_VPS_SPS_PPS_FromMatroska || MustParse_VPS_SPS_PPS_FromFlv) {
            if (Element_Size >= 5
                ...
                && Buffer[Buffer_Offset + 4] ==
                   0xFF) //Trying to detect old proposal of the form of Matroska implementation
...
//---------------------------------------------------------------------------
    void File_Hevc::VPS_SPS_PPS_FromMatroska() {
        ...
        Skip_S1(3, "Reserved");
        Get_S1(5, vid_parameter_set_count,
               MustParse_VPS_SPS_PPS_FromFlv ? "vid_parameter_set+seq_parameter_set count" : "vid_parameter_set count");
        BS_End();
