...
//---------------------------------------------------------------------------
    void File_Vc1::Streams_Fill() {
        ...
        std::string TempRef;
        for (std::map<int16u, temporalreference>::iterator Temp = TemporalReference.begin();
             Temp != TemporalReference.end(); ++Temp) {
        ...
        if ((Demux_IntermediateItemFound && Buffer[Buffer_Offset + 3] == 0x0D) || Buffer[Buffer_Offset + 3] == 0x0F) {
            ...
            if (InitData_Buffer_Size && Buffer[Buffer_Offset + 3] == 0x0F) //First SequenceHeader
            {
                ...
                switch (Config->Demux_InitData_Get()) {
                    ...
                    case 1 :    //In field
                    {
                        std::string Data_Raw((const char *) (Buffer + Buffer_Offset),
                                             (size_t) (Header_End - Buffer_Offset));
    ...
    void File_Vc1::Data_Parse() {
        ...
        switch (Element_Code) {
            ...
            case 0x1C:
                ...
                break;
            case 0x1D:
                UserDefinedFrameHeader();
                break;
            case 0x1E:
                UserDefinedEntryPointHeader();
                break;
            case 0x1F:
                UserDefinedSequenceHeader();
                break;
            default:
    ...
    void File_Vc1::FrameHeader() {
        ...
        if (FrameRate) {
            Element_Info1C((FrameInfo.PTS != (int64u) -1), __T("PTS ") + Ztring().Duration_From_Milliseconds(
                    float64_int64s(((float64) FrameInfo.PTS) / 1000000 + Frame_Count_InThisBlock * 1000 / FrameRate)));
        ...
        if (profile == 3) //Advanced
        {
            ...
            if (PictureFormat == 2) //Interlaced Field
            {
                ...
                if (ptype_ < 5) {
                    Param_Info1(Vc1_Type[Vc1_FieldTypeTable[ptype_][0]]);
                    Element_Info1(Vc1_Type[Vc1_FieldTypeTable[ptype_][0]]); //First field
                    Param_Info1(Vc1_Type[Vc1_FieldTypeTable[ptype_][1]]);
                    Element_Info1(Vc1_Type[Vc1_FieldTypeTable[ptype_][1]]); //Second field
                ...
                size_t ptype_;
                Get_VL(Vc1_ptype, ptype_, "ptype");
                if (ptype_ < 5) {
                    Param_Info1(Vc1_Type[Vc1_ptype[ptype_].mapped_to3]);
                    Element_Info1(Vc1_Type[Vc1_ptype[ptype_].mapped_to3]);
                }
            ...
            if (FrameInfo.PTS != (int64u) -1) {
                ...
                if ((ptype == 0 || ptype == 1) || (Frame_Count_InThisBlock >= 2 && RefFramesCount >=
                                                                                   2)) //IFrame or PFrame or more than 2 RefFrame for BFrames
                {
                    if (framerate_present)
                    ...
                    if (TemporalReference.size() < 30) {
                            ...
                            !TemporalReference_Waiting.empty()) //We must have 2 I or P pictures to be sure not having B picture later
                        {
                            ...
                            for (size_t Pos = 1; Pos <
                                                 TemporalReference_Waiting.size(); Pos++) //All B frames (not the first frame, which is I or P)
                            {
...
// Packet "0E"
    void File_Vc1::EntryPointHeader() {
        ...
        TEST_SB_SKIP("coded_size_flag");
                Info_S2(12, coded_width, "coded_width");
                Param_Info2((coded_width + 1) * 2, " pixels");
                Info_S2(12, coded_height, "coded_height");
                Param_Info2((coded_height + 1) * 2, " pixels");
        TEST_SB_END();
        if (extended_mv)
        ...
        FILLING_BEGIN() ;
            ...
            if (InitData_Buffer_Size) {
                ...
                std::memcpy(InitData_Buffer_Temp + InitData_Buffer_Size, Buffer + Buffer_Offset - (size_t) Header_Size,
                            (size_t) (Header_Size + Element_Size));
...
// Packet "0F"
    void File_Vc1::SequenceHeader() {
        ...
        //Parsing
        BS_Begin();
        Get_S1(2, profile, "profile");
        Param_Info1(Vc1_Profile[profile]);
        ...
        } else if (profile == 3) //Advanced
        {
            ...
            Get_S1(2, colordiff_format, "colordiff_format");
            Param_Info1(Vc1_ColorimetryFormat[colordiff_format]);
            Skip_S1(3, "frmrtq_postproc");
            Skip_S1(5, "bitrtq_postproc");
            Skip_SB("postprocflag");
            Get_S2(12, coded_width, "max_coded_width");
            Param_Info2((coded_width + 1) * 2, " pixels");
            Get_S2(12, coded_height, "max_coded_height");
            Param_Info2((coded_height + 1) * 2, " pixels");
            ...
            TEST_SB_SKIP("display_ext");
                    Info_S2(14, display_x, "display_horiz_size");
                    Param_Info2(display_x + 1, " pixels");
                    Info_S2(14, display_y, "display_vert_size");
                    Param_Info2(display_y + 1, " pixels");
                    TEST_SB_SKIP("aspectratio_flag");
                            Get_S1(4, AspectRatio, "aspect_ratio");
                            Param_Info1(Vc1_PixelAspectRatio[AspectRatio]);
                            if (AspectRatio == 0x0F) {
                    ...
                    TEST_SB_GET(framerate_present, "framerate_flag");
                            TESTELSE_SB_GET(framerate_form, "framerateind");
                                    Get_S2(16, framerateexp, "framerateexp");
                                    Param_Info3((float32) ((framerateexp + 1) / 32.0), " fps", 3);
                                TESTELSE_SB_ELSE("framerateind");
                                    Get_S1(8, frameratecode_enr, "frameratenr");
                                    Param_Info1(Vc1_FrameRate_enr(frameratecode_enr));
                                    Get_S1(4, frameratecode_dr, "frameratedr");
                                    Param_Info1(Vc1_FrameRate_dr(frameratecode_dr));
                            TESTELSE_SB_END();
                    TEST_SB_END();
            ...
            TEST_SB_GET (hrd_param_flag, "hrd_param_flag");
                    ...
                    for (int8u Pos = 0; Pos < hrd_num_leaky_buckets; Pos++) {
                        ...
                        int32u hrd_buffer_value = (int32u) ((hrd_buffer + 1) * pow(2.0, 1 + buffer_size_exponent));
                        Param_Info2(hrd_buffer_value, " bytes");
                        Element_End0();
        ...
        FILLING_BEGIN() ;
            ...
            if (InitData_Buffer_Size) {
                ...
                std::memcpy(InitData_Buffer, Buffer + Buffer_Offset - (size_t) Header_Size,
                            (size_t) (Header_Size + Element_Size));
            }
#endif //MEDIAINFO_DEMUX
        FILLING_END();
