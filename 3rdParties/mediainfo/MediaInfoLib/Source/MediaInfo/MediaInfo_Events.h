...
typedef void (__stdcall MediaInfo_Event_CallBackFunction)(unsigned char *Data_Content, size_t Data_Size,
                                                          void *UserHandler);
...
{
    if (Event->StreamIDs_Size >= 2 && Event->ParserIDs[0] == MediaInfo_Parser_MpegPs &&
        Event->ParserIDs[1] == MediaInfo_Parser_MpegPs_Ext)   // DVD-Video
        return (Event->StreamIDs[0] << 16) | Event->StreamIDs[1];
    if (Event->StreamIDs_Size >= 2 && Event->ParserIDs[0] == MediaInfo_Parser_HdsF4m &&
        Event->StreamIDs_Width[1])                          // HDS F4M having more than one stream per referenced file
        return Event->StreamIDs[0] << 16 | Event->StreamIDs[1];
    if (Event->StreamIDs_Size >= 3 && Event->ParserIDs[0] ==
                                      MediaInfo_Parser_HlsIndex)                                                     // HLS Index
    {
    ...
    if (Event->StreamIDs_Size >= 2 && Event->ParserIDs[0] ==
                                      MediaInfo_Parser_Hls)                                                          // HLS Sequence
        return Event->StreamIDs[1];
    if (Event->StreamIDs_Size >= 2 && Event->ParserIDs[0] == MediaInfo_Parser_Ism &&
        Event->StreamIDs_Width[1])                             // ISM having more than one stream per referenced file
        return Event->StreamIDs[0] << 16 | Event->StreamIDs[1];
    if (Event->StreamIDs_Size >= 3 && Event->ParserIDs[0] == MediaInfo_Parser_Dxw &&
        Event->ParserIDs[1] == MediaInfo_Parser_MpegTs)          // DXW with MPEG-TS
        return (Event->StreamIDs[0] << 16) | Event->StreamIDs[1];
    if (Event->StreamIDs_Size >= 3 && Event->ParserIDs[0] ==
                                      MediaInfo_Parser_DcpAm)                                                        // DCP/IMF AM
        return (Event->StreamIDs[0] << 24) | (Event->StreamIDs[1] << 16) | Event->StreamIDs[2];
    if (Event->StreamIDs_Size >= 3 && Event->ParserIDs[0] ==
                                      MediaInfo_Parser_DcpPkl)                                                       // DCP/IMF PKL
        return (Event->StreamIDs[0] << 24) | (Event->StreamIDs[1] << 16) | Event->StreamIDs[2];
    if (Event->StreamIDs_Size >= 2 && Event->ParserIDs[0] ==
                                      MediaInfo_Parser_DcpCpl)                                                       // DCP/IMF CPL
        return (Event->StreamIDs[0] << 16) | Event->StreamIDs[1];
...
inline std::string MediaInfo_ID_FromEvent_stringA(MediaInfo_Event_Generic *Event, int) // Flags)
{
    ...
    } else if (Event->StreamIDs_Size >= 2 && Event->ParserIDs[0] == MediaInfo_Parser_HdsF4m &&
               Event->StreamIDs_Width[1])                          // HDS F4M having more than one stream per referenced file
    {
        ToReturn << Event->StreamIDs[0];
        ToReturn << '-';
        ToReturn << Event->StreamIDs[1];
    } else if (Event->StreamIDs_Size >= 3 && Event->ParserIDs[0] ==
                                             MediaInfo_Parser_HlsIndex)                                                     // HLS Index
    {
        ToReturn << Event->StreamIDs[0];
        ToReturn << '-';
        if (Event->ParserIDs[1] == MediaInfo_Parser_Hls)
            ...
            ToReturn << Event->StreamIDs[1];
    } else if (Event->StreamIDs_Size >= 2 && Event->ParserIDs[0] ==
                                             MediaInfo_Parser_Hls)                                                          // HLS Sequence
    {
        ToReturn << Event->StreamIDs[1];
    } else if (Event->StreamIDs_Size >= 2 && Event->ParserIDs[0] == MediaInfo_Parser_Ism &&
               Event->StreamIDs_Width[1])                             // ISM having more than one stream per referenced file
    {
        ToReturn << Event->StreamIDs[0];
        ToReturn << '-';
        ToReturn << Event->StreamIDs[1];
    } else if (Event->StreamIDs_Size >= 3 && Event->ParserIDs[0] == MediaInfo_Parser_Dxw &&
               Event->ParserIDs[1] == MediaInfo_Parser_MpegTs)          // DXW with MPEG-TS
    {
        ToReturn << Event->StreamIDs[0];
        ToReturn << '-';
    ...
    if (SubID) {
        ...
        if (SubID) {
            ...
            if (strcmp(ContainerFormat, "DCP AM") == 0                                            // DCP AM
                ...
                || strcmp(ContainerFormat, "HDS F4M") ==
                   0                                               // HDS F4M having more than one stream per referenced file
                ...
                || strcmp(ContainerFormat, "ISM") ==
                   0                                                   // ISM having more than one stream per referenced file
                || strcmp(ContainerFormat, "DXW") == 0 &&
                   strstr(MuxingMode, "MPEG-TS") == MuxingMode      // DXW with MPEG-TS
