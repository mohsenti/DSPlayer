        ...
        for (;;) {
            ...
            switch (signature) {
                ...
                case 0x05054b50 :
                    ...
                    break;
                case 0x06054b50 :
                    if (!end_of_central_directory())
                        return; //Not enough data
                    break;
                case 0x08064b50 :
                    if (!archive_extra_data_record())
                        return;
                    break;
                case 0x06064b50 :
                    if (!Zip64_end_of_central_directory_record())
                        return;
                    break;
                case 0x07064b50 :
                    if (!Zip64_end_of_central_directory_locator())
                        return;
                    break;
                default:
                    Finish();
                    return; //Reject(); return; //Unknown value  //Décommenter quand toutes les signatures sont implémentées
            }
...
//***************************************************************************
    ...
    bool File_Zip::local_file_header() {
        ...
        Param_Info1C((compression_method == 97 || compression_method == 98),
                     Zip_compression_method[compression_method - 97 + 20]);
    ...
    bool File_Zip::central_directory() {
        ...
        if (Element_Offset + 46 + file_name_length + extra_field_length + file_comment_length >
            Element_Size) //central_directory_structure all included
            return false; //Not enough data
    ...
    bool File_Zip::end_of_central_directory() {
        if (Element_Offset + 22 >
            Element_Size) //end_of_central_directory up to relative offset of .ZIP file comment length included
            return false; //Not enough data
    ...
    bool File_Zip::Zip64_end_of_central_directory_record() {
        ...
        int64u size_of_Zip64_end_of_central_directory_record = LittleEndian2int64u(
                Buffer + (size_t) Element_Offset + 4);
        if (Element_Offset + 12 + size_of_Zip64_end_of_central_directory_record >
            Element_Size) //end_of_central_directory all included
            return false; //Not enough data
