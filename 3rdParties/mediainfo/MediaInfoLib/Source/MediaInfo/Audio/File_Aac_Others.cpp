...
//---------------------------------------------------------------------------
    void File_Aac::ALSSpecificConfig() {
        ...
        Get_S2(16, channels, "channels");
        Param_Info2(channels + 1, " channel(s)");
        ...
        if (file_type == 1) //WAVE file
        {
            ...
            File__Analyze::Finish(
                    &MI); //No merge of data, only for trace information, because this is the data about the decoded stream, not the encoded stream
            Element_End0();
        } else
