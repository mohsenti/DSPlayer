...
//---------------------------------------------------------------------------
    void File_Aac::AudioSpecificConfig(size_t End) {
        ...
        Get_S1(4, sampling_frequency_index, "samplingFrequencyIndex");
        Param_Info1(Aac_sampling_frequency[sampling_frequency_index]);
        ...
        Get_S1(4, channelConfiguration, "channelConfiguration");
        Param_Info1(Aac_ChannelConfiguration[channelConfiguration]);
        if (audioObjectType == 5 || audioObjectType == 29) {
            extensionAudioObjectType = 5;
            sbrPresentFlag = true;
            if (audioObjectType == 29)
                psPresentFlag = true;
            Get_S1(4, extension_sampling_frequency_index, "extensionSamplingFrequencyIndex");
            ...
            if (extension_sampling_frequency_index == 0xF) {
                ...
                sampling_frequency_index = Aac_AudioSpecificConfig_sampling_frequency_index(
            ...
            if (syncExtensionType == 0x2b7) {
                ...
                if (extensionAudioObjectType == 5) {
                    ...
                    if (sbrPresentFlag) {
                        ...
                        if (extension_sampling_frequency_index == 0xF) {
                            Get_S3(24, extension_sampling_frequency, "extensionSamplingFrequency");
                            extension_sampling_frequency_index = Aac_AudioSpecificConfig_sampling_frequency_index(
                                    extension_sampling_frequency);
                        } else
                            extension_sampling_frequency = Aac_sampling_frequency[extension_sampling_frequency_index];
...
//---------------------------------------------------------------------------
    void File_Aac::AudioSpecificConfig_OutOfBand(int64s sampling_frequency_, int8u audioObjectType_, bool sbrData,
                                                 bool psData, bool sbrPresentFlag, bool psPresentFlag) {
        ...
        if (sbrPresentFlag || !Infos["Format_Settings_SBR"].empty()) {
            ...
            int32u SamplingRate = (extension_sampling_frequency_index == (int8u) -1) ? (((int32u) Frequency_b) * 2)
                                                                                     : extension_sampling_frequency;
            if (SamplingRate) {
...
//---------------------------------------------------------------------------
    void File_Aac::GetAudioObjectType(int8u &ObjectType, const char *Name) {
        ...
        Element_Info1(ObjectType);
        Element_Info1(Aac_Format_Profile(ObjectType));
        Element_End0();
    }
                    ...
                    switch (frameLengthType[streamID[prog][lay]]) {
                        case 0 :
                            if (CA_system_ID_MustSkipSlices) {
                                ...
                                Skip_BS(8 * MuxSlotLengthBytes[streamID[prog][lay]],
                                        "Encrypted payload[streamID[prog][lay]]");
                                Frame_Count_Valid = 0;
                            } else
                                raw_data_block();
                            break;
...
//---------------------------------------------------------------------------
    void File_Aac::adif_header() {
        ...
        Skip_SB("home");
        Get_SB(bitstream_type, "bitstream_type");
        Param_Info1(bitstream_type ? "VBR" : "CBR");
...
//---------------------------------------------------------------------------
    void File_Aac::adts_fixed_header() {
        ...
        Skip_BS(12, "syncword");
        Get_SB(id, "id");
        Param_Info1(Aac_Adts_ID[id]);
        Skip_BS(2, "layer");
        Get_SB(protection_absent, "protection_absent");
        Get_S1(2, audioObjectType, "profile_ObjectType");
        audioObjectType++;
        Param_Info1(Aac_audioObjectType(audioObjectType));
        Get_S1(4, sampling_frequency_index, "sampling_frequency_index");
        Param_Info2(Aac_sampling_frequency[sampling_frequency_index], " Hz");
        Frequency_b = Aac_sampling_frequency[sampling_frequency_index];
        Skip_SB("private");
...
//---------------------------------------------------------------------------
    void File_Aac::adts_variable_header() {
        ...
        Get_S2(11, adts_buffer_fullness, "adts_buffer_fullness");
        Param_Info1(adts_buffer_fullness == 0x7FF ? "VBR" : "CBR");
