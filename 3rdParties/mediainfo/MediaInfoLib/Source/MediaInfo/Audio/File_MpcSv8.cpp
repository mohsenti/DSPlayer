...
//---------------------------------------------------------------------------
    void File_MpcSv8::Header_Parse() {
        ...
        Header_Fill_Size(Key == Elements::AP ? Element_Offset
                                             : Size); //We don't need the data of audio packet, and we will stop here...
    }
...
//---------------------------------------------------------------------------
    void File_MpcSv8::RG() {
        ...
        Skip_B1("Version");
        Get_L2(TitleGain, "Title gain");
        Param_Info3(((float32) ((int16s) TitleGain)) / 1000, " dB", 2);
        Skip_L2("Title peak");
        Get_L2(AlbumGain, "Album gain");
        Param_Info3(((float32) ((int16s) TitleGain)) / 1000, " dB", 2);
        Skip_L2("Album peak");
    }
        ...
        FILLING_BEGIN() ;
            ...
            if (SampleCount) {
                ...
                Fill(Stream_Audio, 0, Audio_BitRate,
                     File_Size * 8 * Mpc_SampleFreq[SampleFrequency] / SampleCount); //Should be more precise...
            }
