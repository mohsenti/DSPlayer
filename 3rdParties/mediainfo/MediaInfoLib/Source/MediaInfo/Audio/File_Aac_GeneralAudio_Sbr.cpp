...
// k0 = lower frequency boundary
    int8u Aac_k0_Compute(int8u bs_start_freq, int8u extension_sampling_frequency_index) {
        return Aac_k0_startMin[extension_sampling_frequency_index] +
               Aac_k0_offset[extension_sampling_frequency_index][bs_start_freq];
    }
...
// k2 = upper frequency boundary
    int8u Aac_k2_Compute(int8u bs_stop_freq, int8u extension_sampling_frequency_index, int8u k0) {
        switch (bs_stop_freq) {
            case 14 :
                return (int8u) min(64, 2 * k0);
            case 15 :
                return (int8u) min(64, 3 * k0);
            default :;
        }

        return (int8u) min(64, Aac_k2_stopMin[extension_sampling_frequency_index] +
                               Aac_k2_offset[extension_sampling_frequency_index][bs_stop_freq]);
    }
...
// Computing for bs_freq_scale != 0
    ...
    static bool
    Aac_f_master_Compute(int8u &num_env_bands_Master, int8u *f_Master, sbr_handler *sbr, int8u k0, int8u k2) {
        int8u temp1[] = {6, 5, 4};
...
// Derived frequency border tables
    static bool Aac_bands_Compute(const int8u &num_env_bands_Master, int8u *f_Master, sbr_handler *sbr, int8u k2) {
        ...
        sbr->num_env_bands[0] =
                (sbr->num_env_bands[1] >> 1) + (sbr->num_env_bands[1] - ((sbr->num_env_bands[1] >> 1) << 1));
...
//---------------------------------------------------------------------------
    static bool Aac_Sbr_Compute(sbr_handler *sbr, int8u extension_sampling_frequency_index) {
        ...
        switch (extension_sampling_frequency_index) {
            ...
            case 2 :
            case 3 :
                if ((k2 - k0) > 32) return false;
                break;
            case 4 :
                if ((k2 - k0) > 35) return false;
                break;
            case 5 :
            case 6 :
            case 7 :
            case 8 :
                if ((k2 - k0) > 48) return false;
                break;
            default :;
        }
        ...
        switch (bs_frame_class) {
            ...
            case 1 : //FIXVAR
                ...
                for (int8u env = 0; env < sbr->bs_num_env[ch]; env++)
                    Get_SB(sbr->bs_freq_res[ch][sbr->bs_num_env[ch] - 1 - env],
