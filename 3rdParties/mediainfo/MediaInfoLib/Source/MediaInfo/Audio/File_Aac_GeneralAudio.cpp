...
//---------------------------------------------------------------------------
    void File_Aac::GASpecificConfig() {
        ...
        frame_length = frameLengthFlag == 0 ? 1024 : 960;
        Param_Info2(frame_length, " bytes");
...
//---------------------------------------------------------------------------
    void File_Aac::program_config_element() {
        ...
        Get_S1(2, audioObjectType_Temp, "object_type");
        audioObjectType_Temp++;
        Param_Info1(Aac_audioObjectType(audioObjectType_Temp));
        Get_S1(4, sampling_frequency_index_Temp, "sampling_frequency_index");
        Param_Info1(Aac_sampling_frequency[sampling_frequency_index_Temp]);
        ...
        if (!num_side_channel_elements && num_back_channel_elements &&
            num_back_channel_elements < 3) // Hack: e.g. in case of 5.1,
        {
        ...
        switch (Channels_Front) {
            case 0 :
                break;
            case 1 :
                Channels_Positions += __T("Front: C");
                ChannelLayout += __T("C ");
                break;
            case 2 :
                Channels_Positions += __T("Front: L R");
                ChannelLayout += __T("L R ");
                break;
            case 3 :
                Channels_Positions += __T("Front: L C R");
                ChannelLayout +=
                        num_front_channel_elements == 2 ? (front1_element_is_cpe ? __T("L R C ") : __T("C L R ")) : __T(
                                "? ? ? ");
                break;
            default :
                Channels_Positions += __T("Front: ");
                Channels_Positions += Ztring::ToZtring(Channels_Front);
                ChannelLayout += __T("? "); //Which config?
        }
        switch (Channels_Side) {
            case 0 :
                break;
            case 1 :
                Channels_Positions += __T(", Side: C");
                ChannelLayout += __T("Cs ");
                break;
            case 2 :
                Channels_Positions += __T(", Side: L R");
                ChannelLayout += __T("Ls Rs ");
                break;
            case 3 :
                Channels_Positions += __T(", Side: L C R");
                ChannelLayout += __T("? ? ? ");
                break;
            default :
                Channels_Positions += __T(", Side: ");
                Channels_Positions += Ztring::ToZtring(Channels_Side);
                ChannelLayout += __T("? "); //Which config?
        }
        switch (Channels_Back) {
            case 0 :
                break;
            case 1 :
                Channels_Positions += __T(", Back: C");
                ChannelLayout += __T("Cs ");
                break;
            case 2 :
                Channels_Positions += __T(", Back: L R");
                ChannelLayout += __T("Rls Rrs ");
                break;
            case 3 :
                Channels_Positions += __T(", Back: L C R");
                ChannelLayout += __T("Rls Cs Rrs ");
                break;
            default :
                Channels_Positions += __T(", Back: ");
                Channels_Positions += Ztring::ToZtring(Channels_Back);
                ChannelLayout += __T("? "); //Which config?
        }
        switch (Channels_LFE) {
            case 0 :
            ...
            default :
                Channels_Positions += __T(", LFE= ");
                Channels_Positions += Ztring::ToZtring(Channels_LFE);
                ChannelLayout += __T("? "); //Which config?
        }
        ...
        FILLING_BEGIN() ;
            //Integrity test
            if (Aac_sampling_frequency[sampling_frequency_index_Temp] == 0 ||
                Channels > 24) // TODO: full_2023548870.mp4 is buggy
            {
            ...
            if (!Infos["Format_Settings_SBR"].empty()) {
                ...
                Infos["SamplingRate"].From_Number((extension_sampling_frequency_index == (int8u) -1) ? (Frequency_b * 2)
            ...
            if (!Infos["Format_Settings_PS"].empty()) {
                ...
                if (MediaInfoLib::Config.LegacyStreamDisplay_Get()) {
                    ...
                    Infos["SamplingRate"] = Ztring().From_Number(
                            (extension_sampling_frequency_index == (int8u) -1) ? (Frequency_b * 2)
                                                                               : extension_sampling_frequency, 10) +
                                            __T(" / ") + SamplingRate;
                }
...
//---------------------------------------------------------------------------
    void File_Aac::raw_data_block() {
        ...
        do {
            ...
            Get_S1(3, id_syn_ele, "id_syn_ele");
            Param_Info1(Aac_id_syn_ele[id_syn_ele]);
            Element_Name(Aac_id_syn_ele[id_syn_ele]);
            ...
            switch (id_syn_ele) {
                ...
                case 0x05 :
                    ...
                    break; //ID_PCE
                case 0x06 :
                    fill_element(id_syn_ele_Previous);
                    break; //ID_FIL
                case 0x07 :
                    break; //ID_END
...
//---------------------------------------------------------------------------
    void File_Aac::ics_info() {
        ...
        Param_Info1(Aac_window_sequence[window_sequence]);
            ...
            case 2 :    //EIGHT_SHORT_SEQUENCE
                ...
                for (int g = 0; g < num_window_groups; g++) {
                    ...
                    for (int8u i = 0; i < num_swb; i++) {
                        int16u width = Aac_swb_offset_short_window[sampling_frequency_index]->swb_offset[i + 1] -
                                       Aac_swb_offset_short_window[sampling_frequency_index]->swb_offset[i];
    ...
    void File_Aac::gain_control_data() {
        ...
        switch (window_sequence) {
            ...
            case 3 :    //LONG_STOP_SEQUENCE
                ...
                break;
            default:
                return; //Never happens but makes compiler happy
        }
            ...
            while (k < max_sfb) {
                ...
                if (!aacSectionDataResilienceFlag || sect_cb[g][i] < 11 || (sect_cb[g][i] > 11 && sect_cb[g][i] < 16)) {
                    for (;;) {
                        ...
                        Get_S1((window_sequence == 2 ? 3 : 5), sect_len_incr,
                               "sect_len_incr"); // (window_sequence == EIGHT_SHORT_SEQUENCE) => 3
            ...
            for (int8u i = 0; i < num_sec[g]; i++) {
                switch (sect_cb[g][i]) {
                    ...
                    default :
                        ...
                        for (int16u k = sect_sfb_offset[g][sect_start[g][i]];
                             k < sect_sfb_offset[g][sect_end[g][i]]; k += (sect_cb[g][i] < 5 ? 4 : 2)) {
...
//---------------------------------------------------------------------------
    void File_Aac::extension_payload(size_t End, int8u id_aac) {
        ...
        switch (extension_type) {
            case 11 :
                dynamic_range_info();
                break;                //EXT_DYNAMIC_RANGE
            case 12 :
                sac_extension_data(End);
                break;             //EXT_SAC_DATA
            case 13 :
                sbr_extension_data(End, id_aac, 0);
                break;  //EXT_SBR_DATA
            case 14 :
                sbr_extension_data(End, id_aac, 1);
                break;  //EXT_SBR_DATA_CRC
            case 1 :   //EXT_FILL_DATA
                Skip_S1(4, "fill_nibble");
                Param_Info1("must be 0000");
                if (Data_BS_Remain() > End) {
                    Element_Begin1("fill_byte");
                    while (Data_BS_Remain() > End)
                        Skip_S1(8, "fill_byte[i]");
                    Param_Info1("must be 10100101");
                    Element_End0();
                }
                break;
