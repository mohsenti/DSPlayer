...
//---------------------------------------------------------------------------
    void File_SmpteSt0302::Streams_Fill() {
        ...
        for (size_t Pos = 0; Pos < Count_Get(Stream_Audio); Pos++)
            if (Retrieve(Stream_Audio, Pos,
                         Audio_MuxingMode).empty()) //TODO: put "SMPTE ST 302" in this field, the current name is there only for legacy
                Fill(Stream_Audio, Pos, Audio_MuxingMode, "SMPTE ST 302");
            else
                Fill(Stream_Audio, Pos, Audio_MuxingMode,
                     __T("SMPTE ST 302 / ") + Retrieve(Stream_Audio, Pos, Audio_MuxingMode), true);
...
//---------------------------------------------------------------------------
    void File_SmpteSt0302::Read_Buffer_Continue() {
        ...
        BS_Begin();
        Get_S1(2, number_channels, "number_channels");
        Param_Info2((1 + number_channels) * 2, " channels");
        Info_S1(8, channel_identification, "channel_identification");
        Get_S1(2, bits_per_sample, "bits_per_sample");
        Param_Info2((4 + bits_per_sample) * 4, " bits");
        ...
        //Cohenrancy test
        if (Element_Size != 4 + (int64u) audio_packet_size || bits_per_sample == 3 ||
            audio_packet_size % ((1 + number_channels) * (5 + bits_per_sample))) {
        ...
        switch (bits_per_sample) {
            case 0 :
                PcmSize = audio_packet_size * 4 / 5;
                Ratio = 4.0 / 5.0;
                break;
            case 1 :
                PcmSize = audio_packet_size * 5 / 6;
                Ratio = 5.0 / 6.0;
                break;
            case 2 :
                PcmSize = audio_packet_size * 6 / 7;
                Ratio = 6.0 / 7.0;
                break;
            case 3 :
                Reject();
        ...
        while (Element_Offset < Element_Size) {
            ...
            switch (bits_per_sample) {
                case 0  :   //16 bits
                    ...
                    Info[Info_Offset + 2] =
                            (Reverse8(Buffer[Buffer_Pos + 3]) << 4) | (Reverse8(Buffer[Buffer_Pos + 2]) >> 4);
                    Info[Info_Offset + 3] =
                            (Reverse8(Buffer[Buffer_Pos + 4]) << 4) | (Reverse8(Buffer[Buffer_Pos + 3]) >> 4);
                ...
                case 1  :   //20 bits
                    ...
                    Info[Info_Offset + 1] = Reverse8(Buffer[Buffer_Pos + 1]);
                    Info[Info_Offset + 2] =
                            (Reverse8(Buffer[Buffer_Pos + 3]) << 4) | (Reverse8(Buffer[Buffer_Pos + 2]) & 0x0F);
                    Info[Info_Offset + 3] =
                            (Reverse8(Buffer[Buffer_Pos + 4]) << 4) | (Reverse8(Buffer[Buffer_Pos + 3]) >> 4);
                    Info[Info_Offset + 4] =
                            (Reverse8(Buffer[Buffer_Pos + 5]) << 4) | (Reverse8(Buffer[Buffer_Pos + 4]) >> 4);
                ...
                case 2  :   //24 bits
                    ...
                    Info[Info_Offset + 4] =
                            (Reverse8(Buffer[Buffer_Pos + 5]) << 4) | (Reverse8(Buffer[Buffer_Pos + 4]) >> 4);
                    Info[Info_Offset + 5] =
                            (Reverse8(Buffer[Buffer_Pos + 6]) << 4) | (Reverse8(Buffer[Buffer_Pos + 5]) >> 4);
        ...
        FrameInfo.DUR =
                ((int64u) audio_packet_size) * 1000000000 / ((1 + number_channels) * (5 + bits_per_sample) * 48000);
