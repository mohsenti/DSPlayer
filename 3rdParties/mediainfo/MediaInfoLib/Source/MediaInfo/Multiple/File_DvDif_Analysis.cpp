...
//---------------------------------------------------------------------------
            ...
            switch (Buffer[Buffer_Offset] & 0xE0) {
                ...
                case 0x60 : //SCT=3 (Audio)
                {
                    ...
                    if (Buffer[Buffer_Offset + 3 + 0] == 0x50) //audio_source
                    {
                        ...
                        size_t Channel = (QU_FSC ? 2 : 0) + ((Buffer[Buffer_Offset + 1] >> 4) >= (QU_System ? 6 : 5) ? 1
                                                                                                                     : 0); //If Dseq>=5 or 6
                    ...
                    if (Buffer[Buffer_Offset + 8] == 0x80) {
                        if ((QU == 0 && Buffer[Buffer_Offset + 9] == 0x00)  //16-bit 0x8000
                            ...
                            || (QU == (int8u) -1 &&
                                ((Buffer[Buffer_Offset + 9] == 0x00 && Buffer[Buffer_Offset + 10] == 0x80 &&
                                  Buffer[Buffer_Offset + 11] == 0x00)
                                 || (Buffer[Buffer_Offset + 9] == 0x80 &&
                                     Buffer[Buffer_Offset + 10] == 0x00)))) //In case of QU is not already detected
                        {
    ...
    void File_DvDif::Errors_Stats_Update() {
        ...
        if (Speed_FrameCount) //We must have at least one complete frame
        {
            ...
            Event.EventCode = MediaInfo_EventCode_Create(MediaInfo_Parser_DvDif, MediaInfo_Event_DvDif_Analysis_Frame,
                                                         0);
            Event.TimeCode = 0;
            ...
            if (Speed_RecTime_Current.IsValid) {
                ...
                if (Speed_RecZ.empty() ||
                    Speed_RecZ[0].First.FramePos == Speed_FrameCount) //Empty or the same frame as RecDate
                {
                    if (Speed_RecZ.empty())
                        Speed_RecZ.resize(1);
                    Speed_RecZ[0].First.FramePos = Speed_FrameCount;
                ...
                if (Speed_TimeStampsZ[0].First.FramePos ==
                    Speed_FrameCount) //Empty or the same frame as RecDate or the same frame as TimeCode
            ...
            if (!Video_STA_Errors.empty()) {
                ...
                if (Video_STA_Errors_Details.size() > 2) {
                    Ztring Video_STA_Errors_Count_Padded = Ztring::ToZtring(
                            ((float) Video_STA_Errors_Count) * 100 / ((DSF_IsValid && DSF) ? 1500 : 1350) *
                            (QU_FSC ? 2 : 1), 2);
            ...
            if (QU != (int8u) -1 && (!Audio_Invalids.empty() || !Audio_Errors.empty())) {
                ...
                for (size_t Channel = 0; Channel < 4; Channel++) {
                    ...
                    if (Audio_Errors_Count) {
                        ...
                        Ztring Audio_Errors_Count_Padded = Ztring::ToZtring(
                                ((float) Audio_Errors_Count) * 100 / ((DSF_IsValid && DSF) ? 54 : 45) *
                                (QU_FSC ? 2 : 1), 2);
                        if (Audio_Errors_Count_Padded.size() < 2)
                            Audio_Errors_Count_Padded.insert(0, 2 - Audio_Errors_Count_Padded.size(), __T(' '));
                        if (ErrorsAreAlreadyDetected)
                            Errors_Stats_Line_Details += __T(", ");
                        if (Audio_Errors_Count < (size_t) ((QU_System ? 6 : 5) * 9)) {
                            Errors_Stats_Line_Details +=
                                    __T("CH") + Ztring::ToZtring(Channel + 1) + __T(": ") + Audio_Errors_Count_Padded +
                                    __T("%");
                            Audio_Errors_Details.resize(Audio_Errors_Details.size() - 2);
                            Errors_Stats_Line_Details += __T(" (") + Audio_Errors_Details + __T(")");
                        } else
                            Errors_Stats_Line_Details +=
                                    __T("CH") + Ztring::ToZtring(Channel + 1) + __T(": no valid DIF");
            ...
            //Error 6:
            if (Mpeg4_stts && Mpeg4_stts_Pos < Mpeg4_stts->size() &&
                Speed_FrameCount - 1 >= Mpeg4_stts->at(Mpeg4_stts_Pos).Pos_Begin &&
                Speed_FrameCount - 1 < Mpeg4_stts->at(Mpeg4_stts_Pos).Pos_End) {
                if (!Stats_Total_AlreadyDetected) {
                    Stats_Total_AlreadyDetected = true;
                    Stats_Total++;
                    Stats_Total_WithoutArb++;
        ...
        if (Speed_TimeCode_Current_Theory.IsValid) {
            ...
            if (Speed_TimeCode_Current_Theory.Time.Frames >= Frames_Max) {
                ...
                if (Speed_TimeCode_Current_Theory.Time.Seconds >= 60) {
                    ...
                    if (!DSF && Speed_TimeCode_Current_Theory.Time.DropFrame &&
                        Speed_TimeCode_Current_Theory.Time.Minutes % 10)
    ...
    void File_DvDif::Errors_Stats_Update_Finnish() {
        ...
        if (Speed_FrameCount_Video_STA_Errors)
            Errors_Stats_End_Lines += __T("Frame count with video error concealment: ") +
                                      Ztring::ToZtring(Speed_FrameCount_Video_STA_Errors) + __T(" frames &");
        if (!Video_STA_Errors_Total.empty()) {
        ...
        if (!Audio_Errors_Total.empty()) {
            for (size_t Channel = 0; Channel < 8; Channel++) {
                if (Speed_FrameCount_Audio_Errors[Channel])
                    Errors_Stats_End_Lines +=
                            __T("Frame count with CH") + Ztring::ToZtring(Channel + 1) + __T(" audio error code: ") +
                            Ztring::ToZtring(Speed_FrameCount_Audio_Errors[Channel]) + __T(" frames &");
                ...
                if (Errors_Details.size() > 2) {
                    Errors_Stats_End_Lines +=
                            __T("Total audio error code for CH") + Ztring::ToZtring(Channel + 1) + __T(": ");
                    Ztring Errors_Count_Padded = Ztring::ToZtring(Errors_Count);
                    if (Errors_Count_Padded.size() < 8)
                        Errors_Count_Padded.insert(0, 8 - Errors_Count_Padded.size(), __T(' '));
                    Errors_Stats_End_Lines += __T(" ") + Errors_Count_Padded + __T(" errors");
        ...
        if (Speed_FrameCount_Timecode_Incoherency)
            Errors_Stats_End_Lines += __T("Frame count with DV timecode incoherency: ") +
                                      Ztring::ToZtring(Speed_FrameCount_Timecode_Incoherency) + __T(" frames &");

        //Error 4: DIF incohereny
        if (Speed_FrameCount_Contains_NULL)
            Errors_Stats_End_Lines +=
                    __T("Frame count with DIF incoherency: ") + Ztring::ToZtring(Speed_FrameCount_Contains_NULL) +
                    __T(" frames &");

        //Error 5: Arbitrary bit inconsistency
        if (Speed_FrameCount_Arb_Incoherency)
            Errors_Stats_End_Lines += __T("Frame count with Arbitrary bit inconsistency: ") +
                                      Ztring::ToZtring(Speed_FrameCount_Arb_Incoherency) + __T(" frames &");

        //Error 6: Stts fluctuation
        if (Speed_FrameCount_Stts_Fluctuation)
            Errors_Stats_End_Lines +=
                    __T("Frame count with stts fluctuation: ") + Ztring::ToZtring(Speed_FrameCount_Stts_Fluctuation) +
                    __T(" frames &");
        ...
        if (!Speed_RecDateZ_Current.empty() || !Speed_RecTimeZ_Current.empty()) //Date and Time must be both available
        {
            ...
            if (Speed_TimeStampsZ_Pos) {
                ...
                if (Speed_TimeStampsZ[Speed_TimeStampsZ_Pos].Last.FramePos -
                    (Speed_TimeStampsZ_Pos ? Speed_TimeStampsZ[Speed_TimeStampsZ_Pos - 1].Last.FramePos : 0) ==
                    1) //Only one frame, the "Last" part is not filled
                ...
                if (Speed_TimeStampsZ[Speed_TimeStampsZ_Pos].Last.FramePos -
                    (Speed_TimeStampsZ_Pos ? Speed_TimeStampsZ[Speed_TimeStampsZ_Pos - 1].Last.FramePos : 0) == 1) {
                ...
                for (size_t Pos = 0; Pos < Speed_TimeStampsZ.size(); Pos++) {
                    ...
                    //TimeCode_range
                    Errors_Stats_End_Lines += Speed_TimeStampsZ[Pos].First.TimeCode.empty() ? Ztring(__T("XX:XX:XX:XX"))
                                                                                            : Speed_TimeStampsZ[Pos].First.TimeCode;

                    Errors_Stats_End_Lines += __T(" - ");

                    Errors_Stats_End_Lines += Speed_TimeStampsZ[Pos].Last.TimeCode.empty() ? Ztring(__T("XX:XX:XX:XX"))
                                                                                           : Speed_TimeStampsZ[Pos].Last.TimeCode;

                    Errors_Stats_End_Lines += __T("\t");

                    //Recorded date/time_range
                    Errors_Stats_End_Lines += Speed_TimeStampsZ[Pos].First.Date.empty() ? Ztring(__T("XXXX-XX-XX"))
                                                                                        : Speed_TimeStampsZ[Pos].First.Date;
                    Errors_Stats_End_Lines += __T(' ');
                    Errors_Stats_End_Lines += Speed_TimeStampsZ[Pos].First.Time.empty() ? Ztring(__T("XX:XX:XX:XX"))
                                                                                        : Speed_TimeStampsZ[Pos].First.Time;

                    Errors_Stats_End_Lines += __T(" - ");

                    Errors_Stats_End_Lines += Speed_TimeStampsZ[Pos].Last.Date.empty() ? Ztring(__T("XXXX-XX-XX"))
                                                                                       : Speed_TimeStampsZ[Pos].Last.Date;
                    Errors_Stats_End_Lines += __T(' ');
                    Errors_Stats_End_Lines += Speed_TimeStampsZ[Pos].Last.Time.empty() ? Ztring(__T("XX:XX:XX:XX"))
                                                                                       : Speed_TimeStampsZ[Pos].Last.Time;
        ...
        Fill(Stream_Video, 0, "Errors_Stats_Begin",
             "Frame # \tAbsolute time\tDV timecode\tN\tRecorded date/time     \tN\tA\tN\tS\tE\t1\t2\t3\t4\t5\t6\t7\t8\t9\t0\t1\t2\t3\t4\t5\t6\t7\t8\t9\t0");
