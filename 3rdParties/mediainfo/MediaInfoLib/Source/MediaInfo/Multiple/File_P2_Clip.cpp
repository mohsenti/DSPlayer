                ...
                if (ClipContent) {
                    ...
                    if (Duration_Frames && EditUnit_Numerator && EditUnit_Denominator)
                        Fill(Stream_General, 0, General_Duration,
                             ((float32) Duration_Frames) * 1000 * EditUnit_Numerator / EditUnit_Denominator, 0);
                        ...
                        while (Track) {
                            ...
                            if (Field == "Video") {
                                ...
                                if (ChildElement) {
                                    ...
                                    if (Text.size() == 11) {
                                        int64u ToFill = (Text[0] - '0') * 10 * 60 * 60 * 1000
                                                        ...
                                                        + (Text[7] - '0') * 1000;
                                        if (FrameRate)
                                            ToFill += float64_int64s(
                                                    ((Text[9] - '0') * 10 + (Text[10] - '0')) * 1000 / FrameRate);
                                ...
                                if (File_Name.size() > 10 + 1 + 4 + 1
                                    ...
                                    Dirs = Dir::GetAllFileNames(
                                            File_Name.substr(0, File_Name.size() - 10 - 1) + PathSeparator + MXF_File +
                                            __T(".MXF"), Dir::Include_Files);
                                    Exists = false;
                                    for (size_t Pos = 0; Pos < Dirs.size(); Pos++)
                                        if (Dirs[Pos].size() > 4
                        ...
                        if (Device) {
                            ...
                            if (Manufacturer && ModelName)
                                Fill(Stream_General, 0, General_Encoded_Application,
                                     string(Manufacturer->GetText()) + " " + ModelName->GetText());
                        }
                        ...
                        if (Shoot) {
                            ...
                            if (Location) {
                                ...
                                if (Longitude && Latitude)
                                    Fill(Stream_General, 0, General_Recorded_Location,
                                         string(Latitude->GetText()) + ", " + Longitude->GetText());
                            }
                        }
