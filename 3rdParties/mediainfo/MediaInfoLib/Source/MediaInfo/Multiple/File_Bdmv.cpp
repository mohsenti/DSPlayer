...
//---------------------------------------------------------------------------
    static const char *Bdmv_Type(int32u Type_Indicator, size_t Start_Adress_Pos) {
        switch (Type_Indicator) {
            case Elements::CLPI :
                return Clpi_Offsets[Start_Adress_Pos];
            case Elements::INDX :
                return Indx_Offsets[Start_Adress_Pos];
            case Elements::MOBJ :
                return Mobj_Offsets[Start_Adress_Pos];
            case Elements::MPLS :
                return Mpls_Offsets[Start_Adress_Pos];
            default             :
                return "";
        }
    }
    ...
    static const char *Clpi_Format(int8u StreamType) {
        switch (StreamType) {
            ...
            case 0x92 :
                return "Subtitle";
            case 0xA1 :
                return "E-AC-3"; //Secondary
            case 0xA2 :
                return "DTS"; //Secondary
            case 0xEA :
                return "VC-1";
            default   :
                return "";
...
//---------------------------------------------------------------------------
    void File_Bdmv::Read_Buffer_Continue() {
        ...
        FILLING_BEGIN() ;
            ...
            switch (type_indicator) {
                case Elements::CLPI :
                    Fill(Stream_General, 0, General_Format, "Blu-ray Clip info");
                    break;
                case Elements::INDX :
                    Fill(Stream_General, 0, General_Format, "Blu-ray Index");
                    break;
                case Elements::MOBJ :
                    Fill(Stream_General, 0, General_Format, "Blu-ray Movie object");
                    break;
                case Elements::MPLS :
                    Fill(Stream_General, 0, General_Format, "Blu-ray Playlist");
                    break;
                default             :;
            }
        FILLING_END();
...
//---------------------------------------------------------------------------
    void File_Bdmv::BDMV() {
        ...
        ZtringList List = Dir::GetAllFileNames(
                File_Name + PathSeparator + __T("PLAYLIST") + PathSeparator + __T("*.mpls"), Dir::Include_Files);
        ...
        if (Dir::Exists(File_Name + PathSeparator + __T("BDJO")) &&
            !Dir::GetAllFileNames(File_Name + PathSeparator + __T("BDJO")).empty())
        ...
        if (FileName::Extension_Get(File_Name).empty())
            ...
            Fill(Stream_General, 0, General_FileName,
                 FileName::Name_Get(File_Name) + __T('.') + FileName::Extension_Get(File_Name), true);
        File_Name.clear();
    ...
    void File_Bdmv::Clpi_ProgramInfo() {
        ...
        for (int8u program_sequence = 0; program_sequence < number_of_program_sequences; program_sequence++) {
            ...
            for (int16u Pos = 0; Pos < number_of_streams_in_ps; Pos++) {
                ...
                Get_B1(stream_type, "Stream type");
                Param_Info1(Clpi_Format(stream_type));
                Element_Info1(Clpi_Format(stream_type));
                switch (Clpi_Type(stream_type)) {
                    case Stream_Video :
                        StreamCodingInfo_Video();
                        break;
                    case Stream_Audio :
                        StreamCodingInfo_Audio();
                        break;
                    case Stream_Text  :
                        StreamCodingInfo_Text();
                        break;
                    default           :;
                }
...
//---------------------------------------------------------------------------
    void File_Bdmv::Indx_Indexes() {
        ...
        Get_S1(2, FirstPlayback_object_type, "object_type");
        Param_Info1(Indx_object_type[FirstPlayback_object_type]);
        Skip_S4(30, "reserved");
        BS_End();
        ...
        Param_Info1(Indx_object_type[TopMenu_object_type]);
        Skip_S4(30, "reserved");
        BS_End();
        ...
        for (int16u Pos = 0; Pos < number_of_Titles; Pos++) {
            ...
            Get_S1(2, Title_object_type, "object_type");
            Param_Info1(Indx_object_type[Title_object_type]);
            Info_S1(2, Title_title_search, "title_search");
            Param_Info1(Indx_title_search[Title_title_search]);
            Skip_S4(28, "reserved");
            BS_End();
...
//---------------------------------------------------------------------------
    void File_Bdmv::Indx_Indexes_Index(int8u object_type) {
        BS_Begin();
        Info_S1(2, playback_type, "playback_type");
        Param_Info1(Indx_playback_type[object_type][playback_type]);
        Skip_S2(14, "reserved");
        BS_End();
        switch (object_type) {
            case 1 : //HDMV
            {
                Info_B2(id_ref, "id_ref");
                Element_Info1(id_ref);
                Skip_B4("reserved");
            }
                break;
            case 2 : //BD-J
            {
                Info_Local(5, id_ref, "id_ref");
                Element_Info1(id_ref);
                Skip_B1("reserved");
            }
                break;
    ...
    void File_Bdmv::Indx_ExtensionData() {
        ...
        for (std::map<int32u, int32u>::iterator ext = exts.begin(); ext != exts.end(); ++ext) {
            if (Base_Offset + ext->first >= Element_Offset) {
                ...
                int32u type_indicator;
                Get_C4(type_indicator, "type_indicator");
                Element_Info1(Ztring().From_CC4(type_indicator));
                switch (type_indicator) {
                    case 0x49444558 :
                        Indx_ExtensionData_IDEX();
                        break;
                    default         :
...
//---------------------------------------------------------------------------
    void File_Bdmv::Mobj_MovieObjects() {
        ...
        for (int16u mobjs_Pos = 0; mobjs_Pos < number_of_mobjs; mobjs_Pos++) {
            ...
            BS_Begin();
            Info_SB(resume, "resume");
            Param_Info1(resume ? "suspend" : "discard");
            Info_SB(menu_call, "menu_call");
            Param_Info1(menu_call ? "enable" : "disable");
            Info_SB(title_search, "title_search");
            Param_Info1(title_search ? "enable" : "disable");
            Skip_BS(13, "reserved");
            BS_End();
            Get_B2(number_of_navigation_commands, "number_of_navigation_commands");
            for (int16u navigation_command_Pos = 0;
                 navigation_command_Pos < number_of_navigation_commands; navigation_command_Pos++) {
...
//---------------------------------------------------------------------------
    void File_Bdmv::Mpls_AppInfoPlayList() {
        ...
        BS_Begin();
        Skip_S1(6, "unknown");
        Info_S2(2, playback_type, "playback_type");
        Param_Info1(Mpls_playback_type[playback_type]);
        BS_End();
...
//---------------------------------------------------------------------------
    void File_Bdmv::Mpls_PlayList_PlayItem() {
        ...
        Element_Info1(Clip_Information_file_name);
        Skip_Local(4, "Clip_codec_identifier");
        Skip_B2("unknown");
        Skip_B1("Unknown");
        Get_B4(Time_In, "Time (In)");
        Param_Info1((float32) Time_In / 45000);
        Get_B4(Time_Out, "Time (Out)");
        Param_Info1((float32) Time_Out / 45000);
        Skip_B4("UO1");
        Skip_B4("UO2");
        Skip_B4("An?");
        ...
        if (Clip_Information_file_names.find(Clip_Information_file_name) == Clip_Information_file_names.end() &&
            File_Name.size() > 10 + 1 + 8) {
            ...
            MediaInfo_Internal MI;
            MI.Option(__T("File_Bdmv_ParseTargetedFile"),
                      Config->File_Bdmv_ParseTargetedFile_Get() ? __T("1") : __T("0"));
            MI.Option(__T("File_IsReferenced"), __T("1"));
...
//---------------------------------------------------------------------------
    void File_Bdmv::Mpls_PlayList_PlayItem_STN_table() {
        ...
        while (Element_Offset + 16 <= End) {
            ...
            Skip_B1("unknown");
            Get_B2(mPID, "mPID");
            Element_Name(Ztring::ToZtring(mPID, 16));
            Skip_B2("SPid");
            Skip_B2("sCid");
            Skip_B2("sPID");
            ...
            switch (Clpi_Type(stream_type)) {
                case Stream_Video :
                    Mpls_PlayList_PlayItem_STN_table_Video();
                    break;
                case Stream_Audio :
                    Mpls_PlayList_PlayItem_STN_table_Audio();
                    break;
                case Stream_Text  :
                    Mpls_PlayList_PlayItem_STN_table_Text();
                    break;
                default           :
                    StreamKind_Last = Stream_Max;
            }
            ...
            FILLING_BEGIN() ;
                if (StreamKind_Last != Stream_Max) {
                    ...
                    Fill(StreamKind_Last, StreamPos_Last, Fill_Parameter(StreamKind_Last, Generic_Duration),
                         Mpls_PlayList_PlayItem_Duration / 45);
                }
            FILLING_END();
        }
...
//---------------------------------------------------------------------------
    void File_Bdmv::Mpls_PlayList_PlayItem_STN_table_Video() {
        ...
        Param_Info1(Clpi_Video_Format[Format]);
        Get_S1(4, FrameRate, "frame_rate");
        Param_Info1(Clpi_Video_FrameRate[FrameRate]);
        BS_End();
...
//---------------------------------------------------------------------------
    void File_Bdmv::Mpls_PlayList_PlayItem_STN_table_Audio() {
        ...
        BS_Begin();
        Get_S1(4, Channels, "channel_layout");
        Param_Info1(Clpi_Audio_Channels[Channels]);
        Get_S1(4, SamplingRate, "sampling_rate");
        Param_Info1(Clpi_Audio_SamplingRate[SamplingRate]);
        BS_End();
...
//---------------------------------------------------------------------------
    void File_Bdmv::Mpls_PlayList_SubPlayItem(int8u SubPath_type, int16u Pos) {
        ...
        Skip_B4("unknown");
        Skip_B1("unknown");
        Info_B4(Time_In, "time (in)");
        Param_Info1((float32) Time_In / 45000);
        Info_B4(Time_Out, "time (out)");
        Param_Info1((float32) Time_Out / 45000);
        Skip_B2("sync PI");
        Skip_B4("sync PTS");
                ...
                if (File_Name.size() >= 10 + 1 + 8) {
                    ...
                    if (MI.Open(CLPI_File)) {
                        if (MI.Count_Get(Stream_Video)) {
                            ...
                            Fill(Stream_Video, Pos, Video_ID, MI.Get(Stream_Video, 0, Video_ID) + __T(" / ") + ID,
                                 true);
                            Fill(Stream_Video, Pos, Video_ID_String,
                                 MI.Get(Stream_Video, 0, Video_ID_String) + __T(" / ") + ID_String, true);
                            if (!Format_Profile.empty())
                                Fill(Stream_Video, Pos, Video_Format_Profile,
                                     MI.Get(Stream_Video, 0, Video_Format_Profile) + __T(" / ") + Format_Profile, true);
                            if (!BitRate.empty())
                                Fill(Stream_Video, Pos, Video_BitRate, Ztring::ToZtring(
                                        BitRate.To_int32u() + MI.Get(Stream_Video, 0, Video_BitRate).To_int32u()) +
                                                                       __T(" / ") + BitRate, true);
                            if (!Source.empty())
                                Fill(Stream_Video, Pos, "Source", Clip_Information_file_name + __T(".m2ts / ") + Source,
                                     true);
                        }
                    }
                }
...
//---------------------------------------------------------------------------
    void File_Bdmv::Mpls_PlayListMarks() {
        ...
        for (int16u Pos = 0; Pos < count; Pos++) {
            ...
            Skip_B1("unknown");
            Get_B1(type, "type");
            Param_Info1(Mpls_PlayListMarks_Mark_type(type));
            switch (type) {
                case 1 : //entry-mark
                case 2 : //link point
                {
                    ...
                    Get_B4(time, "time");
                    Param_Info2(time / 45, " milliseconds");
                    Skip_B2("unknown");
                    Skip_B4("unknown");
                    ...
                    FILLING_BEGIN() ;
                        ...
                        {
                            Fill(Stream_Menu, 0, Ztring().Duration_From_Milliseconds(
                                    (int64u) ((time - time_Pos0) / 45)).To_UTF8().c_str(),
                                 __T("Chapter ") + Ztring::ToZtring(time_Pos));
                            time_Pos++;
                        }
                    FILLING_END();
                }
            ...
            if (Entry->first >= Element_Offset) //If valid
            {
                ...
                switch (Entry->second.ID1) {
                    case 0x0001 :
                        switch (Entry->second.ID2) {
                            case 0x0001 :
                                break; //Mpls_ExtensionData_pip_metadata(); break;
                            default:;
                        }
                        break;
                    case 0x0002 :
                        switch (Entry->second.ID2) {
                            case 0x0001 :
                                break; //Mpls_ExtensionData_STN_table(); break;
                            case 0x0002 :
                                Mpls_ExtensionData_SubPath_entries();
                                break;
                            case 0x0003 :
                                break; //Mpls_ExtensionData_active_video_window(); break;
                            default:;
                        }
                        break;
                    default             :;
...
//---------------------------------------------------------------------------
    void File_Bdmv::StreamCodingInfo_Video() {
        ...
        Get_S1(4, Format, "Format");
        Param_Info1(Clpi_Video_Format[Format]);
        Get_S1(4, FrameRate, "Frame rate");
        Param_Info1(Clpi_Video_FrameRate[FrameRate]);
        Get_S1(4, AspectRatio, "Aspect ratio");
        Param_Info1(Clpi_Video_AspectRatio[AspectRatio]);
        Skip_BS(4, "Reserved");
        BS_End();
        ...
        FILLING_BEGIN() ;
            if (StreamKind_Last == Stream_Max) {
                ...
                if (Clpi_Video_Height[AspectRatio])
                    Fill(Stream_Video, StreamPos_Last, Video_DisplayAspectRatio, Clpi_Video_AspectRatio[AspectRatio], 3,
                         true);
            }
        FILLING_END();
    }
...
//---------------------------------------------------------------------------
    void File_Bdmv::StreamCodingInfo_Audio() {
        ...
        int8u Channels, SamplingRate;
        BS_Begin();
        Get_S1(4, Channels, "Channel layout");
        Param_Info1(Clpi_Audio_Channels[Channels]);
        Get_S1(4, SamplingRate, "Sampling Rate");
        Param_Info1(Clpi_Audio_SamplingRate[SamplingRate]);
        BS_End();
        Get_Local(3, Language, "Language");
        Element_Info1(Language);
