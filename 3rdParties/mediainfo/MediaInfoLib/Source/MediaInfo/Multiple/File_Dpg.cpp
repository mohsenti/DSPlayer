...
//---------------------------------------------------------------------------
    void File_Dpg::FileHeader_Parse() {
        ...
        Get_L4(FrameRate, "Frame rate");
        Param_Info2(FrameRate / 0x100, " fps");
    ...
    void File_Dpg::Read_Buffer_Continue() {
        ...
        if (Audio_Size) {
            ...
            Open_Buffer_Continue(Parser,
                                 (size_t) ((File_Offset + Buffer_Size < Audio_Offset + Audio_Size) ? Buffer_Size : (
                                         Audio_Offset + Audio_Size - File_Offset)));
...
#if defined(MEDIAINFO_MPEGV_YES)
            Open_Buffer_Continue(Parser,
                                 (size_t) ((File_Offset + Buffer_Size < Video_Offset + Video_Size) ? Buffer_Size : (
                                         Video_Offset + Video_Size - File_Offset)));
            if (Parser->Status[IsAccepted]) {
                //Merging
                Parser->Open_Buffer_Unsynch();
                Finish(Parser);
