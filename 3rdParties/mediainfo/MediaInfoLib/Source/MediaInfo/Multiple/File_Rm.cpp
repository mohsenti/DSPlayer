...
//---------------------------------------------------------------------------
    void File_Rm::CONT() {
        ...
        Get_Local(title_len, title,
                  "title"); //An array of ASCII characters that represents the title information for the RealMedia file.
        Get_B2(author_len, "author_len"); //The length of the author data in bytes.
        Get_Local(author_len, author,
                  "author"); //An array of ASCII characters that represents the author information for the RealMedia file.
        Get_B2(copyright_len, "copyright_len"); //The length of the copyright data in bytes.
        Get_Local(copyright_len, copyright,
                  "copyright"); //An array of ASCII characters that represents the copyright information for the RealMedia file.
        Get_B2(comment_len, "comment_len"); //The length of the comment data in bytes.
        Get_Local(comment_len, comment,
                  "comment"); //An array of ASCII characters that represents the comment information for the RealMedia file.
...
//---------------------------------------------------------------------------
    void File_Rm::MDPR() {
        ...
        Get_B4(start_time,
               "start_time"); //The time offset in milliseconds to add to the time stamp of each packet in a physical stream.
        ...
        Get_String(mime_type_size, mime_type,
                   "mime_type"); //A nonunique MIME style type/subtype string for data associated with the stream.
        ...
        FILLING_BEGIN() ;
            if (MDPR_IsStream) {
                ...
                Fill(StreamKind_Last, StreamPos_Last, Fill_Parameter(StreamKind_Last, Generic_BitRate), avg_bit_rate,
                     10, true);
...
//---------------------------------------------------------------------------
    void File_Rm::MDPR_realvideo() {
        ...
        switch (FrameRate) {
            case 23 :
                Fill(Stream_Video, StreamPos_Last, Video_FrameRate, ((float) 24) * 1000 / 1001);
                break; //Hard-coded frame rate?
            case 29 :
                Fill(Stream_Video, StreamPos_Last, Video_FrameRate, ((float) 30) * 1000 / 1001);
                break; //Hard-coded frame rate?
            default :
                Fill(Stream_Video, StreamPos_Last, Video_FrameRate, (float) FrameRate);
        }
    }
...
//---------------------------------------------------------------------------
    void File_Rm::MDPR_fileinfo() {
        ...
        Get_B2(Version, "object_version");
        INTEGRITY_VERSION(0);
        Get_B2(num_physical_streams,
               "num_physical_streams"); //The number of physical streams that make up this logical stream.
        ...
        Get_B2(num_properties,
               "num_properties"); //The number of NameValueProperty structures contained in this structure. These name/value structures can be used to identify properties of this logical stream (for example, language).
        ...
        for (int16u Pos = 0; Pos < num_properties; Pos++) {
            ...
            switch (type) {
                case 0 : //Unsigned integer
                    Skip_B4("value_data");
                    break; //unsigned integer
                case 2 : //String
                    Skip_Local(value_length, "value_data");
                    break; //string
                default :
...
//---------------------------------------------------------------------------
    void File_Rm::RJMD_property(std::string Name) {
        ...
        Get_B4(num_subproperties,
               "num_subproperties"); //The number of subproperties for this MetadataProperty  structure.
