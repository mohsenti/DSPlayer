        ...
        if (Complete_Stream && !Complete_Stream->Streams.empty()) {
            ...
            if (Config->File_Filter_HasChanged()) {
                ...
                for (int32u Pos = 0x01; Pos < 0x10; Pos++)
                    Complete_Stream->Streams[Pos]->Searching_Payload_Start_Set(
                            Searching_Payload_Start); //base PID depends of File_Filter configuration
            ...
            if (File__Duplicate_HasChanged()) {
                ...
                for (complete_stream::transport_stream::programs::const_iterator Program = transportItem.Programs.begin();
                     Program != transportItem.Programs.end(); ++Program) {
                    //Do we want this program?
                    bool Wanted = false;
                    for (std::map<const String, File__Duplicate_MpegTs *>::const_iterator Duplicate = Complete_Stream->Duplicates.begin();
...
//---------------------------------------------------------------------------
    ...
    bool File_MpegTs::File__Duplicate_Set(const Ztring &Value) {
        ...
        {
            for (std::vector<ZtringList::iterator>::iterator Order = Orders_ToAdd.begin();
                 Order < Orders_ToAdd.end(); ++Order)
                Orders_ToRemove.push_back(*Order);
            Orders_ToAdd.clear();
        }

        //For each target to add
        for (std::vector<ZtringList::iterator>::iterator Target = Targets_ToAdd.begin();
             Target < Targets_ToAdd.end(); ++Target) {
            ...
            for (std::vector<ZtringList::iterator>::iterator Order = Orders_ToRemove.begin();
                 Order < Orders_ToRemove.end(); ++Order)
                Complete_Stream->Duplicates[**Target]->Configure(**Order, true);
        }

        //For each target to remove
        for (std::vector<ZtringList::iterator>::iterator Target = Targets_ToRemove.begin();
             Target < Targets_ToRemove.end(); ++Target) {
            std::map<const String, File__Duplicate_MpegTs *>::iterator Pointer = Complete_Stream->Duplicates.find(
                    **Target);
            if (Pointer != Complete_Stream->Duplicates.end()) {
                //Duplicates_Speed
                for (std::vector<File__Duplicate_MpegTs *>::iterator Duplicate = Complete_Stream->Duplicates_Speed.begin();
                     Duplicate < Complete_Stream->Duplicates_Speed.end(); ++Duplicate)
                    if (*Duplicate == Pointer->second)
                        *Duplicate = NULL;

                //Duplicates_Speed_FromPID
                for (std::vector<std::vector<File__Duplicate_MpegTs *> >::iterator Duplicate_FromPID = Complete_Stream->Duplicates_Speed_FromPID.begin();
                     Duplicate_FromPID < Complete_Stream->Duplicates_Speed_FromPID.end(); ++Duplicate_FromPID)
                    for (std::vector<File__Duplicate_MpegTs *>::iterator Duplicate = Duplicate_FromPID->begin();
                         Duplicate < Duplicate_FromPID->end(); ++Duplicate)
                        if (*Duplicate == Pointer->second)
                            *Duplicate = NULL;
    ...
    void File_MpegTs::File__Duplicate_Write() {
        ...
        if (ToUpdate) {
            ...
            for (size_t Pos = 0; Pos < Duplicates_Speed_Size; Pos++) {
                ...
                for (size_t program_map_PIDs_Pos = 0;
                     program_map_PIDs_Pos < program_map_PIDs_Size; program_map_PIDs_Pos++)
                    if (Dup->program_map_PIDs[program_map_PIDs_Pos]) {
                        bool AlreadyPresent = false;
                        for (size_t Duplicates_Speed_FromPID_Pos = 0; Duplicates_Speed_FromPID_Pos <
                                                                      Complete_Stream->Duplicates_Speed_FromPID[program_map_PIDs_Pos].size(); Duplicates_Speed_FromPID_Pos++)
                            if (Complete_Stream->Duplicates_Speed_FromPID[program_map_PIDs_Pos][Duplicates_Speed_FromPID_Pos] ==
                                Dup)
                                AlreadyPresent = true;
                        if (!AlreadyPresent)
                ...
                for (size_t elementary_PIDs_Pos = 0; elementary_PIDs_Pos < elementary_PIDs_Size; elementary_PIDs_Pos++)
                    if (Dup->elementary_PIDs[elementary_PIDs_Pos]) {
                        bool AlreadyPresent = false;
                        for (size_t Duplicates_Speed_FromPID_Pos = 0; Duplicates_Speed_FromPID_Pos <
                                                                      Complete_Stream->Duplicates_Speed_FromPID[elementary_PIDs_Pos].size(); Duplicates_Speed_FromPID_Pos++)
                            if (Complete_Stream->Duplicates_Speed_FromPID[elementary_PIDs_Pos][Duplicates_Speed_FromPID_Pos] ==
                                Dup)
                                AlreadyPresent = true;
                        if (!AlreadyPresent)
...
//---------------------------------------------------------------------------
    ...
    size_t File_MpegTs::Output_Buffer_Get(size_t Pos) {
        if (Complete_Stream != NULL && Pos < Complete_Stream->Duplicates_Speed.size() &&
            Complete_Stream->Duplicates_Speed[Pos] != NULL)
