...
//***************************************************************************
    ...
    protected :
        ...
        void Streams_Finish_Component_ForTimeCode(const int128u ComponentUID, float64 EditRate, int32u TrackID,
                                                  int64s Origin, bool IsSourcePackage);

        void
        Streams_Finish_Component_ForAS11(const int128u ComponentUID, float64 EditRate, int32u TrackID, int64s Origin);
        ...
        void
        DMSegment_Duration();                                  //0202 (copy of StructuralComponent_Duration) //TODO: merge with StructuralComponent_Duration
...
#define Info_UL(_INFO, _NAME, _PARAM) int128u _INFO;
        ...
        struct mxftimecode {
            ...
            mxftimecode(int16u RoundedTimecodeBase_ = 0, int64u StartTimecode_ = (int64u) -1, bool DropFrame_ = false)
                    : RoundedTimecodeBase(RoundedTimecodeBase_), StartTimecode(StartTimecode_), DropFrame(DropFrame_) {
            }
            ...
            float64 Get_TimeCode_StartTimecode_Temp(const int64u &File_IgnoreEditsBefore) const {
                if (RoundedTimecodeBase) {
                    float64 TimeCode_StartTimecode_Temp =
                            ((float64) (StartTimecode + File_IgnoreEditsBefore)) / RoundedTimecodeBase;
                    if (DropFrame) {
        ...
        essences Essences;
        std::bitset<Stream_Max +
                    1> StreamPos_StartAtZero; //information about the base of StreamPos (0 or 1, 1 is found in 1 file) TODO: per Essence code (last 4 bytes of the Essence header 0xTTXXTTXX)
        ...
        void
        ChooseParser__FromEssenceContainer(const essences::iterator &Essence, const descriptors::iterator &Descriptor);
        ...
        void AcquisitionMetadata_Sony_E201_Add(size_t Id, const string &Value) {
            ...
            if ((*AcquisitionMetadata_Sony_E201_Lists[Id])[AcquisitionMetadata_Sony_E201_Lists[Id]->size() - 1].Value ==
                Value) {
                (*AcquisitionMetadata_Sony_E201_Lists[Id])[AcquisitionMetadata_Sony_E201_Lists[Id]->size() -
                                                           1].FrameCount++;
                return;
            }
            AcquisitionMetadata_Sony_E201_Lists[Id]->push_back(acquisitionmetadata(Value));
