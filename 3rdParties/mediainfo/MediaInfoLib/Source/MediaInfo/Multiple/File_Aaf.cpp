...
//---------------------------------------------------------------------------
    void File_Aaf::Header_Parse() {
        switch (Step) {
            case Step_Fat           :
                Header_Fill_Code(0, "FAT");
                Header_Fill_Size(((int64u) 1) << SectorShift);
                break;
            case Step_MiniFat       :
                Header_Fill_Code(0, "MiniFAT");
                Header_Fill_Size(((int64u) 1) << SectorShift);
                break;
            case Step_Directory     :
...
//---------------------------------------------------------------------------
    void File_Aaf::Fat() {
        ...
        while (Element_Offset < Element_Size) {
            int32u Pointer;
            Get_L4(Pointer, "Pointer");
            Param_Info1(Ztring::ToZtring(Pointers.size()));
...
//---------------------------------------------------------------------------
    void File_Aaf::MiniFat() {
        ...
        while (Element_Offset < Element_Size) {
            int32u Pointer;
            Get_L4(Pointer, "Pointer");
            Param_Info1(Ztring::ToZtring(MiniPointers.size()));
            //Pointer=LittleEndian2int32u(Buffer+Buffer_Offset+(size_t)Element_Offset);
            //Element_Offset+=4;
...
//---------------------------------------------------------------------------
    void File_Aaf::Directory_Entry() {
        ...
        Skip_L2("cb");
        Get_L1(mse, "mse");
        Element_Info1(AAf_tagSTGTY(mse));
        Info_L1(flags, "flags");
        Element_Info1(AAf_tagDECOLOR(flags));
        Skip_L4("LeftSib SID");
        Skip_L4("RightSib SID");
        Skip_L4(mse == 2 ? "0" : "Child SID"); //Zero if stream
        Skip_L16(mse == 2 ? "0" : "clsId");//Zero if stream
        Skip_L4("UserFlags");
        Info_L8(CreationTime, mse == 2 ? "0" : "Create time");
        if (mse != 2) {
            Param_Info1(CreationTime ? Ztring().Date_From_Milliseconds_1601(CreationTime / 10000) : Ztring());
        } //Zero if stream
        Info_L8(ModificationTime, mse == 2 ? "0" : "Modify time");
        if (mse != 2) {
            Param_Info1(CreationTime ? Ztring().Date_From_Milliseconds_1601(ModificationTime / 10000) : Ztring());
        } //Zero if stream
        ...
        } else if (mse == 2 && Size) //If stream
        {
            ...
            if (Size < MiniSectorCutoff) //MiniFAT
            {
                ...
                while (Pointers_Pos < MiniPointers.size()) {
                    ...
                    Stream->StreamOffsets.push_back(
                            ((1 + sectsMiniStream[SectPos]) << SectorShift) + (MiniSectPos << MiniSectorShift));
                    Param_Info1(Ztring::ToZtring(
                            ((1 + sectsMiniStream[SectPos]) << SectorShift) + (MiniSectPos << MiniSectorShift)));
...
//---------------------------------------------------------------------------
    void File_Aaf::StreamElement() {
        if (Streams_Pos >= Streams.size() ||
            Streams[Streams_Pos]->Size >= 0x1000000) //TODO: more serious test about size
            return; //Incoherancy
        ...
        if (Streams[Streams_Pos]->StreamOffsets.size() != 1) {
            ...
            if (Streams[Streams_Pos]->Buffer == NULL)
                Streams[Streams_Pos]->Buffer = new int8u[(size_t) ((1 + (Streams[Streams_Pos]->Size >> Shift))
                        << Shift)];
            memcpy(Streams[Streams_Pos]->Buffer + Streams_Pos2 * (((int64u) 1) << Shift), Buffer + Buffer_Offset,
                   (size_t) Element_Size);
        }
        ...
        for (int16u Pos = 0; Pos < Count; Pos++) {
            ...
            switch (Keys[Pos]) {
                ...
                case 0x0002 :
                    ...
                    break;
                case 0x0003 :
                    Element_Name("ClassDefinitions");
                    ClassDefinitions();
                    break;
                case 0x0004 :
                    Element_Name("TypeDefinitions");
                    TypeDefinitions();
                    break;
                case 0x0005 :
                ...
                case 0x0006 :
                    ...
                    break;
                case 0x0007 :
                    Element_Name("MetaDefinition");
                    MetaDefinition();
                    break;
                case 0x0008 :
                    Element_Name("ParentClass");
                    ParentClass();
                    break;
                case 0x0009 :
                ...
                case 0x000B :
                    ...
                    break;
                case 0x000C :
                    Element_Name("IsOptional");
                    IsOptional();
                    break;
                case 0x000D :
                    Element_Name("LocalIdentification");
                    LocalIdentification();
                    break;
                case 0x000E :
                    Element_Name("IsUniqueIdentifier");
                    IsUniqueIdentifier();
                    break;
                case 0x000F :
                    Element_Name("Size");
                ...
                case 0x3D02 :
                    ...
                    break;
                case 0x4001 :
                    Element_Name("NetworkLocator");
                    NetworkLocator();
                    break;
                default     :
                    Skip_XX(xxxSize, "Unknown");
            }
            Element_End0();
        }
