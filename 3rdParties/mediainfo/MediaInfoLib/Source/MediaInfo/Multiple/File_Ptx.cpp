...
//---------------------------------------------------------------------------
    void File_Ptx::Read_Buffer_Continue() {
        ...
        for (int32u Pos = 0; Pos < Info_Count; Pos++) {
            ...
            if (Info_Length) {
                Info_UTF8(Info_Length, Info, "Name");
                Element_Info1(Info);
            }
            Element_End();
        }
        ...
        for (int16u Pos = 0; Pos < 1 + Names_Count; Pos++) {
            ...
            if (Name_Length) {
                Info_UTF8(Name_Length, Name, "Name");
                Element_Name(Name);
            }
            Element_End();
        }
        ...
        for (int16u Pos = 0; Pos < Audio_Count; Pos++) {
            ...
            Skip_L4("(Same 1/2/3) Unknown");
            Info_L3(Number, "(Same 1/2/3) Ordered number");
            Element_Info1(Number);
            Element_Info1(Name);
            Element_End();
        ...
        for (int32u Pos = 0; Pos < FileName_Count; Pos++) {
            ...
            Get_UTF8(FileName_Length, FileName, "File Name");
            ...
            if (Names.size() > 1 || FileNames.size() == 1) {
                ...
                for (int32u Pos = 0; Pos < FileName_Count; Pos++) {
                    if (Roles[Pos] == 0x02
                        ...
                        && FileNamesLowerCase[Pos].find(Names[Pos - Pos_Offset] + __T(".wav")) +
                           Names[Pos - Pos_Offset].size() + 4 == FileNames[Pos].size()) {
