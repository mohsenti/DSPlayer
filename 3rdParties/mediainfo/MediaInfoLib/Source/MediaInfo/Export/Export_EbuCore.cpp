...
//---------------------------------------------------------------------------
    ...
    Ztring EbuCore_VideoCompressionCodeCS_Name(int32u termID, MediaInfo_Internal &MI,
...
//---------------------------------------------------------------------------
    ...
    Ztring EbuCore_AudioCompressionCodeCS_Name(int32u termID, MediaInfo_Internal &MI,
                                               size_t StreamPos) //xxyyzz: xx=main number, yy=sub-number, zz=sub-sub-number
    {
        switch (termID / 10000) {
            case 4 :
...
//---------------------------------------------------------------------------
    Ztring EbuCore_Transform_Video(Ztring &ToReturn, MediaInfo_Internal &MI, size_t StreamPos,
                                   Export_EbuCore::version Version) {
        ...
        if (!MI.Get(Stream_Video, StreamPos, Video_Format_Version).empty())
            ToReturn +=
                    __T(" videoFormatVersionId=\"") + MI.Get(Stream_Video, StreamPos, Video_Format_Version) + __T("\"");
        ToReturn += __T(">\n");
        ...
        if (!MI.Get(Stream_Video, StreamPos, Video_Width).empty()) {
            ...
            ToReturn += __T("\t\t\t\t<ebucore:width unit=\"pixel\">") + MI.Get(Stream_Video, StreamPos, Video_Width) +
                        __T("</ebucore:width>\n");
        }
        ...
        //lines
        if (!MI.Get(Stream_Video, StreamPos, Video_Height_Original).empty())
            ToReturn += __T("\t\t\t\t<ebucore:lines>") + MI.Get(Stream_Video, StreamPos, Video_Height_Original) +
                        __T("</ebucore:lines>\n");
        ...
        if (!MI.Get(Stream_Video, StreamPos, Video_DisplayAspectRatio).empty()) {
            ...
            if (AspectRatioString_Pos != (size_t) -1) {
                ...
                if (AspectRatioString_Pos != (size_t) -1) {
                    int64s Denominator = float64_int64s(pow((double) 10,
                                                            (int) (AspectRatioString.size() - AspectRatioString_Pos +
                                                                   1))); //Computing the right denomintor compared to the count of decimals in the value e.g. 1.778 will have a denominator of 1000 (3 digits after the comma)
            ...
            ToReturn +=
                    __T("\t\t\t\t\t<ebucore:factorNumerator>") + factorNumerator + __T("</ebucore:factorNumerator>\n");
            ToReturn += __T("\t\t\t\t\t<ebucore:factorDenominator>") + factorDenominator +
                        __T("</ebucore:factorDenominator>\n");
        ...
        //if (!MI.Get(Stream_Video, StreamPos, Video_Format_Profile).empty())
        {
            ...
            if (!TermID_String.empty())
                ToReturn += __T(" typeLink=\"http://www.ebu.ch/metadata/cs/ebu_VideoCompressionCodeCS.xml#") +
                            TermID_String + __T("\"");
            ToReturn += __T("/>\n");
        }
        ...
        if (!MI.Get(Stream_Video, StreamPos, Video_CodecID).empty() ||
            !MI.Get(Stream_Video, StreamPos, Video_Format_Commercial_IfAny).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:codec>\n");
            if (!MI.Get(Stream_Video, StreamPos, Video_CodecID).empty()) {
                ToReturn += __T("\t\t\t\t\t<ebucore:codecIdentifier>\n");
                ToReturn += __T("\t\t\t\t\t\t<dc:identifier>") + MI.Get(Stream_Video, StreamPos, Video_CodecID) +
                            __T("</dc:identifier>\n");
                ToReturn += __T("\t\t\t\t\t</ebucore:codecIdentifier>\n");
            }
            if (!MI.Get(Stream_Video, StreamPos, Video_Format_Commercial_IfAny).empty())
                ToReturn += __T("\t\t\t\t\t<ebucore:name>") +
                            MI.Get(Stream_Video, StreamPos, Video_Format_Commercial_IfAny) + __T("</ebucore:name>\n");
            ToReturn += __T("\t\t\t\t</ebucore:codec>\n");
        }

        //bitRate
        if (!MI.Get(Stream_Video, StreamPos, Video_BitRate).empty())
            ToReturn += __T("\t\t\t\t<ebucore:bitRate>") + MI.Get(Stream_Video, StreamPos, Video_BitRate) +
                        __T("</ebucore:bitRate>\n");

        //bitRateMax
        if (!MI.Get(Stream_Video, StreamPos, Video_BitRate_Maximum).empty())
            ToReturn += __T("\t\t\t\t<ebucore:bitRateMax>") + MI.Get(Stream_Video, StreamPos, Video_BitRate_Maximum) +
                        __T("</ebucore:bitRateMax>\n");
        ...
        if (!MI.Get(Stream_Video, StreamPos, Video_ActiveFormatDescription).empty())
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"ActiveFormatDescription\">") +
                        MI.Get(Stream_Video, StreamPos, Video_ActiveFormatDescription) +
                        __T("</ebucore:technicalAttributeString>\n");

        //technicalAttributeString - Standard
        if (!MI.Get(Stream_Video, StreamPos, Video_Standard).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"Standard\">") +
                        MI.Get(Stream_Video, StreamPos, Video_Standard) + __T("</ebucore:technicalAttributeString>\n");
        }

        //technicalAttributeString - ColorSpace
        if (!MI.Get(Stream_Video, StreamPos, Video_ColorSpace).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"ColorSpace\">") +
                        MI.Get(Stream_Video, StreamPos, Video_ColorSpace) +
                        __T("</ebucore:technicalAttributeString>\n");
        }

        //technicalAttributeString - ChromaSubsampling
        if (!MI.Get(Stream_Video, StreamPos, Video_ChromaSubsampling).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"ChromaSubsampling\">") +
                        MI.Get(Stream_Video, StreamPos, Video_ChromaSubsampling) +
                        __T("</ebucore:technicalAttributeString>\n");
        }

        //technicalAttributeString - colour_primaries
        if (!MI.Get(Stream_Video, StreamPos, __T("colour_primaries")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"colour_primaries\">") +
                        MI.Get(Stream_Video, StreamPos, __T("colour_primaries")) +
                        __T("</ebucore:technicalAttributeString>\n");
        }

        //technicalAttributeString - transfer_characteristics
        if (!MI.Get(Stream_Video, StreamPos, __T("transfer_characteristics")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"transfer_characteristics\">") +
        ...
        if (!MI.Get(Stream_Video, StreamPos, __T("matrix_coefficients")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"matrix_coefficients\">") +
                        MI.Get(Stream_Video, StreamPos, __T("matrix_coefficients")) +
                        __T("</ebucore:technicalAttributeString>\n");
        }

        //technicalAttributeString - colour_range
        if (!MI.Get(Stream_Video, StreamPos, __T("colour_range")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"colour_range\">") +
                        MI.Get(Stream_Video, StreamPos, __T("colour_range")) +
                        __T("</ebucore:technicalAttributeString>\n");
        }

        //technicalAttributeString - StreamSize
        if (!MI.Get(Stream_Video, StreamPos, Video_StreamSize).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeInteger typeLabel=\"StreamSize\"") +
                        (Version >= Export_EbuCore::Version_1_6 ? Ztring(__T(" unit=\"byte\"")) : Ztring()) + __T(">") +
                        MI.Get(Stream_Video, StreamPos, Video_StreamSize) +
                        __T("</ebucore:technicalAttributeInteger>\n");
        }

        //technicalAttributeString - BitDepth
        if (!MI.Get(Stream_Video, StreamPos, Video_BitDepth).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeInteger typeLabel=\"BitDepth\"") +
                        (Version >= Export_EbuCore::Version_1_6 ? Ztring(__T(" unit=\"bit\"")) : Ztring()) + __T(">") +
                        MI.Get(Stream_Video, StreamPos, Video_BitDepth) + __T("</ebucore:technicalAttributeInteger>\n");
        }

        //technicalAttributeString
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("FpaManufacturer")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"FPAManufacturer\">") +
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("FpaVersion")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"FPAVersion\">") +
                        MI.Get(Stream_Other, As11_UkDpp_Pos, __T("FpaVersion")) +
                        __T("</ebucore:technicalAttributeString>\n");
        }
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("VideoComments")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:comment typeLabel=\"VideoComments\">") +
                        MI.Get(Stream_Other, As11_UkDpp_Pos, __T("VideoComments")) + __T("</ebucore:comment>\n");
        }
...
//---------------------------------------------------------------------------
    Ztring EbuCore_Transform_Audio(Ztring &ToReturn, MediaInfo_Internal &MI, size_t StreamPos,
        ...
        if (!MI.Get(Stream_Audio, StreamPos, Audio_Format_Version).empty())
            ToReturn +=
                    __T(" audioFormatVersionId=\"") + MI.Get(Stream_Audio, StreamPos, Audio_Format_Version) + __T("\"");
        ToReturn += __T(">\n");
        ...
        //if (!MI.Get(Stream_Audio, StreamPos, Audio_Format_Profile).empty())
        {
            ...
            if (!TermID_String.empty())
                ToReturn += __T(" typeLink=\"http://www.ebu.ch/metadata/cs/ebu_AudioCompressionCodeCS.xml#") +
                            TermID_String + __T("\"");
            ToReturn += __T("/>\n");
        }
        ...
        if (!MI.Get(Stream_Audio, StreamPos, Audio_CodecID).empty() ||
            !MI.Get(Stream_Audio, StreamPos, Audio_Format_Commercial_IfAny).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:codec>\n");
            if (!MI.Get(Stream_Audio, StreamPos, Audio_CodecID).empty()) {
                ToReturn += __T("\t\t\t\t\t<ebucore:codecIdentifier>\n");
            ...
            if (!MI.Get(Stream_Audio, StreamPos, Audio_Format_Commercial_IfAny).empty())
                ToReturn += __T("\t\t\t\t\t<ebucore:name>") +
                            MI.Get(Stream_Audio, StreamPos, Audio_Format_Commercial_IfAny) + __T("</ebucore:name>\n");
            ToReturn += __T("\t\t\t\t</ebucore:codec>\n");
        }

        //audioTrackConfiguration
        if (As11_Core_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_Core_Pos, __T("AudioTrackLayout")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:audioTrackConfiguration typeLabel=\"") +
                        MI.Get(Stream_Other, As11_Core_Pos, __T("AudioTrackLayout")) + __T("\"/>\n");
        }

        //samplingRate
        if (!MI.Get(Stream_Audio, StreamPos, Audio_SamplingRate).empty())
            ToReturn += __T("\t\t\t\t<ebucore:samplingRate>") + MI.Get(Stream_Audio, StreamPos, Audio_SamplingRate) +
                        __T("</ebucore:samplingRate>\n");

        //sampleSize
        if (!MI.Get(Stream_Audio, StreamPos, Audio_BitDepth).empty())
            ToReturn += __T("\t\t\t\t<ebucore:sampleSize>") + MI.Get(Stream_Audio, StreamPos, Audio_BitDepth) +
                        __T("</ebucore:sampleSize>\n");

        //bitRate
        if (!MI.Get(Stream_Audio, StreamPos, Audio_BitRate).empty())
            ToReturn += __T("\t\t\t\t<ebucore:bitRate>") + MI.Get(Stream_Audio, StreamPos, Audio_BitRate) +
                        __T("</ebucore:bitRate>\n");

        //bitRateMax
        if (!MI.Get(Stream_Audio, StreamPos, Audio_BitRate_Maximum).empty())
            ToReturn += __T("\t\t\t\t<ebucore:bitRateMax>") + MI.Get(Stream_Audio, StreamPos, Audio_BitRate_Maximum) +
                        __T("</ebucore:bitRateMax>\n");
        ...
        if (!MI.Get(Stream_Audio, StreamPos, Audio_Channel_s_).empty())
            ToReturn += __T("\t\t\t\t<ebucore:channels>") + MI.Get(Stream_Audio, StreamPos, Audio_Channel_s_) +
                        __T("</ebucore:channels>\n");

        //format - technicalAttributeString - ChannelPositions
        if (!MI.Get(Stream_Audio, StreamPos, Audio_ChannelPositions).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"ChannelPositions\">") +
                        MI.Get(Stream_Audio, StreamPos, Audio_ChannelPositions) +
                        __T("</ebucore:technicalAttributeString>\n");
        }

        //format - technicalAttributeString - ChannelLayout
        if (!MI.Get(Stream_Audio, StreamPos, Audio_ChannelLayout).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"ChannelLayout\">") +
                        MI.Get(Stream_Audio, StreamPos, Audio_ChannelLayout) +
                        __T("</ebucore:technicalAttributeString>\n");
        }
        ...
        if (!MI.Get(Stream_Audio, StreamPos, Audio_Format_Settings_Endianness).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"Endianness\">") +
        ...
        if (!MI.Get(Stream_Audio, StreamPos, Audio_Format_Settings_Wrapping).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeString typeLabel=\"Wrapping\">") +
        ...
        if (!MI.Get(Stream_Audio, StreamPos, Audio_StreamSize).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:technicalAttributeInteger typeLabel=\"StreamSize\"") +
                        (Version >= Export_EbuCore::Version_1_6 ? Ztring(__T(" unit=\"byte\"")) : Ztring()) + __T(">") +
                        MI.Get(Stream_Audio, StreamPos, Audio_StreamSize) +
                        __T("</ebucore:technicalAttributeInteger>\n");
        }

        //technicalAttributeString
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("AudioComments")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:comment typeLabel=\"AudioComments\">") +
                        MI.Get(Stream_Other, As11_UkDpp_Pos, __T("AudioComments")) + __T("</ebucore:comment>\n");
        }
...
//---------------------------------------------------------------------------
    void EbuCore_Transform_AcquisitionMetadata(Ztring &ToReturn, MediaInfo_Internal &MI, size_t StreamPos,
                                               Export_EbuCore::version Version) {
        ToReturn += __T("\t\t\t<!-- (Proof of concept, for feedback only)\n");
        ...
        {
            bool NotFirst = false;
            for (size_t i = MediaInfoLib::Config.Info_Get(Stream_Other).size();
                 i < MI.Count_Get(Stream_Other, StreamPos); ++i) {
                Ztring Name = MI.Get(Stream_Other, StreamPos, i, Info_Name);
                if (Name.size() > 7 && Name.find(__T("_Values"), 7) == Name.size() - 7) {
                    ZtringList Values;
                    Values.Separator_Set(0, __T(" / "));
                            ...
                            if (((Method == 0 && Values.size() == 1) || (Method == 1 && Values.size() > 1))) {
                                ...
                                if (Values[Pos] != __T("Infinite")) {
                                    ...
                                    if (Name == __T("FocusDistance")
                                        ...
                                        Ztring CookeProtocol_CalibrationType = MI.Get(Stream_Other, StreamPos,
                                                                                      __T("CookeProtocol_CalibrationType_Values"));
...
//---------------------------------------------------------------------------
    Ztring Export_EbuCore::Transform(MediaInfo_Internal &MI, version Version) {
        ...
        if (DateTime.size() >= 4 && DateTime[0] == __T('U') && DateTime[1] == __T('T') && DateTime[2] == __T('C') &&
            DateTime[3] == __T(' ')) {
            DateTime.erase(0, 4);
            DateTime += __T('Z');
        }
        ...
        if (Version == Version_1_5)
            ToReturn +=
                    __T("<ebucore:ebuCoreMain xmlns:dc=\"http://purl.org/dc/elements/1.1/\" xmlns:ebucore=\"urn:ebu:metadata-schema:ebuCore_2014\"\n    xmlns:xalan=\"http://xml.apache.org/xalan\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n    xsi:schemaLocation=\"urn:ebu:metadata-schema:ebuCore_2014 http://www.ebu.ch/metadata/schemas/EBUCore/20140318/EBU_CORE_20140318.xsd\" version=\"1.5\" dateLastModified=\"") +
                    Date + __T("\" timeLastModified=\"") + Time + __T("\">\n");
        else
            ToReturn +=
                    __T("<ebucore:ebuCoreMain xmlns:dc=\"http://purl.org/dc/elements/1.1/\" xmlns:ebucore=\"urn:ebu:metadata-schema:ebuCore_2015\"\n    xmlns:xalan=\"http://xml.apache.org/xalan\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"\n    xsi:schemaLocation=\"urn:ebu:metadata-schema:ebuCore_2015 http") +
                    (MediaInfoLib::Config.Https_Get() ? Ztring(__T("s")) : Ztring()) +
                    __T("://www.ebu.ch/metadata/schemas/EBUCore/20150522/ebucore_20150522.xsd\" version=\"1.6\" dateLastModified=\"") +
                    Date + __T("\" timeLastModified=\"") + Time + __T("\">\n");
        ...
        if (As11_Core_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_Core_Pos, __T("ProgrammeTitle")).empty()) {
            ...
            ToReturn += __T("\t\t\t<dc:title>") + MI.Get(Stream_Other, As11_Core_Pos, __T("ProgrammeTitle")) +
                        __T("</dc:title>\n");
            ...
            ToReturn += __T("\t\t\t<dc:title>") + MI.Get(Stream_Other, As11_Core_Pos, __T("EpisodeTitleNumber")) +
                        __T("</dc:title>\n");
            ToReturn += __T("\t\t</ebucore:title>\n");
        }
        ...
        if (As11_Core_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_Core_Pos, __T("SeriesTitle")).empty()) {
            ...
            ToReturn += __T("\t\t\t<dc:title>") + MI.Get(Stream_Other, As11_Core_Pos, __T("SeriesTitle")) +
                        __T("</dc:title>\n");
            ToReturn += __T("\t\t</ebucore:alternativeTitle>\n");
        }
        if (As11_Core_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_Core_Pos, __T("EpisodeTitleNumber")).empty() &&
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("Synopsis")).empty()) {
            ...
            ToReturn += __T("\t\t\t<dc:description>") + MI.Get(Stream_Other, As11_UkDpp_Pos, __T("Synopsis")) +
                        __T("</dc:description>\n");
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ProductPlacement")).empty()) {
            ...
            ToReturn += __T("\t\t\t<dc:description>") +
                        Ztring(MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ProductPlacement")) == __T("Yes") ? __T("true")
                                                                                                           : __T(
                                       "false")) + __T("</dc:description>\n");
            ToReturn += __T("\t\t</ebucore:description>\n");
        }
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && (!MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ContactEmail")).empty() ||
            ...
            if (!MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ContactEmail")).empty())
                ToReturn += __T("\t\t\t\t\t<ebucore:emailAddress>") +
                            XML_Encode(MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ContactEmail"))) +
                            __T("</ebucore:emailAddress>\n");
            if (!MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ContactTelephoneNumber")).empty())
                ToReturn += __T("\t\t\t\t\t<ebucore:telephoneNumber>") +
                            XML_Encode(MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ContactTelephoneNumber"))) +
                            __T("</ebucore:telephoneNumber>\n");
            ToReturn += __T("\t\t\t\t</ebucore:details>\n");
            ToReturn += __T("\t\t\t</ebucore:contactDetails>\n");
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("Originator")).empty()) {
            ...
            ToReturn += __T("\t\t\t\t<ebucore:organisationName>") +
                        XML_Encode(MI.Get(Stream_Other, As11_UkDpp_Pos, __T("Originator"))) +
                        __T("</ebucore:organisationName>\n");
            ToReturn += __T("\t\t\t</ebucore:organisationDetails>\n");
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("Distributor")).empty()) {
            ...
            ToReturn += __T("\t\t\t<ebucore:organisationDetails>\n");
            ToReturn += __T("\t\t\t\t<ebucore:organisationName>") +
                        MI.Get(Stream_Other, As11_UkDpp_Pos, __T("Distributor")) + __T("</ebucore:organisationName>\n");
            ToReturn += __T("\t\t\t</ebucore:organisationDetails>\n");
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("CopyrightYear")).empty()) {
            ...
            ToReturn += __T("\t\t\t<ebucore:copyrighted startYear=\"") +
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("Genre")).empty()) {
            ...
            ToReturn +=
                    __T("\t\t\t<ebucore:genre typeDefinition=\"") + MI.Get(Stream_Other, As11_UkDpp_Pos, __T("Genre")) +
                    __T("\"/>\n");
            ToReturn += __T("\t\t</ebucore:type>\n");
        }
        ...
        if (!MI.Get(Stream_General, 0, General_Format).empty())
            ToReturn += (Version >= Version_1_6 ? __T(" containerFormatName=\"") : __T(" formatLabel=\"")) +
                        MI.Get(Stream_General, 0, General_Format) + __T("\"");
        ...
        if (!MI.Get(Stream_General, 0, General_CodecID).empty() ||
            (!MI.Get(Stream_General, 0, General_Format_Commercial_IfAny).empty())) {
            ToReturn += __T("\t\t\t\t<ebucore:codec>\n");
            if (!MI.Get(Stream_General, 0, General_CodecID).empty()) {
                ToReturn += __T("\t\t\t\t\t<ebucore:codecIdentifier>\n");
                ToReturn += __T("\t\t\t\t\t\t<dc:identifier>") + MI.Get(Stream_General, 0, General_CodecID) +
                            __T("</dc:identifier>\n");
                ToReturn += __T("\t\t\t\t\t</ebucore:codecIdentifier>\n");
            }
            if (!MI.Get(Stream_General, 0, General_Format_Commercial_IfAny).empty())
                ToReturn +=
                        __T("\t\t\t\t\t<ebucore:name>") + MI.Get(Stream_General, 0, General_Format_Commercial_IfAny) +
                        __T("</ebucore:name>\n");
            ToReturn += __T("\t\t\t\t</ebucore:codec>\n");
        }
        ...
        if (As11_Core_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_Core_Pos, __T("ShimName")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) +
                        __T(" typeLabel=\"AS11ShimName\">") + MI.Get(Stream_Other, As11_Core_Pos, __T("ShimName")) +
                        __T("</ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) + __T(">\n");
        }
        ...
        if (As11_Core_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_Core_Pos, __T("ShimVersion")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) +
                        __T(" typeLabel=\"AS11ShimVersion\">") +
                        MI.Get(Stream_Other, As11_Core_Pos, __T("ShimVersion")) + __T("</ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) + __T(">\n");
        }
        ...
        if (!MI.Get(Stream_General, 0, __T("Format_Profile")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) +
                        __T(" typeLabel=\"FormatProfile\">") + MI.Get(Stream_General, 0, __T("Format_Profile")) +
                        __T("</ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) + __T(">\n");
        }
        ...
        if (!MI.Get(Stream_General, 0, __T("Format_Profile")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) +
                        __T(" typeLabel=\"FormatSettings\">") + MI.Get(Stream_General, 0, __T("Format_Settings")) +
                        __T("</ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) + __T(">\n");
        }
        ...
        if (!MI.Get(Stream_General, 0, __T("Encoded_Application")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) +
                        __T(" typeLabel=\"WritingApplication\">") +
                        MI.Get(Stream_General, 0, __T("Encoded_Application")) + __T("</ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) + __T(">\n");
        }
        ...
        if (!MI.Get(Stream_General, 0, __T("Encoded_Library/String")).empty()) {
            ToReturn += __T("\t\t\t\t<ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) +
                        __T(" typeLabel=\"WritingLibrary\">") +
                        MI.Get(Stream_General, 0, __T("Encoded_Library/String")) + __T("</ebucore:") +
                        Ztring(Version >= Version_1_6 ? __T("technicalAttributeString") : __T("comment")) + __T(">\n");
        }
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("SigningPresent")).empty()) {
            ...
            ToReturn += __T("\t\t\t<ebucore:signingFormat signingPresenceFlag=\"") +
                        Ztring(MI.Get(Stream_Other, As11_UkDpp_Pos, __T("SigningPresent")) == __T("Yes") ? __T("true")
                                                                                                         : __T(
                                       "false")) + __T("\"/>\n");
            //ToReturn+=__T("\t\t\t</ebucore:dataFormat>\n");
            if (Version == Version_1_5)
        ...
        if (As11_Core_Pos != (size_t) -1 &&
            ...
            ToReturn += __T("\t\t\t<ebucore:dataFormat>\n");
            ToReturn += __T("\t\t\t\t<ebucore:captioningFormat captioningPresenceFlag=\"") +
                        Ztring(MI.Get(Stream_Other, As11_Core_Pos, __T("OpenCaptionsPresent")) == __T("Yes") ? __T(
                                "true") : __T("false")) + __T("\" closed=\"true\"/>\n");
            ToReturn += __T("\t\t\t</ebucore:dataFormat>\n");
        }
        ...
        if (As11_UkDpp_Pos != (size_t) -1 &&
            ...
            ToReturn += __T("\t\t\t<ebucore:dataFormat>\n");
            ToReturn += __T("\t\t\t\t<ebucore:captioningFormat captioningPresenceFlag=\"") +
                        Ztring(MI.Get(Stream_Other, As11_UkDpp_Pos, __T("OpenCaptionsPresent")) == __T("Yes") ? __T(
                                "true") : __T("false")) + __T("\" closed=\"false\"/>\n");
            ToReturn += __T("\t\t\t</ebucore:dataFormat>\n");
        }
        ...
        if (As11_UkDpp_Pos != (size_t) -1 &&
            ...
            ToReturn += __T("\t\t\t\t<ebucore:timecode>") +
                        MI.Get(Stream_Other, As11_UkDpp_Pos, __T("TotalProgrammeDuration")) +
                        __T("</ebucore:timecode>\n");
            ToReturn += __T("\t\t\t</ebucore:duration>\n");
            if (Version == Version_1_5 && durationDone)
                ToReturn += __T("\t\t\t-->\n");
        ...
        if (!MI.Get(Stream_General, 0, General_FileSize).empty())
            ToReturn += __T("\t\t\t<ebucore:fileSize>") + MI.Get(Stream_General, 0, General_FileSize) +
                        __T("</ebucore:fileSize>\n");
        ...
        if (!MI.Get(Stream_General, 0, General_CompleteName).empty()) {
            ToReturn += __T("\t\t\t<ebucore:locator>") + XML_Encode(MI.Get(Stream_General, 0, General_CompleteName)) +
                        __T("</ebucore:locator>\n");
        }

        //format - technicalAttributeString - AudioLoudnessStandard
        if (As11_UkDpp_Pos != (size_t) -1 &&
            !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("AudioLoudnessStandard")).empty()) {
            ToReturn += __T("\t\t\t<ebucore:technicalAttributeString typeLabel=\"AudioLoudnessStandard\">") +
        ...
        if (!MI.Get(Stream_Other, As11_UkDpp_Pos, __T("AudioDescriptionType")).empty()) {
            ToReturn += __T("\t\t\t<ebucore:technicalAttributeString typeLabel=\"AudioDescriptionType\">") +
        ...
        if (!MI.Get(Stream_General, 0, General_OverallBitRate).empty()) {
            ToReturn += __T("\t\t\t<ebucore:technicalAttributeInteger typeLabel=\"OverallBitRate\"") +
                        (Version >= Version_1_6 ? Ztring(__T(" unit=\"bps\"")) : Ztring()) + __T(">") +
                        MI.Get(Stream_General, 0, General_OverallBitRate) +
                        __T("</ebucore:technicalAttributeInteger>\n");
        }
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_Core_Pos, __T("ProgrammeHasText")).empty()) {
            ToReturn += __T("\t\t\t<ebucore:technicalAttributeBoolean typeLabel=\"ProgrammeHasText\">") +
        ...
        if (As11_UkDpp_Pos != (size_t) -1 &&
            !MI.Get(Stream_Other, As11_Core_Pos, __T("AudioDescriptionPresent")).empty()) {
            ToReturn += __T("\t\t\t<ebucore:technicalAttributeBoolean typeLabel=\"AudioDescriptionPresent\">") +
                        Ztring(MI.Get(Stream_Other, As11_UkDpp_Pos, __T("AudioDescriptionPresent")) == __T("Yes") ? __T(
                                "true") : __T("false")) + __T("</ebucore:technicalAttributeBoolean>\n");
        }
        ...
        if (!MI.Get(Stream_General, 0, General_Encoded_Date).empty()) {
            ...
            ToReturn += __T("\t\t\t<ebucore:dateCreated startDate=\"") + Date + __T("\" startTime=\"") + Time +
                        __T("\"/>\n");
        }
        ...
        if (!MI.Get(Stream_General, 0, General_Tagged_Date).empty()) {
            ...
            ToReturn += __T("\t\t\t<ebucore:dateModified startDate=\"") + Date + __T("\" startTime=\"") + Time +
                        __T("\"/>\n");
        }
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ProductionNumber")).empty()) {
            ...
            ToReturn += __T("\t\t\t<dc:identifier>") + MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ProductionNumber")) +
                        __T("</dc:identifier>\n");
        ...
        if (As11_UkDpp_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_UkDpp_Pos, __T("OtherIdentifier")).empty()) {
            ...
            ToReturn += __T("\t\t\t<dc:identifier>") + MI.Get(Stream_Other, As11_UkDpp_Pos, __T("OtherIdentifier")) +
                        __T("</dc:identifier>\n");
            ToReturn += __T("\t\t</ebucore:identifier>\n");
        }
        if (As11_UkDpp_Pos != (size_t) -1 &&
            ...
            ToReturn +=
                    __T("\t\t\t<dc:identifier>") + MI.Get(Stream_Other, As11_UkDpp_Pos, __T("OtherIdentifierType")) +
                    __T("</dc:identifier>\n");
            ToReturn += __T("\t\t</ebucore:identifier>\n");
        }
        ...
        if (As11_Core_Pos != (size_t) -1 && !MI.Get(Stream_Other, As11_Core_Pos, __T("PrimaryAudioLanguage")).empty()) {
            ...
            ToReturn += __T("\t\t\t<dc:language>") + MI.Get(Stream_Other, As11_Core_Pos, __T("PrimaryAudioLanguage")) +
                        __T("</dc:language>\n");
        ...
        if (As11_UkDpp_Pos != (size_t) -1 &&
            ...
            ToReturn +=
                    __T("\t\t\t<dc:language>") + MI.Get(Stream_Other, As11_UkDpp_Pos, __T("SecondaryAudioLanguage")) +
                    __T("</dc:language>\n");
            ToReturn += __T("\t\t</ebucore:language>\n");
        }
        ...
        if (As11_UkDpp_Pos != (size_t) -1 &&
            ...
            ToReturn +=
                    __T("\t\t\t<dc:language>") + MI.Get(Stream_Other, As11_UkDpp_Pos, __T("TertiaryAudioLanguage")) +
                    __T("</dc:language>\n");
            ToReturn += __T("\t\t</ebucore:language>\n");
        }
        ...
        if (As11_UkDpp_Pos != (size_t) -1 &&
            ...
            ToReturn +=
                    __T("\t\t\t<dc:language>") + MI.Get(Stream_Other, As11_UkDpp_Pos, __T("ProgrammeTextLanguage")) +
                    __T("</dc:language>\n");
            ToReturn += __T("\t\t</ebucore:language>\n");
        }
        ...
        if (As11_Segmentation_Pos != (size_t) -1) {
            ...
            for (;;) {
                ...
                ToReturn +=
                        __T("\t\t<ebucore:part partNumber=\"") + Ztring::ToZtring(Pos) + __T("\" partTotalNumber=\"") +
