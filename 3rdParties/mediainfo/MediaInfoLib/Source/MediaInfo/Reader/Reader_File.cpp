...
//---------------------------------------------------------------------------
    size_t Reader_File::Format_Test_PerParser(MediaInfo_Internal *MI, const String &File_Name) {
        ...
        if (!Config_Partial_Begin.empty() && Config_Partial_Begin[0] >= __T('0') &&
            Config_Partial_Begin[0] <= __T('9')) {
        ...
        //Parser
        MI->Open_Buffer_Init((Partial_End <= MI->Config.File_Size ? Partial_End : MI->Config.File_Size) - Partial_Begin,
                             File_Name);

        //Buffer
        MI->Option(__T("File_Buffer_Size_Hint_Pointer"),
                   Ztring::ToZtring((size_t) (&MI->Config.File_Buffer_Size_ToRead)));
            ...
            while (!(Status[File__Analyze::IsFinished] || (StopAfterFilled && Status[File__Analyze::IsFilled]))) {
                ...
                if (MI->Open_Buffer_Continue_GoTo_Get() != (int64u) -1) {
                    ...
                    if (!(GoTo > F.Position_Get() && GoTo < F.Position_Get() + Buffer_NoJump_Temp)) //No smal jumps
                    {
                        ...
                        MI->Open_Buffer_Init((int64u) -1,
                                             MI->Config.File_Current_Offset + F.Position_Get() - Partial_Begin);
                    }
                }
...
MI->Config.File_Buffer_Size_ToRead > MI->Config.File_Buffer_Size_Max) {
                    ...
                    if (MI->Config.File_Buffer_Size_Max >= 64 * 1024 * 1024)
                        MI->Config.File_Buffer_Size_Max =
                                64 * 1024 * 1024; //limitation of the buffer in order to avoid to big memory usage
...
F.Position_Get() >= F.Size_Get()) {
                    ...
                    if (MI->Config.File_Names_Pos && MI->Config.File_Names_Pos < MI->Config.File_Names.size()) {
                        MI->Config.File_Current_Offset +=
                                MI->Config.File_Names_Pos <= MI->Config.File_Sizes.size() ? MI->Config.File_Sizes[
                                        MI->Config.File_Names_Pos - 1] : F.Size_Get();
                        F.Close();
#if MEDIAINFO_EVENTS
...
#endif //MEDIAINFO_READTHREAD
                {
                    MI->Config.File_Buffer_Size = F.Read(MI->Config.File_Buffer,
                                                         (F.Position_Get() + MI->Config.File_Buffer_Size_ToRead <
                                                          (Partial_End <= MI->Config.File_Size ? Partial_End
                                                                                               : MI->Config.File_Size))
                                                         ? MI->Config.File_Buffer_Size_ToRead : ((size_t) (
                                                                 (Partial_End <= MI->Config.File_Size ? Partial_End
                                                                                                      : MI->Config.File_Size) -
                                                                 F.Position_Get())));
#if MEDIAINFO_READTHREAD
                ...
                */
                ...
                if (MI->Config.ParseSpeed >= 1.0 && !MI->Config.File_IsGrowing &&
                ...
                if (MI->Config.File_IsGrowing && (Growing_Temp != (int64u) -1 ||
                                                  MI->Config.File_Current_Offset + F.Position_Get() >=
                                                  MI->Config.File_Size)) {
                    for (size_t CountOfSeconds = 0;
                         CountOfSeconds < (size_t) MI->Config.File_GrowingFile_Delay_Get(); CountOfSeconds++) {
                        ...
                        if (LastFile_Size_New != LastFile_Size_Old || Files_Count_New != Files_Count_Old) {
                            if (MI->Config.File_Names.size() ==
                                1) //if more than 1 file, file size config is already done in TestContinuousFileNames()
                            {
                                MI->Config.File_Current_Size = MI->Config.File_Size = LastFile_Size_New;
                                MI->Open_Buffer_Init(MI->Config.File_Size,
                                                     MI->Config.File_Current_Offset + F.Position_Get() -
                                                     MI->Config.File_Buffer_Size);
                            }
                            break;
                        }
