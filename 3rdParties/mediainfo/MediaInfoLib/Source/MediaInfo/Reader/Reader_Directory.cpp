...
namespace MediaInfoLib {
    ...
    size_t Reader_Directory::Format_Test(MediaInfo_Internal *MI, String File_Name) {
        ...
        if (Dir::Exists(File_Name) &&
            File_Name.rfind(Ztring(1, PathSeparator) + __T("CONTENT")) + 8 == File_Name.size())
...
//---------------------------------------------------------------------------
    ...
    void Reader_Directory::Bdmv_Directory_Cleanup(ZtringList &List) {
        ...
        Ztring ToSearch =
                Ztring(1, PathSeparator) + __T("BDMV") + PathSeparator + __T("index.bdmv"); //"\BDMV\index.bdmv"
            ...
            if (BDMV_Pos != string::npos && BDMV_Pos != 0 && BDMV_Pos + 16 == List[File_Pos].size()) {
                ...
                if (List.Find(ToSearch) != string::npos) {
                    ...
                    for (size_t Pos = 0; Pos < List.size(); Pos++) {
                        if (List[Pos].find(ToSearch) == 0 &&
                            List[Pos] != ToSearch) //Remove all subdirs of ToSearch but not ToSearch
                        {
...
//---------------------------------------------------------------------------
    ...
    void Reader_Directory::P2_Directory_Cleanup(ZtringList &List) {
        ...
        Ztring ToSearch = Ztring(1, PathSeparator) + __T("CONTENTS") + PathSeparator + __T("CLIP") +
                          PathSeparator; //"/CONTENTS/CLIP/"
        for (size_t File_Pos = 0; File_Pos < List.size(); File_Pos++) {
            ...
            if (P2_Pos != string::npos && P2_Pos != 0 && P2_Pos + 1 + 8 + 1 + 4 + 1 + 10 == List[File_Pos].size()) {
                ...
                for (size_t Pos = 0; Pos < List.size(); Pos++) {
                    if (List[Pos].find(Path_Begin) == 0 &&
                        List[Pos].find(Path_Begin + __T("CONTENTS") + PathSeparator + __T("CLIP") + PathSeparator) ==
                        string::npos) //Remove all subdirs of Path_Begin but not with CLIP
                    {
...
//---------------------------------------------------------------------------
    ...
    void Reader_Directory::Xdcam_Directory_Cleanup(ZtringList &List) {
        ...
        for (size_t File_Pos = 0; File_Pos < List.size(); File_Pos++) {
            ...
            if (Xdcam_Pos != string::npos && Xdcam_Pos != 0 && (Xdcam_FileName.Extension_Get() == __T("XML")) &&
                Xdcam_Pos + 1 + 4 + 1 + Xdcam_FileName.Name_Get().size() + 4 == List[File_Pos].size()) {
                //This is a XDCAM CLIP
                Ztring Path_Begin = List[File_Pos];
                Path_Begin.resize(Path_Begin.size() - (1 + 4 + 1 + 12));
                Path_Begin += Ztring(1, PathSeparator);
                if (Dir::Exists(Path_Begin + __T("Edit")) && Dir::Exists(Path_Begin + __T("General")) &&
                    Dir::Exists(Path_Begin + __T("Sub"))) {
                    bool HasChanged = false;
                    for (size_t Pos = 0; Pos < List.size(); Pos++) {
                        if (List[Pos].find(Path_Begin) == 0 &&
                            (List[Pos].find(Path_Begin + __T("Clip") + PathSeparator) == string::npos ||
                             (List[Pos].find(__T(".XML")) !=
                              List[Pos].size() - 4))) //Remove all subdirs of Path_Begin but not with the right XML
                        {
