...
//***************************************************************************
    ...
    bool File__Duplicate_MpegTs::Is_Wanted(int16u ProgNum, int16u PID) const {
        ...
        if (Wanted_program_numbers.find(ProgNum) != Wanted_program_numbers.end())
            Wanted = true;
        else if (Wanted_program_map_PIDs.find(PID) != Wanted_program_map_PIDs.end())
            Wanted = true;
        return Wanted;
    }
    ...
    bool File__Duplicate_MpegTs::Manage_PMT(const int8u *ToAdd, size_t ToAdd_Size) {
        ...
        if (!Is_Wanted(StreamID, elementary_PIDs_program_map_PIDs[StreamID])) {
            delete[] PMT[StreamID].Buffer;
            PMT[StreamID].Buffer = NULL;
            return false;
        }

        //program_info_length
        const int16u program_info_length = CC2(FromTS.Buffer + FromTS.Offset + 2) & 0x0FFF;
        std::memcpy(PMT[StreamID].Buffer + PMT[StreamID].Offset, FromTS.Buffer + FromTS.Offset,
                    4 + program_info_length);
        ...
        while (FromTS.Offset + 5 <= FromTS.End) {
            ...
            if (Wanted_elementary_PIDs.empty() ||
                Wanted_elementary_PIDs.find(elementary_PID) != Wanted_elementary_PIDs.end()) {
                //Integrating it
...
//***************************************************************************
    ...
    bool
    File__Duplicate_MpegTs::Parsing_Begin(const int8u *ToAdd, size_t ToAdd_Size, std::map<int16u, buffer> &ToModify_) {
        //Managing big chunks
        ...
        for (int32u CRC_32_Offset = (int32u) FromTS.Begin;
             CRC_32_Offset < FromTS.End + 4; CRC_32_Offset++) //After syncword
        ...
        if (ToModify.Buffer != NULL && ToModify.Size < FromTS.Size) {
            delete[] ToModify.Buffer;
            ToModify.Buffer = NULL;
        }
        if (ToModify.Buffer == NULL)
