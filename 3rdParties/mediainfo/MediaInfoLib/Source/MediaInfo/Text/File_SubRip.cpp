...
//---------------------------------------------------------------------------
    bool File_SubRip::FileHeader_Begin() {
        ...
        bool IsLocal = false;
        Ztring Temp;
        Temp.From_UTF8((const char *) Buffer + (HasBOM ? 3 : 0),
                       (Buffer_Size > 65536 ? 65536 : Buffer_Size) - (HasBOM ? 3 : 0));
        if (Temp.empty()) {
            Temp.From_Local((const char *) Buffer + (HasBOM ? 3 : 0), (Buffer_Size > 65536 ? 65536 : Buffer_Size) -
                                                                      (HasBOM ? 3 : 0)); // Trying from local code page
            IsLocal = true;
        }
        ...
        if (!IsVTT) {
            ...
            for (size_t Pos = 0; Pos < List.size(); Pos++) {
                ...
                if (List(Pos, 1).size() > 22 && List(Pos, 1)[2] == __T(':') && List(Pos, 1)[5] == __T(':') &&
                    List(Pos, 1).find(__T(" --> ")) != string::npos)
                    IsOk++;
                else
                    IsNok++;
            }
