...
//---------------------------------------------------------------------------
    void File_Pdf::Read_Buffer_Continue() {
        switch (State) {
            case State_Parsing_xref         :
                xref();
                if (!Element_IsWaitingForMoreData()) trailer();
                break;
            case State_Parsing_startxref    :
                eof();
                startxref();
                break;
            case State_Parsing_object       :
                break; //Using elements
            default                         :
                Finish();
        }
    }
...
//---------------------------------------------------------------------------
    void File_Pdf::Header_Parse() {
        offsets::iterator Offsets_Next = upper_bound(Offsets.begin(), Offsets.end(),
                                                     (int32u) (File_Offset + Buffer_Offset));
...
//---------------------------------------------------------------------------
    void File_Pdf::xref() {
        ...
        while (Element_Offset < Element_Size && (Buffer[Buffer_Offset + (size_t) Element_Offset] == '\r' ||
        ...
        for (int32u Pos = 0; Pos < Count; ++Pos) {
            ...
            if (Pos > 100)
                ...
                Skip_String(18, "Entry");
                Param_Info1(Base + Pos);
            ...
            if (Get_Next(Key, Value)) {
                for (;;) {
                    ...
                    if (Key.empty())
                        ...
                        Param_Info1(__T("Document Catalog is at offset 0x" +
                                        Ztring().From_Number(Objects[ObjectNumber].Offset, 16)));
                    } else if (Key == "Info") {
                        int32u ObjectNumber = Value.To_int32u();
                        ...
                        Objects[(int32u) -1].Bottoms.push_back(ObjectNumber);
                        Param_Info1(
                                __T("Info is at offset 0x" + Ztring().From_Number(Objects[ObjectNumber].Offset, 16)));
                    } else if (Key == "Prev") {
                        Prev = Value.To_int32u();
                        Param_Info1(
                                __T("Previous Cross-Reference Table is at offset 0x" + Ztring().From_Number(Prev, 16)));
                    }
                }
            ...
            if (Get_Next(Key, Value)) {
                for (;;) {
                    ...
                    if (Key.empty())
                        ...
                        Param_Info1(
                                __T("Metadata is at offset 0x" + Ztring().From_Number(Objects[ObjectNumber].Offset)));
                    }
                }
...
//---------------------------------------------------------------------------
    int64u File_Pdf::SizeOfLine() {
        ...
        while (End < Buffer_Size && Buffer[End] != '\r' && Buffer[End] != '\n' &&
               !(End + 1 < Buffer_Size && Buffer[End] == '<' && Buffer[End + 1] == '<') &&
               !(End + 1 < Buffer_Size && Buffer[End] == '>' && Buffer[End + 1] == '>'))
            End++;
    ...
    bool File_Pdf::Get_Next(string &Key, Ztring &Value) {
        ...
        if (Line_Begin < Line.size() && Line[Line_Begin] == '/') {
            ...
            for (;;) {
                ...
                if (!HasParenthesis && !HasBracket && HasValue && Line[Line_End] == '<' && Line_End + 1 < Line.size() &&
                    Line[Line_End + 1] == '<')
                    break;
