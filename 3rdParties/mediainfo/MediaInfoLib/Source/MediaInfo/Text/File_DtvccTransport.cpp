    ...
    void File_DtvccTransport::Streams_Update_PerStream(size_t Pos) {
        ...
        if (Streams[Pos]->Parser) {
            for (size_t Pos2 = 0; Pos2 < Streams[Pos]->Parser->Count_Get(Stream_Text); Pos2++) {
                ...
                Fill(Stream_Text, StreamPos_Last, "MuxingMode",
                     Format == Format_DVD ? __T("DVD-Video") : __T("DTVCC Transport"));
        ...
        if (process_cc_data_flag) {
            for (int8u Pos = 0; Pos < cc_count; Pos++) {
                ...
                if (Format == Format_DVD) {
                    ...
                    Param_Info1(DtvccTransport_cc_type(cc_type));
                    cc_valid = true;
                } else {
                    //Normal DTVCC Transport
                        ...
                        if (Parser_Pos == 2) {
                            ...
                            if (cc_type == 3) {
                                ...
                                Open_Buffer_Continue(Streams[Parser_Pos]->Parser,
                                                     Buffer + (size_t) (Buffer_Offset + Element_Offset), 0);
