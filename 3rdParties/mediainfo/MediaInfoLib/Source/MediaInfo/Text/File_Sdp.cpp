        ...
        for (streams::iterator Stream = Streams.begin(); Stream != Streams.end(); ++Stream) {
            if (Stream->second.Parser) {
                ...
                for (size_t Pos = 0; Pos < Count; Pos++)
                    Fill(Stream_Text, Current + Pos, Text_ID, Stream->second.Parser->Get(Stream_Text, Pos, Text_ID),
                         true);
            }
        }
    }
...
//---------------------------------------------------------------------------
    void File_Sdp::Header_Parse() {
        ...
        for (int8u Pos = 0; Pos < 5; Pos++) {
            ...
            BS_End();
            FieldLines[Pos] =
                    ((Field ? 1 : 0) << 7) | (Reserved << 5) | Line; //Removing field information ((Field?1:0)<<7) |
            if (FieldLines[Pos]) {
