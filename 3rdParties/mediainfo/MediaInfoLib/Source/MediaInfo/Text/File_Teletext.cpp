...
//---------------------------------------------------------------------------
    void File_Teletext::Streams_Finish() {
        ...
        if (Teletexts && !Teletexts->empty())
            for (std::map<int16u, teletext>::iterator Teletext = Teletexts->begin();
                 Teletext != Teletexts->end(); ++Teletext) {
    ...
    void File_Teletext::Data_Parse() {
        if (PageNumber == 0xFF) {
            ...
            for (; PosX < 40; ++PosX) {
                ...
                if (byte != Stream.CC_Displayed_Values[Y][PosX] &&
                    (!C[7] || Y)) // C[7] is "Suppress Header", to be tested when Y==0
                {
                    Char Uni;
