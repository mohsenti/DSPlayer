...
//***************************************************************************
    ...
    void File_OtherText::Read_Buffer_Continue() {
        ...
        if (File.empty())
            File.From_Local((const char *) Buffer,
                            Buffer_Size > 65536 ? 65536 : Buffer_Size); // Trying from local code page
        if (File.size() < 0x100) {
        ...
        if (Lines[0] == __T("[Script Info]")
            ...
            Codec = __T("Adobe");
                   ...
                   && Lines[0][0] == __T('-') && Lines[0][1] == __T('-') && Lines[0][2] == __T('>') &&
                   Lines[0][3] == __T('>') && Lines[0][4] == __T(' ')
                   && Lines[0][5] == __T('0')
                   && Lines[1].empty() != true
                ) {
            Format = __T("AQTitle");
            ...
            Format = __T("Cheeta");
                   ...
                   && Lines[0][0] == __T('~') && Lines[0][1] == __T('C') && Lines[0][2] == __T('P') &&
                   Lines[0][3] == __T('C') && Lines[0][9] == __T('~')
