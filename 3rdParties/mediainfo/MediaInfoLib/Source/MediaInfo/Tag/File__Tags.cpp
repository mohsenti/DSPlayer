...
//---------------------------------------------------------------------------
    void File__Tags_Helper::Streams_Finish() {
        if (!Base->IsSub) {
            Base->Fill(Stream_General, 0, General_StreamSize,
                       TagsSize + Base->Retrieve(Stream_General, 0, General_StreamSize).To_int64u(), 10, true);
        ...
        for (;;) {
            ...
            if (!Parser) {
                ...
                } else if (ID8 == 0x4C59524943534245LL) //"LYRICSBE"
                {
                    ...
                    std::string Buf((const char *) (Base->Buffer + Base->Buffer_Offset),
                                    Base->Buffer_Size - Base->Buffer_Offset);
            ...
            if (Parser) {
                ...
                int64u Size_ToParse = (Parser_Buffer_Size < Base->Buffer_Size - Base->Buffer_Offset)
                                      ? Parser_Buffer_Size : (Base->Buffer_Size - Base->Buffer_Offset);
        ...
        if (TagSizeIsFinal) {
            if (JumpTo_WantedByParser != (int64u) -1) {
                if (JumpTo_WantedByParser < Id3v1_Size + Lyrics3_Size + Lyrics3v2_Size + ApeTag_Size)
                    ...
                    Base->GoTo(JumpTo_WantedByParser - Id3v1_Size - Lyrics3_Size - Lyrics3v2_Size - ApeTag_Size,
                               ParserName);
            }
            SearchingForEndTags = false;
        } else
...
//---------------------------------------------------------------------------
    bool File__Tags_Helper::DetectBeginOfEndTags_Test() {
        ...
        //Id3v1
        if (Id3v1_Size == 0 && File_EndTagSize == 0 &&
            Base->File_Size >= 128) //Only one, at the end, larger than 128 bytes
        {
            if (Base->File_Offset > Base->File_Size - 128 - 32) //Must be at least at the end less 128 bytes
            {
                Base->GoTo(Base->File_Size - 128 - 32,
                           "Tags detection"); //32 to be able to quickly see another tag system
                TagSizeIsFinal = false;
                return false;
            }

            if (Base->File_Offset + Base->Buffer_Size <
                Base->File_Size) //Must be at least at the end less 128 bytes of tags
            {
        ...
        //Lyrics3
        if (Lyrics3_Size == 0 &&
            Base->File_Size >= File_EndTagSize + 9) //Only one, larger than File_EndTagSize+15 bytes
        {
            ...
            if (Base->File_Offset + Base->Buffer_Size < Base->File_Size -
                                                        File_EndTagSize) //Must be at the end less File_EndTagSize+9 bytes plus 9 bytes of tags
            {
            ...
            if (CC8(Base->Buffer + ((size_t) ((Base->File_Size - File_EndTagSize - 9) - Base->File_Offset))) ==
                CC8("LYRICSEN")) {
                ...
                if (Base->File_Offset + Base->Buffer_Size <
                    Base->File_Size - File_EndTagSize) //Must be at the end less File_EndTagSize
                {
                    if (Base->File_Offset != Base->File_Size - File_EndTagSize)
                        Base->GoTo(Base->File_Size - File_EndTagSize, "Tags");
                    TagSizeIsFinal = false;
        ...
        //Lyrics3v2
        if (Lyrics3v2_Size == 0 &&
            Base->File_Size >= File_EndTagSize + 15) //Only one, larger than File_EndTagSize+15 bytes
        {
            ...
            if (CC8(Base->Buffer + ((size_t) ((Base->File_Size - File_EndTagSize - 15 + 6) - Base->File_Offset))) ==
                CC8("LYRICS20")) {
                Lyrics3v2_Size = Ztring((const char *) (Base->Buffer +
                                                        ((size_t) ((Base->File_Size - File_EndTagSize - 15) -
                                                                   Base->File_Offset))), 6).To_int32u() + 15;
        ...
        //ApeTag
        if (ApeTag_Size == 0 &&
            Base->File_Size >= File_EndTagSize + 32) //Only one, larger than File_EndTagSize+32 bytes
        {
            ...
            if (Base->File_Offset + Base->Buffer_Size < Base->File_Size -
                                                        File_EndTagSize) //Must be at the end less File_EndTagSize+15 bytes plus 15 bytes of tags
            {
            ...
            if (CC8(Base->Buffer + ((size_t) ((Base->File_Size - File_EndTagSize - 32) - Base->File_Offset))) ==
                CC8("APETAGEX")) {
                ApeTag_Size = LittleEndian2int32u(
                        Base->Buffer + ((size_t) ((Base->File_Size - File_EndTagSize - 32 + 12) - Base->File_Offset)));
                if (LittleEndian2int32u(
                        Base->Buffer + ((size_t) ((Base->File_Size - File_EndTagSize - 32 + 8) - Base->File_Offset))) ==
                    2000)
                    ApeTag_Size += 32;
