...
//---------------------------------------------------------------------------
    Ztring MediaInfo_Internal::Inform() {
...
#endif //defined(MEDIAINFO_MPEG7_YES)
#if defined(MEDIAINFO_PBCORE_YES)
        if (MediaInfoLib::Config.Inform_Get() == __T("PBCore") ||
            MediaInfoLib::Config.Inform_Get() == __T("PBCore_1.2"))
            return Export_PBCore().Transform(*this);
        if (MediaInfoLib::Config.Inform_Get() == __T("PBCore2") ||
            MediaInfoLib::Config.Inform_Get() == __T("PBCore_2.0"))
        ...
        if (XML_0_7_78_MA || XML_0_7_78_MI) {
            size_t Modified;
            Retour += __T("<media ref=\"") +
                      MediaInfo_Internal::Xml_Content_Escape(Get(Stream_General, 0, General_CompleteName),
                                                             Modified) += __T("\"");
        ...
        if (XML_0_7_78_MA)
            Retour +=
                    __T("<MediaInfo xmlns=\"http") + (MediaInfoLib::Config.Https_Get() ? Ztring(__T("s")) : Ztring()) +
                    __T("://mediaarea.net/mediainfo\" version=\"2.0beta1\">\n");
        if (XML)
            Retour += __T("<File>\n");
#endif //defined(MEDIAINFO_XML_YES)
        ...
        if (XML_0_7_78_MA) {
            if (MediaInfoLib::Config.Trace_Level_Get() || MediaInfoLib::Config.Inform_Get() == __T("Details")) {
                Retour += __T("<MediaTrace xmlns=\"http") +
                          (MediaInfoLib::Config.Https_Get() ? Ztring(__T("s")) : Ztring()) +
                          __T("://mediaarea.net/mediatrace\" version=\"0.1\">\n");
                if (!Details.empty())
...
//---------------------------------------------------------------------------
    ...
    Ztring MediaInfo_Internal::Inform(stream_t StreamKind, size_t StreamPos, bool IsDirect) {
        ...
        if (MediaInfoLib::Config.Inform_Get(__T("General")).empty()
            ...
            bool XML_0_7_78 = (MediaInfoLib::Config.Inform_Get() == __T("MAXML") ||
                               MediaInfoLib::Config.Inform_Get() == __T("MIXML")) ? true : false;
            if (XML_0_7_78)
                XML = true;
#endif //defined(MEDIAINFO_XML_YES)
#if defined(MEDIAINFO_CSV_YES)
            ...
            for (size_t Champ_Pos = 0; Champ_Pos < Size; Champ_Pos++) {
                ...
                Ztring A = Get((stream_t) StreamKind, StreamPos, Champ_Pos,
                               Info_Measure_Text); // TODO Bug sinon? voir Ztring
...
#endif //defined(MEDIAINFO_XML_YES
                if ((MediaInfoLib::Config.Complete_Get() ||
                     Get((stream_t) StreamKind, StreamPos, Champ_Pos, Info_Options)[InfoOption_ShowInInform] ==
                     __T('Y')))
                    Shouldshow = true;
                if (Shouldshow && !Get((stream_t) StreamKind, StreamPos, Champ_Pos, Info_Text).empty()) {
...
#if defined(MEDIAINFO_XML_YES)
                    if (XML_0_7_78 &&
                        MediaInfoLib::Config.Info_Get(StreamKind).Read(Champ_Pos, Info_Measure) == __T(" ms")) {
                    ...
                    if (XML) {
                        ...
                        if (XML_0_7_78 && Nom.size() > 8 && Nom.rfind(__T("_Version")) == Nom.size() - 8 &&
                            Valeur.size() > 8 && Valeur.rfind(__T("Version "), 0) == 0) {
        ...
        while (Retour.find(__T('$'), PosX) != (size_t) -1) {
            ...
            if (Retour.size() > PosX + 2 && !(Retour(PosX + 1) == __T('i') && Retour(PosX + 2) == __T('f') &&
                                              Retour(PosX + 3) ==
                                              __T('('))) //To keep out "%" without any signification, or "$if(..."
            {
                Ztring ARemplacer = Ztring(__T("$") + Retour.SubString(__T("$"), __T("$"), PosX)) + __T("$");
                Ztring RemplacerPar = MediaInfoLib::Config.Language_Get(
                        Retour.SubString(__T("$"), __T("$"), PosX)); //TODO : case sensitive
        ...
        while (!Retour.SubString(__T("["), __T("]")).empty()) {
            ...
            if (ValueToFind_Pos != Error) {
                ...
                if (RemplacerPar.empty())
                    ...
                    Ztring Crochets_RemplacerPar = Info(
                            MediaInfoLib::Config.Info_Get(StreamKind).Find(ATraiter.SubString(__T("%"), __T("%"))));
        ...
        while (Retour.find(__T('%'), PosX) != (size_t) -1) {
            PosX = Retour.find(__T('%'), PosX);
            if (Retour.size() > PosX + 2 && Retour[PosX + 1] >= __T('A') &&
                Retour[PosX + 1] <= __T('Z')) //To keep out "%" without any signification
            {
                Ztring ARemplacer = Ztring(__T("%") + Retour.SubString(__T("%"), __T("%"), PosX)) + __T("%");
