...
//***************************************************************************
    ...
    Ztring MediaInfo_Config_MediaInfo::Option(const String &Option, const String &Value) {
        ...
        if (Option_Lower == __T("file_audio_mergemonostreams")) {
            ...
            return __T("Disabled due to compilation options");
#endif //MEDIAINFO_ADVANCED
        } else if (Option_Lower == __T("file_sequencefilesskipframes") ||
                   Option_Lower == __T("file_sequencefileskipframes")) {
#if MEDIAINFO_ADVANCED
            ...
            if (Value.find(__T(':')) != string::npos) {
                ...
                if (!ValueZ.empty())
                    ValueInt64u +=
                            Ztring(ValueZ).To_int64u() * 1000 * 1000 * 1000 / (int64u) pow(10.0, (int) ValueZ.size());
            } else
            ...
            return __T("Encryption manager is disabled due to compilation options");
#endif //MEDIAINFO_AES
        } else if (Option_Lower == __T("file_encryption_mode") ||
                   Option_Lower == __T("file_encryption_modeofoperation")) {
#if MEDIAINFO_AES
...
//---------------------------------------------------------------------------
    ...
    string MediaInfo_Config_MediaInfo::Encryption_Method_GetS() {
        ...
        switch (Encryption_Method) {
            case Encryption_Method_Segment:
                return "Segment";
            default:
                return string();
        }
...
//---------------------------------------------------------------------------
    ...
    void MediaInfo_Config_MediaInfo::Event_Send(File__Analyze *Source, const int8u *Data_Content, size_t Data_Size,
                ...
                if (Temp->DTS != (int64u) -1) {
                    if (File_IgnoreEditsBefore && File_EditRate) {
                        int64u TimeOffset = float64_int64s(
                                ((float64) File_IgnoreEditsBefore) / File_EditRate * 1000000000);
        ...
        if (!FileIsReferenced && !Events_TimestampShift_Disabled) {
            ...
            if (Event->StreamIDs_Size >= 2 && Event->ParserIDs[0] == MediaInfo_Parser_MpegTs &&
                Event->ParserIDs[1] == MediaInfo_Parser_MpegPs) {
                ...
                if (!Events_TimestampShift_Delayed.empty() && Events_TimestampShift_Reference_ID != (int64u) -1 &&
                    Event->PTS != (int64u) -1) {
                    ...
                    for (size_t Pos = 0; Pos < Events_TimestampShift_Delayed.size(); Pos++)
                        if (Events_TimestampShift_Delayed[Pos]) {
                            Event_Send(NULL, Events_TimestampShift_Delayed[Pos]->Data_Content,
                                       Events_TimestampShift_Delayed[Pos]->Data_Size,
                                       Events_TimestampShift_Delayed[Pos]->File_Name);
                ...
                if ((Event->EventCode & 0x00FFFF00) ==
                    (MediaInfo_Event_Global_SimpleText << 8)) //If it is MediaInfo_Event_Global_SimpleText
                {
                    //Store the event if there is no reference stream
                    if (Events_TimestampShift_Reference_ID == (int64u) -1 ||
                        Events_TimestampShift_Reference_PTS == (int64u) -1) {
                        ...
                        if (((*EventCode) & 0x00FFFFFF) == (MediaInfo_Event_Global_SimpleText << 8) &&
                            Data_Size == sizeof(MediaInfo_Event_Global_SimpleText_0)) {
                            ...
                            if (New->Row_Values) {
                                ...
                                for (size_t Row_Pos = 0; Row_Pos < New->Row_Max; Row_Pos++) {
                                    ...
                                    std::memcpy(Row_Values[Row_Pos], Old->Row_Values[Row_Pos],
                                                New->Column_Max * sizeof(wchar_t));
                                }
                            ...
                            if (New->Row_Attributes) {
                                ...
                                for (size_t Row_Pos = 0; Row_Pos < New->Row_Max; Row_Pos++) {
                                    ...
                                    std::memcpy(Row_Attributes[Row_Pos], Old->Row_Attributes[Row_Pos],
                                                New->Column_Max * sizeof(int8u));
                                }
                    ...
                    if (Event->PTS >
                        Events_TimestampShift_Reference_PTS + 60000000000LL) // difference more than 60 seconds
                    {
                        int64u Shift = Event->PTS - Events_TimestampShift_Reference_PTS;
                        if (Shift >= 55555555555555LL - 10000000000LL &&
                            Shift <= 55555555555555LL + 10000000000LL) //+/- 10 second
        ...
        if (Source) {
            ...
            // Copying buffers
            int32u *EventCode = (int32u *) Data_Content;
            if (((*EventCode) & 0x00FFFFFF) == ((MediaInfo_Event_Global_Demux << 8) | 4) &&
            ...
            if ((Event_Generic->EventCode & 0x00FFFFFF) ==
                ((MediaInfo_Event_Global_Demux << 8) | 0x04)) //Demux version 4
            {
    ...
    void MediaInfo_Config_MediaInfo::Event_Accepted(File__Analyze *Source) {
        ...
        for (events_delayed::iterator Event = Events_Delayed.begin(); Event != Events_Delayed.end(); ++Event)
            if (Event->first == Source) {
                for (size_t Pos = 0; Pos < Event->second.size(); Pos++)
                    if (Event->second[Pos]) {
                        Event_Send(NULL, Event->second[Pos]->Data_Content, Event->second[Pos]->Data_Size,
                                   Event->second[Pos]->File_Name);
                        ...
                        if (IsDemux) {
                            ...
                            Old->Content = NULL;
                            if (Old->Offsets_Size) {
                                delete[] Old->Offsets_Content;
                                Old->Offsets_Content = NULL;
                            }
                            if (Old->Offsets_Size) {
                                delete[] Old->OriginalContent;
                                Old->OriginalContent = NULL;
                            }
                        }

                        delete Event->second[Pos];
                        Event->second[Pos] = NULL;
...
//---------------------------------------------------------------------------
    ...
    void MediaInfo_Config_MediaInfo::File_Curl_Set(const Ztring &NewValue) {
        ...
        if (Pos != string::npos) {
            Ztring Field = NewValue.substr(0, Pos);
            Field.MakeLowerCase();
