...
//***************************************************************************
    ...
    Ztring MediaInfo_Config::Option(const String &Option, const String &Value_Raw) {
        ...
        transform(Option_Lower.begin(), Option_Lower.begin() + Egal_Pos, Option_Lower.begin(),
                  (int (*)(int)) tolower); //(int(*)(int)) is a patch for unix
        ...
        if (Option_Lower.empty()) {
            ...
            transform(Value_Lower.begin(), Value_Lower.end(), Value_Lower.begin(),
                      (int (*)(int)) tolower); //(int(*)(int)) is a patch for unix
            ...
            switch (Demux_Get()) {
                ...
                case 1 :
                    return __T("Frame");
                case 2 :
                    return __T("Container");
                case 4 :
                    return __T("Elementary");
                default:
                    return Ztring();
            }
        ...
        } else if (Option_Lower == __T("detailsformat_get")) //Legacy for trace_format
        {
            ...
            transform(NewValue_Lower.begin(), NewValue_Lower.end(), NewValue_Lower.begin(),
                      (int (*)(int)) tolower); //(int(*)(int)) is a patch for unix
            ...
            return FormatDetection_MaximumOffset_Get() == (int64u) -1 ? Ztring(__T("-1")) : Ztring::ToZtring(
            ...
            return MpegTs_MaximumOffset_Get() == (int64u) -1 ? Ztring(__T("-1")) : Ztring::ToZtring(
            ...
            return MpegTs_MaximumScanDuration_Get() == (int64u) -1 ? Ztring(__T("-1")) : Ztring::ToZtring(
                    MpegTs_MaximumOffset_Get());
...
//---------------------------------------------------------------------------
    void MediaInfo_Config::Trace_Modificator_Set(const ZtringList &NewValue) {
        ...
        if (List.size() != 2)
            return;
        transform(List[0].begin(), List[0].end(), List[0].begin(),
                  (int (*)(int)) tolower); //(int(*)(int)) is a patch for unix
...
//---------------------------------------------------------------------------
    ...
    void MediaInfo_Config::Language_Set(stream_t StreamKind) {
        ...
        for (size_t Pos = 0; Pos < Info[StreamKind].size(); Pos++) {
            ...
            if (!Language_Raw &&
                ToReplace.find(__T('/')) != Error) //Complex values, like XXX/YYY --> We translate both XXX and YYY
            {
            ...
            Info[StreamKind](Pos,
                             Info_Measure_Text).clear(); //I don(t know why, but if I don't do this Delphi/C# debugger make crashing the calling program
...
//---------------------------------------------------------------------------
    const Ztring &MediaInfo_Config::CodecID_Get(stream_t KindOfStream, infocodecid_format_t Format, const Ztring &Value,
                                                infocodecid_t KindOfCodecIDInfo) {
        ...
        if (CodecID[Format][KindOfStream].empty()) {
            switch (KindOfStream) {
                case Stream_General :
                    switch (Format) {
                        case InfoCodecID_Format_Mpeg4 :
                            MediaInfo_Config_CodecID_General_Mpeg4(CodecID[Format][KindOfStream]);
                            break;
                        default:;
                    }
                    break;
                case Stream_Video   :
                    switch (Format) {
                        case InfoCodecID_Format_Matroska :
                            MediaInfo_Config_CodecID_Video_Matroska(CodecID[Format][KindOfStream]);
                            break;
                        case InfoCodecID_Format_Mpeg4    :
                            MediaInfo_Config_CodecID_Video_Mpeg4(CodecID[Format][KindOfStream]);
                            break;
                        case InfoCodecID_Format_Real     :
                            MediaInfo_Config_CodecID_Video_Real(CodecID[Format][KindOfStream]);
                            break;
                        case InfoCodecID_Format_Riff     :
                            MediaInfo_Config_CodecID_Video_Riff(CodecID[Format][KindOfStream]);
                            break;
                        default:;
                    }
                    break;
                case Stream_Audio   :
                    switch (Format) {
                        case InfoCodecID_Format_Matroska :
                            MediaInfo_Config_CodecID_Audio_Matroska(CodecID[Format][KindOfStream]);
                            break;
                        case InfoCodecID_Format_Mpeg4    :
                            MediaInfo_Config_CodecID_Audio_Mpeg4(CodecID[Format][KindOfStream]);
                            break;
                        case InfoCodecID_Format_Real     :
                            MediaInfo_Config_CodecID_Audio_Real(CodecID[Format][KindOfStream]);
                            break;
                        case InfoCodecID_Format_Riff     :
                            MediaInfo_Config_CodecID_Audio_Riff(CodecID[Format][KindOfStream]);
                            break;
                        default:;
                    }
                    break;
                case Stream_Text    :
                    switch (Format) {
                        case InfoCodecID_Format_Matroska :
                            MediaInfo_Config_CodecID_Text_Matroska(CodecID[Format][KindOfStream]);
                            break;
                        case InfoCodecID_Format_Mpeg4    :
                            MediaInfo_Config_CodecID_Text_Mpeg4(CodecID[Format][KindOfStream]);
                            break;
                        case InfoCodecID_Format_Riff     :
                            MediaInfo_Config_CodecID_Text_Riff(CodecID[Format][KindOfStream]);
                            break;
                        default:;
                    }
                    break;
                case Stream_Other   :
                    switch (Format) {
                        case InfoCodecID_Format_Mpeg4    :
                            MediaInfo_Config_CodecID_Other_Mpeg4(CodecID[Format][KindOfStream]);
                            break;
                        default:;
                    }
                    break;
...
//---------------------------------------------------------------------------
    const Ztring &
    MediaInfo_Config::Library_Get(infolibrary_format_t Format, const Ztring &Value, infolibrary_t KindOfLibraryInfo) {
        ...
        if (Library[Format].empty()) {
            switch (Format) {
                case InfoLibrary_Format_DivX :
                    MediaInfo_Config_Library_DivX(Library[Format]);
                    break;
                case InfoLibrary_Format_XviD :
                    MediaInfo_Config_Library_XviD(Library[Format]);
                    break;
                case InfoLibrary_Format_MainConcept_Avc :
                    MediaInfo_Config_Library_MainConcept_Avc(Library[Format]);
                    break;
                case InfoLibrary_Format_VorbisCom :
                    MediaInfo_Config_Library_VorbisCom(Library[Format]);
                    break;
                default:;
            }
        }
...
//---------------------------------------------------------------------------
    Ztring MediaInfo_Config::Info_Parameters_Get(bool Complete) {
        ...
        Language_Set(
                Ztring()); //TODO: it is reseted to English, it should actually not modify the language config (MediaInfo_Config_xxx() modifies the language config)
        ...
        if (List.empty()) {
            ...
            for (size_t Pos = 0; Pos < List.size(); Pos++) {
                if (List[Pos].find(__T("CallBack=memory://")) == 0)
                    Event_CallBackFunction = (MediaInfo_Event_CallBackFunction *) Ztring(
                            List[Pos].substr(18, std::string::npos)).To_int64u();
...
//---------------------------------------------------------------------------
    ...
    Ztring MediaInfo_Config::Event_CallBackFunction_Get() {
        ...
        return __T("CallBack=memory://") + Ztring::ToZtring((size_t) Event_CallBackFunction) +
               __T(";UserHandler=memory://") + Ztring::ToZtring((size_t) Event_UserHandler);
    }
    ...
    void MediaInfo_Config::Event_Send(const int8u *Data_Content, size_t Data_Size) {
        ...
        if (Event_CallBackFunction) {
            MEDIAINFO_DEBUG1("Event",
                             Debug += ", EventID=";
                                     Debug += Ztring::ToZtring(LittleEndian2int32u(Data_Content), 16).To_UTF8();)
