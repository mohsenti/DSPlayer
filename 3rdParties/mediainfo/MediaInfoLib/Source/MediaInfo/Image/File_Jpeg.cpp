    ...
    string Jpeg2000_Rsiz(int16u Rsiz) {
        switch (Rsiz) {
            ...
            case 0x0004:
                return "D-Cinema 4k";
            case 0x0005:
                return "D-Cinema 2k Scalable";
            case 0x0006:
                return "D-Cinema 4k Scalable";
            case 0x0007:
                return "Long-term storage";
            case 0x0101:
                return "BCS@L1"; //Broadcast Contribution Single Tile
            case 0x0102:
                return "BCS@L2"; //Broadcast Contribution Single Tile
            case 0x0103:
                return "BCS@L3"; //Broadcast Contribution Single Tile
            case 0x0104:
                return "BCS@L4"; //Broadcast Contribution Single Tile
            case 0x0105:
                return "BCS@L5"; //Broadcast Contribution Single Tile
            case 0x0205:
                return "BCM@L5"; //Broadcast Contribution Multi-tile
            case 0x0306:
                return "BCMR@L6"; //Broadcast Contribution Multi-tile Reversible
            case 0x0307:
                return "BCMR@L7"; //Broadcast Contribution Multi-tile Reversible
            default:
    ...
    void File_Jpeg::Read_Buffer_Continue() {
        if (Config->ParseSpeed >= 1.0 && IsSub && Status[IsFilled]) {
            ...
            if (Buffer_TotalBytes < Demux_TotalBytes) {
                Skip_XX(Demux_TotalBytes - Buffer_TotalBytes,
                        "Data"); //We currently don't want to parse data during demux
    ...
    void File_Jpeg::Header_Parse() {
        ...
        switch (code) {
            ...
            case Elements::EOI  :
                ...
                break;
            default   :
                Get_B2(size, "Fl - Frame header length");
        }
...
//---------------------------------------------------------------------------
    void File_Jpeg::SIZ() {
        ...
        for (int16u Pos = 0; Pos < Count; Pos++) {
            ...
            BS_Begin();
            Skip_SB("Signed");
            Get_S1(7, BitDepth, "BitDepth");
            Param_Info1(1 + BitDepth);
            Element_Info1(1 + BitDepth);
            BS_End();
            Get_B1(compSubsX, "compSubsX");
            Element_Info1(compSubsX);
            Get_B1(compSubsY, "compSubsY");
            Element_Info1(compSubsY);
            Element_End0();
        ...
        FILLING_BEGIN_PRECISE() ;
            if (Frame_Count == 0 && Field_Count == 0) {
                ...
                if (StreamKind_Last == Stream_Image)
                    Fill(Stream_Image, 0, Image_Codec_String, "JPEG 2000", Unlimited, true,
                         true); //To Avoid automatic filling
                Fill(StreamKind_Last, 0, StreamKind_Last == Stream_Image ? (size_t) Image_Width : (size_t) Video_Width,
                     Xsiz);
                Fill(StreamKind_Last, 0,
                     StreamKind_Last == Stream_Image ? (size_t) Image_Height : (size_t) Video_Height,
                     Ysiz * (Interlaced ? 2 : 1)); //If image is from interlaced content, must multiply height by 2
                ...
                if (!ChromaSubsampling.empty()) {
                    ...
                    if (ChromaSubsampling == __T("4:4:4") &&
                        (Retrieve(StreamKind_Last, 0, "Format_Profile") == __T("D-Cinema 2k") ||
                         Retrieve(StreamKind_Last, 0, "Format_Profile") == __T("D-Cinema 4k")))
...
//---------------------------------------------------------------------------
    void File_Jpeg::COD() {
        ...
        Info_B1(DimX, "Code-blocks dimensions X (2^(n+2))");
        Param_Info2(1 << (DimX + 2), " pixels");
        Info_B1(DimY, "Code-blocks dimensions Y (2^(n+2))");
        Param_Info2(1 << (DimY + 2), " pixels");
        ...
        FILLING_BEGIN() ;
            if (Frame_Count == 0 && Field_Count == 0) {
                switch (MultipleComponentTransform) {
                    case 0x01 :
                        Fill(StreamKind_Last, 0, "Compression_Mode", "Lossless");
                        break;
                    case 0x02 :
                        Fill(StreamKind_Last, 0, "Compression_Mode", "Lossy");
                        break;
                    default   :;
                }
            }
...
//---------------------------------------------------------------------------
    void File_Jpeg::SOF_() {
        ...
        for (int8u Pos = 0; Pos < Count; Pos++) {
            ...
            Get_B1(SamplingFactor.Ci, "Ci - Component identifier");
            if (SamplingFactor.Ci > Count)
                Element_Info1(Ztring().append(1, (Char) SamplingFactor.Ci));
            else
                Element_Info1(SamplingFactor.Ci);
            BS_Begin();
            Get_S1(4, SamplingFactor.Hi, "Hi - Horizontal sampling factor");
            Element_Info1(SamplingFactor.Hi);
            Get_S1(4, SamplingFactor.Vi, "Vi - Vertical sampling factor");
            Element_Info1(SamplingFactor.Vi);
            BS_End();
        ...
        FILLING_BEGIN_PRECISE() ;
            if (Frame_Count == 0 && Field_Count == 0) {
                ...
                if (StreamKind_Last == Stream_Image)
                    Fill(Stream_Image, 0, Image_Codec_String, "JPEG", Unlimited, true,
                         true); //To Avoid automatic filling
                if (StreamKind_Last == Stream_Video)
                ...
                switch (APPE_Adobe0_transform) {
                    ...
                    default   : {
                        ...
                        switch (Count) {
                            case 1 :
                                Fill(StreamKind_Last, 0, "ColorSpace", "Y");
                                break;
                            case 2 :
                                Fill(StreamKind_Last, 0, "ColorSpace", "YA");
                                break;
                            case 3 :
                                if (!APP0_JFIF_Parsed && Ci['R'] == 1 && Ci['G'] == 1 &&
                                    Ci['B'] == 1)                                                       //RGB
                                    Fill(StreamKind_Last, 0, "ColorSpace", "RGB");
                                else if ((Ci['Y'] == 1 && ((Ci['C'] == 1 && Ci['c'] ==
                                                                            1)                                                                         //YCc
                                                           || Ci['C'] ==
                                                              2))                                                                                       //YCC
                                         ||
                                         APP0_JFIF_Parsed                                                                                                   //APP0 JFIF header present so YCC
                                         || APPE_Adobe0_transform ==
                                            0                                                                                           //transform set to YCC
                                         || (SamplingFactors[0].Ci == 0 && SamplingFactors[1].Ci == 1 &&
                                             SamplingFactors[2].Ci == 2)                                 //012
                                         || (SamplingFactors[0].Ci == 1 && SamplingFactors[1].Ci == 2 &&
                                             SamplingFactors[2].Ci == 3))                                //123
                                    Fill(StreamKind_Last, 0, "ColorSpace", "YUV");
                                break;
                            case 4 :
                                if (!APP0_JFIF_Parsed && Ci['R'] == 1 && Ci['G'] == 1 && Ci['B'] == 1 &&
                                    Ci['A'] == 1)                                         //RGBA
                                    Fill(StreamKind_Last, 0, "ColorSpace", "RGBA");
                                else if ((Ci['Y'] == 1 && Ci['A'] == 1 && ((Ci['C'] == 1 && Ci['c'] ==
                                                                                            1)                                                           //YCcA
                                                                           || Ci['C'] ==
                                                                              2))                                                                         //YCCA
                                         ||
                                         ...
                                         || (SamplingFactors[0].Ci == 0 && SamplingFactors[1].Ci == 1 &&
                                             SamplingFactors[2].Ci == 2 && SamplingFactors[3].Ci == 3)     //0123
                                         || (SamplingFactors[0].Ci == 1 && SamplingFactors[1].Ci == 2 &&
                                             SamplingFactors[2].Ci == 3 && SamplingFactors[3].Ci == 4))    //1234
                                    Fill(StreamKind_Last, 0, "ColorSpace", "YUVA");
                                else if (APPE_Adobe0_transform ==
                                         0)                                                                                          //transform set to CMYK
                                    Fill(StreamKind_Last, 0, "ColorSpace", "YCCB");
                                break;
                            default:;
                        }
                ...
                //Chroma subsampling
                if ((SamplingFactors.size() == 3 || SamplingFactors.size() == 4) && SamplingFactors[1].Hi == 1 &&
                    SamplingFactors[2].Hi == 1 && SamplingFactors[1].Vi == 1 && SamplingFactors[2].Vi == 1) {
                    ...
                    if (!ChromaSubsampling.empty()) {
                        if (SamplingFactors.size() == 4) {
                            if (ChromaSubsampling == "4:4:4" && SamplingFactors[3].Hi == 1 &&
                                SamplingFactors[3].Vi == 1)
                                ChromaSubsampling += ":4";
                            else
                                ChromaSubsampling += ":?";
                        }
    ...
    void File_Jpeg::APP0_AVI1() {
        ...
        if (Element_Size >= 14) {
            ...
            if (FieldOrder == 0 && IsSub && FieldSize && FieldSize != Buffer_Size) {
                if (FieldSizeLessPadding > 1 && FieldSizeLessPadding <= Buffer_Size &&
                    Buffer[FieldSizeLessPadding - 2] == 0xFF && Buffer[FieldSizeLessPadding - 1] == 0xD9  //EOI
