...
//---------------------------------------------------------------------------
    static const char *Tiff_Tag_Name(int32u Tag) {
        switch (Tag) {
            case Tiff_Tag::ImageWidth                   :
                return "ImageWidth";
            case Tiff_Tag::ImageLength                  :
                return "ImageLength";
            case Tiff_Tag::BitsPerSample                :
                return "BitsPerSample";
            case Tiff_Tag::Compression                  :
                return "Compression";
            case Tiff_Tag::PhotometricInterpretation    :
                return "PhotometricInterpretation";
            case Tiff_Tag::ImageDescription             :
                return "ImageDescription";
            case Tiff_Tag::SamplesPerPixel              :
                return "SamplesPerPixel";
            case Tiff_Tag::ExtraSamples                 :
                return "ExtraSamples";
            default                                     :
                return "";
        }
    ...
    static const char *Tiff_Compression_Mode(int32u Compression) {
        switch (Compression) {
            ...
            case 32773 :
                return "Lossless";
            default    :
                return ""; //Unknown or depends of the compresser (e.g. JPEG can be lossless or lossy)
        }
    }
    ...
    static const char *Tiff_PhotometricInterpretation_ColorSpace(int32u PhotometricInterpretation) {
        switch (PhotometricInterpretation) {
            ...
            case 3 :
                return "RGB"; //Palette
            case 4 :
                return "A"; //Transparency mask;
            case 5 :
                return "CMYK";
            case 6 :
                return "YUV"; //YCbCr
            case 8 :
                return "CIELAB"; //What is it?
            default    :
                return ""; //Unknown
        }
    }
...
//---------------------------------------------------------------------------
    void File_Tiff::Header_Parse() {
        ...
        Header_Fill_Size(
                2 + 12 * ((int64u) NrOfDirectories) + 4); //2 for header + 12 per directory + 4 for next IFD offset
    }
...
//---------------------------------------------------------------------------
    void File_Tiff::Read_Directory() {
        ...
        Element_Begin0();
        ifditem IfdItem;
        Get_X2(IfdItem.Tag, "Tag");
        Param_Info1(Tiff_Tag_Name(IfdItem.Tag));
        Get_X2(IfdItem.Type, "Type");
        Param_Info1(Tiff_Type_Name(IfdItem.Type));
...
//---------------------------------------------------------------------------
    void File_Tiff::GetValueOffsetu(ifditem &IfdItem) {
        ZtringList &Info = Infos[IfdItem.Tag];
        Info.clear();
        ...
        switch (IfdItem.Type) {
            ...
            case 3:                /* 16-bit (2-byte) unsigned integer. */
                for (int16u Pos = 0; Pos < IfdItem.Count; Pos++) {
                    ...
                    switch (IfdItem.Tag) {
                        case Tiff_Tag::Compression :
                            Element_Info1(Tiff_Compression(Ret16));
                            break;
                        case Tiff_Tag::PhotometricInterpretation :
                            Element_Info1(Tiff_PhotometricInterpretation(Ret16));
                            break;
                        default :
