...
const char *
__stdcall MediaInfoA_GetI(void *Handle, MediaInfo_stream_t StreamKind, size_t StreamNumber, size_t Parameter,
                          MediaInfo_info_C KindOfInfo) {
    return WC2MB(Handle, MediaInfo_GetI(Handle, StreamKind, StreamNumber, Parameter, KindOfInfo));
}

const char *
__stdcall MediaInfoA_Get(void *Handle, MediaInfo_stream_t StreamKind, size_t StreamNumber, const char *Parameter,
                         MediaInfo_info_C KindOfInfo, MediaInfo_info_C KindOfSearch) {
    return WC2MB(Handle, MediaInfo_Get(Handle, StreamKind, StreamNumber, MB2WC(Handle, 0, Parameter), KindOfInfo,
                                       KindOfSearch));
}

size_t          __stdcall
MediaInfoA_SetI(void *Handle, const char *ToSet, MediaInfo_stream_t StreamKind, size_t StreamNumber, size_t Parameter,
                const char *OldParameter) {
    return MediaInfo_SetI(Handle, MB2WC(Handle, 0, ToSet), StreamKind, StreamNumber, Parameter,
                          MB2WC(Handle, 1, OldParameter));
}

size_t          __stdcall
MediaInfoA_Set(void *Handle, const char *ToSet, MediaInfo_stream_t StreamKind, size_t StreamNumber,
               const char *Parameter, const char *OldParameter) {
    return MediaInfo_Set(Handle, MB2WC(Handle, 0, ToSet), StreamKind, StreamNumber, MB2WC(Handle, 1, Parameter),
                         MB2WC(Handle, 2, OldParameter));
}
...
const char *
__stdcall MediaInfoListA_GetI(void *Handle, size_t FilePos, MediaInfo_stream_t StreamKind, size_t StreamNumber,
...
const char *
__stdcall MediaInfoListA_Get(void *Handle, size_t FilePos, MediaInfo_stream_t StreamKind, size_t StreamNumber,
                             const char *Parameter, MediaInfo_info_C KindOfInfo, MediaInfo_info_C KindOfSearch) {
    return WC2MB(Handle,
                 MediaInfoList_Get(Handle, FilePos, StreamKind, StreamNumber, MB2WC(Handle, 1, Parameter), KindOfInfo,
                                   KindOfSearch));
}
...
MediaInfoListA_SetI(void *Handle, const char *ToSet, size_t FilePos, MediaInfo_stream_t StreamKind, size_t StreamNumber,
    ...
    return MediaInfoList_SetI(Handle, MB2WC(Handle, 0, ToSet), FilePos, StreamKind, StreamNumber, Parameter,
                              MB2WC(Handle, 1, OldParameter));
}
...
MediaInfoListA_Set(void *Handle, const char *ToSet, size_t FilePos, MediaInfo_stream_t StreamKind, size_t StreamNumber,
                   const char *Parameter, const char *OldParameter) {
    return MediaInfoList_Set(Handle, MB2WC(Handle, 0, ToSet), FilePos, StreamKind, StreamNumber,
                             MB2WC(Handle, 1, Parameter), MB2WC(Handle, 2, OldParameter));
}
...
size_t          __stdcall
MediaInfoListA_Count_Get(void *Handle, size_t FilePos, MediaInfo_stream_t StreamKind, size_t StreamNumber) {
...
void *__stdcall MediaInfo_New() {
    ...
    MEDIAINFO_DEBUG2("New",
                     Debug += ", returns ";
                             Debug += Ztring::ToZtring((size_t) Handle).To_UTF8();)
...
void            __stdcall MediaInfo_Delete(void *Handle) {
    ...
    if (MI_Outputs.size() == 1 &&
        MI_Outputs.find(NULL) != MI_Outputs.end()) //In case of the last object : delete the NULL object, no more need
    {
...
MediaInfo_Open_Buffer(void *Handle, const unsigned char *Begin, size_t Begin_Size, const unsigned char *End,
    ...
    MANAGE_SIZE_T("Open_Buffer",
                  ...
                  Debug += ", Begin_Size=";
                          Debug += Ztring::ToZtring(Begin_Size).To_UTF8();
                          Debug += ", End_Size=";
                          Debug += Ztring::ToZtring(End_Size).To_UTF8();)
}
...
MediaInfo_Open_Buffer_Init(void *Handle, MediaInfo_int64u File_Size, MediaInfo_int64u File_Offset) {
    MANAGE_SIZE_T("Open_Buffer_Init",
                  ...
                  Debug += ", File_Size=";
                          Debug += Ztring::ToZtring(File_Size).To_UTF8();
                          Debug += ", File_Offset=";
                          Debug += Ztring::ToZtring(File_Offset).To_UTF8();)
}
...
size_t          __stdcall MediaInfo_Open_Buffer_Continue(void *Handle, MediaInfo_int8u *Buffer, size_t Buffer_Size) {
    MANAGE_SIZE_T("Open_Buffer_Continue",
                  ...
                  Debug += ", Buffer_Size=";
                          Debug += Ztring::ToZtring(Buffer_Size).To_UTF8();)
}
...
const wchar_t *
__stdcall MediaInfo_GetI(void *Handle, MediaInfo_stream_t StreamKind, size_t StreamNumber, size_t Parameter,
                         MediaInfo_info_C KindOfInfo) {
    ...
    MANAGE_STRING("GetI",
                  ...
                  Debug += ", StreamKind=";
                          Debug += Ztring::ToZtring((size_t) StreamKind).To_UTF8();
                          Debug += ", StreamNumber=";
                          Debug += Ztring::ToZtring(StreamNumber).To_UTF8();
                          Debug += ", Parameter=";
                          Debug += Ztring::ToZtring(Parameter).To_UTF8();
                          Debug += ", KindOfInfo=";
                          Debug += Ztring::ToZtring(KindOfInfo).To_UTF8();)
}

const wchar_t *
__stdcall MediaInfo_Get(void *Handle, MediaInfo_stream_t StreamKind, size_t StreamNumber, const wchar_t *Parameter,
                        MediaInfo_info_C KindOfInfo, MediaInfo_info_C KindOfSearch) {
    StreamKind = (MediaInfo_stream_t) (((size_t) StreamKind) & 0xFF);
    KindOfInfo = (MediaInfo_info_C) (((size_t) KindOfInfo) & 0xFF);
    ...
    MANAGE_STRING("Get",
                  ...
                  Debug += ", StreamKind=";
                          ...
                          Debug += Ztring::ToZtring(StreamNumber).To_UTF8();
                          Debug += ", Parameter=";
                          Debug += Ztring(Parameter).To_UTF8();
                          Debug += ", KindOfInfo=";
                          Debug += Ztring::ToZtring(KindOfInfo).To_UTF8();
                          Debug += ", KindOfSearch=";
                          Debug += Ztring::ToZtring(KindOfSearch).To_UTF8();)
}

size_t          __stdcall
MediaInfo_SetI(void *Handle, const wchar_t *ToSet, MediaInfo_stream_t StreamKind, size_t StreamNumber, size_t Parameter,
...
size_t          __stdcall MediaInfo_Output_Buffer_GetI(void *Handle, size_t Pos) {
    MANAGE_SIZE_T("Output_Buffer_GetI",
                  ...
                  Debug += ", Pos=";
                          Debug += Ztring::ToZtring(Pos).To_UTF8();)
}
...
size_t          __stdcall MediaInfo_Count_Get(void *Handle, MediaInfo_stream_t StreamKind, size_t StreamNumber) {
    ...
    MANAGE_SIZE_T("Count_Get",
                  ...
                  Debug += ", StreamKind=";
                          Debug += Ztring::ToZtring((size_t) StreamKind).To_UTF8();
                          Debug += ", StreamNumber=";
                          Debug += Ztring::ToZtring((size_t) StreamNumber).To_UTF8();)
}
...
const wchar_t *
__stdcall MediaInfoList_GetI(void *Handle, size_t FilePos, MediaInfo_stream_t StreamKind, size_t StreamNumber,
                             size_t Parameter, MediaInfo_info_C KindOfInfo) {
    StreamKind = (MediaInfo_stream_t) (((size_t) StreamKind) & 0xFF);
    KindOfInfo = (MediaInfo_info_C) (((size_t) KindOfInfo) & 0xFF);
    MANAGE_STRING("GetI",
...
const wchar_t *
__stdcall MediaInfoList_Get(void *Handle, size_t FilePos, MediaInfo_stream_t StreamKind, size_t StreamNumber,
...
size_t          __stdcall
MediaInfoList_SetI(void *Handle, const wchar_t *ToSet, size_t FilePos, MediaInfo_stream_t StreamKind,
...
size_t          __stdcall
MediaInfoList_Set(void *Handle, const wchar_t *ToSet, size_t FilePos, MediaInfo_stream_t StreamKind,
...
size_t          __stdcall
MediaInfoList_Count_Get(void *Handle, size_t FilePos, MediaInfo_stream_t StreamKind, size_t StreamNumber) {
    StreamKind = (MediaInfo_stream_t) (((size_t) StreamKind) & 0xFF);
    MANAGE_SIZE_T("Count_Get",
                  MediaInfoList,
