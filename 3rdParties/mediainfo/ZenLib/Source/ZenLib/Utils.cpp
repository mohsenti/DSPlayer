...
// Little Endian - float 64 bits
    float64 LittleEndian2float64(const char *Liste) {
        ...
        if (Exponent == 0 || Exponent == 0x7FF)
            return 0; //These are denormalised numbers, NANs, and other horrible things
        Exponent -= 0x3FF; //Bias
        float64 Answer = (((float64) Mantissa) / 4503599627370496.0 + 1.0) *
                         std::pow((float64) 2, (int) Exponent); //(1+Mantissa) * 2^Exponent
        if (Sign)
            Answer = -Answer;
...
// Little Endian - float 80 bits
    float80 LittleEndian2float80(const char *Liste) {
        ...
        float80 Answer = (((float80) Mantissa) / 9223372036854775808.0 + 1.0) *
                         std::pow((float) 2, (int) Exponent); //(1+Mantissa) * 2^Exponent
        if (Sign)
            Answer = -Answer;
