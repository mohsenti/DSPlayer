...
//---------------------------------------------------------------------------
    bool File::Open(const tstring &File_Name_, access_t Access) {
        ...
        ZENLIB_DEBUG1("File Open",
                      Debug += ", File_Name=";
                              Debug += Ztring(File_Name_).To_UTF8();)
        ...
        */
        ...
        switch (Access) {
            case Access_Read         :
                mode = ios_base::binary | ios_base::in;
                break;
            case Access_Write        :
                mode = ios_base::binary | ios_base::in | ios_base::out;
                break;
            case Access_Read_Write   :
                mode = ios_base::binary | ios_base::in | ios_base::out;
                break;
            case Access_Write_Append :
                if (!Exists(File_Name))
...
//---------------------------------------------------------------------------
    size_t File::Read(int8u *Buffer, size_t Buffer_Size_Max) {
        ZENLIB_DEBUG1("File Read",
                      Debug += ", File_Name=";
                              Debug += Ztring(File_Name).To_UTF8();
                              Debug += ", MaxSize=";
                              Debug += Ztring::ToZtring(Buffer_Size_Max).To_UTF8())
    ...
    bool File::GoTo(int64s Position_ToMove, move_t MoveMethod) {
        ZENLIB_DEBUG1("File GoTo",
                      Debug += ", File_Name=";
                              ...
                              Debug += Ztring(File_Name).To_UTF8();
                              Debug += ", MoveMethod=";
                              Debug += Ztring::ToZtring(MoveMethod).To_UTF8();
                              Debug += ", MaxSize=";
                              Debug += Ztring::ToZtring(Position_ToMove).To_UTF8())
...
//---------------------------------------------------------------------------
    Ztring File::Modified_Get() {
        ...
        if (Result < 0)
            return __T(""); //Error
        Ztring Time;
        Time.Date_From_Seconds_1970((int64s) Stat.st_mtime);
        return Time;
#elif defined WINDOWS
        FILETIME TimeFT;
...
//---------------------------------------------------------------------------
    Ztring File::Modified_Local_Get() {
        ...
        if (Result < 0)
            return __T(""); //Error
        Ztring Time;
        Time.Date_From_Seconds_1970_Local(Stat.st_mtime);
        return Time;
#elif defined WINDOWS
        FILETIME TimeFT;
        if (GetFileTime(File_Handle, NULL, NULL, &TimeFT))
