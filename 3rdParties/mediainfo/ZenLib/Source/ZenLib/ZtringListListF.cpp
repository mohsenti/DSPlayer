    ...
    bool ZtringListListF::Save(const Ztring &FileName) {
        ...
        if (Backup_Nb_Max > 0) {
            ...
            for (int8u I1 = Backup_Nb_Max - 1; I1 > 0; I1--) {
                Ztring Z1 = Name + __T(".sav");
                Z1 += Ztring::ToZtring(I1);
                Ztring Z2 = Name + __T(".sav");
                Z2 += Ztring::ToZtring(I1 + 1);
...
// Annulation
    bool ZtringListListF::Cancel() {
        ...
        for (int8u I1 = 1; I1 <= Backup_Nb; I1++) {
            Ztring Z2 = Name + __T(".sav");
            Z2 += Ztring::ToZtring(I1); //Visual C++ 6 patch
            Ztring Z3 = Name + __T(".sav");
            Z3 += Ztring::ToZtring(I1 - 1); //Visual C++ 6 patch
