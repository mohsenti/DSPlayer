    ...
    do {
        ...
        if ((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_URLANDPORT) != HTTP_CLIENT_FLAG_URLANDPORT) {
            ...
            if ((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession, "Host", 4, pHTTPSession->HttpUrl.UrlHost.pParam,
                                                (pHTTPSession->HttpUrl.UrlPort.pParam -
                                                 pHTTPSession->HttpUrl.UrlHost.pParam) - 1)) != HTTP_CLIENT_SUCCESS) {
                break;
            }
        }
        ...
        if ((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_KEEP_ALIVE) == HTTP_CLIENT_FLAG_KEEP_ALIVE) {
            if ((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_USINGPROXY) != HTTP_CLIENT_FLAG_USINGPROXY) {
                // No proxy keep alive:
                if ((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession, "Connection", 10, "Keep-Alive", 10)) !=
                    HTTP_CLIENT_SUCCESS) {
                    break;
                }
        ...
        if ((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_NO_CACHE) == HTTP_CLIENT_FLAG_NO_CACHE) {
            if ((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession, "Cache-Control", 13, "no-cache", 8)) !=
                HTTP_CLIENT_SUCCESS) {
                break;
            }
        }
        ...
        if (pHTTPSession->HttpCredentials.CredAuthSchema != AuthSchemaNone ||
            pHTTPSession->HttpProxy.ProxyAuthSchema !=
            AuthSchemaNone) // If we have to authenticate we should use the HEAD verb
        {
...
UINT32 HTTPClientWriteData(HTTP_SESSION_HANDLE pSession,
    ...
    do {
        ...
        if ((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_SEND_CHUNKED) == HTTP_CLIENT_FLAG_SEND_CHUNKED) {
            ...
            HTTPStrLToH(Chunk,
                        nBufferLength); //pChunkHeaderPtr =  //Jerome/cppcheck: Variable 'pChunkHeaderPtr' is assigned a value that is never used.
...
UINT32
HTTPClientFindFirstHeader(HTTP_SESSION_HANDLE pSession, CHAR *pSearchClue, CHAR *pHeaderBuffer, UINT32 *nLength) {
            ...
            if (nSocketEvents == 0) {
                ...
                if ((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_SECURE) == HTTP_CLIENT_FLAG_SECURE) {
                    ...
                    if (nRetCode > 0) {
                        ...
                        if ((nRetCode = HTTPWrapperSSLRecv(pConnection->HttpSocket, pData, *(nLength), 0)) ==
                            SOCKET_ERROR) {
                            // Socket error
                ...
                if (!((nBytesCount == 1 && *pPtr == 0x0d) || (nBytesCount == 2 && *pPtr == 0x0a))) {
                    ...
                    if (memcmp(pPtr - 2, HTTP_CLIENT_CRLF, 2) == 0) {
                        ...
                        *pPtr = 0;  // null terminate the chunk parameter
                        pHTTPSession->HttpCounters.nRecivedChunkLength = HTTPStrHToL(
                                ChunkHeader); // Convert to a number
                        // Set the HTTP counters
        ...
        while (nBytesRead > 0) {
            ...
            if ((INT32) nProjectedfreeSpace < 0) {
                if (HTTP_CLIENT_MEMORY_RESIZABLE == FALSE) {
                    ...
                    // We can resizes so..
                    nRetCode = HTTPIntrnResizeBuffer(pHTTPSession,
                                                     nProjectedBufferLength + HTTP_CLIENT_MEMORY_RESIZE_FACTOR);
                    if (nRetCode != HTTP_CLIENT_SUCCESS) {
                        break;
                    }
                }
    ...
    do {
        ...
        while (AuthHeaders == FALSE)  // address multiple authentication methods presented by the server
        {
            if (pHTTPSession->HttpHeadersInfo.nHTTPStatus == HTTP_STATUS_UNAUTHORIZED) {
                ...
                if (HTTPIntrnHeadersFind(pHTTPSession, "www-authenticate", &pHTTPSession->HttpAuthHeader.AuthHeader,
                                         TRUE, nOffset) != HTTP_CLIENT_SUCCESS) {
                ...
                if (pPtr) {
                    ...
                    nOffset = (pHTTPSession->HttpAuthHeader.AuthHeader.pParam -
                               pHTTPSession->HttpHeaders.HeadersIn.pParam) + 3;
                }
            } else {
                AuthHeaders = TRUE;
            }
        ...
        if (pHTTPSession->HttpHeadersInfo.nHTTPStatus == HTTP_STATUS_PROXY_AUTHENTICATION_REQUIRED) {
            ...
            if (HTTPIntrnHeadersFind(pHTTPSession, "proxy-authenticate", &pHTTPSession->HttpAuthHeader.AuthHeader, TRUE,
        ...
        if ((pHTTPSession->HttpHeadersInfo.nHTTPStatus == HTTP_STATUS_OBJECT_MOVED) ||
            ...
            if (HTTPIntrnHeadersFind(pHTTPSession, "location", &pHTTPSession->HttpHeadersInfo.HttpRedirectURL, TRUE,
                                     0) != HTTP_CLIENT_SUCCESS) {
...
// Can be used for evaluating a HEAD request
{
    ...
    do {
        ...
        if (pHTTPSession->HttpHeaders.HttpVerb == VerbHead) {
            ...
            // Content-Length
            if (pHTTPSession->HttpHeadersInfo.nHTTPPostContentLength >
                0) // Attempt to remove only if it was previusly set
            {
        ...
        if (pHTTPSession->HttpHeaders.HttpVerb != HttpCachedVerb) {
            if ((nRetCode = HTTPClientSetVerb((HTTP_SESSION_HANDLE) pHTTPSession, HttpCachedVerb)) !=
                HTTP_CLIENT_SUCCESS) {
                break;
            }
        }
        ...
        if (RestoreHeadersFlag == TRUE) {
            ...
            {
                IToA(ContentLength,
                     pHTTPSession->HttpHeadersInfo.nHTTPPostContentLength); // Convert the buffer length to a string value //pPtr =  //Jerome/cppcheck: Variable 'pPtr' is assigned a value that is never used.
                if ((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession, "Content-Length", 14, ContentLength,
                                                    strlen(ContentLength))) != HTTP_CLIENT_SUCCESS) {
                    return nRetCode;
                }
    ...
    do {
        ...
        switch (pHTTPSession->HttpHeadersInfo.nHTTPStatus) {
            ...
            case (HTTP_STATUS_PROXY_AUTHENTICATION_REQUIRED): // For Proxy authentication
            {
                ...
                // Build and send the data first the hard-coded static portion
                if ((nRetCode = HTTPIntrnSend(pHTTPSession, INITIAL_PROXY_HDR, &nSegmentLength)) !=
                    HTTP_CLIENT_SUCCESS) {
                    break;
                }
                // Set the counters
...
static UINT32 HTTPIntrnAuthSendDigest(P_HTTP_SESSION pHTTPSession) {
    ...
    do {
        ...
        if ((nRetCode = HTTPIntrnSend(pHTTPSession, HttpParamRealm.pParam, &HttpParamRealm.nLength)) !=
            HTTP_CLIENT_SUCCESS) {
            break;
        }
        // Set the counters
        ...
        if ((nRetCode = HTTPIntrnSend(pHTTPSession, HttpParamQop.pParam, &HttpParamQop.nLength)) !=
            HTTP_CLIENT_SUCCESS) {
            break;
        }
        // Set the counters
        ...
        // Send the algorithem
        if ((nRetCode = HTTPIntrnSend(pHTTPSession, HttpParamAlg.pParam, &HttpParamAlg.nLength)) !=
            HTTP_CLIENT_SUCCESS) {
            break;
        }
        // Set the counters
        ...
        if ((nRetCode = HTTPIntrnSend(pHTTPSession, HttpParamNonce.pParam, &HttpParamNonce.nLength)) !=
            HTTP_CLIENT_SUCCESS) {
            break;
        }
        // Set the counters
        ...
        if ((nRetCode = HTTPStrGetDigestToken(pHTTPSession->HttpAuthHeader.AuthHeader, "opaque", &HttpParamOpq)) ==
            ...
            // Set the counters
            pHTTPSession->HttpCounters.nSentHeaderBytes += nSegmentLength;
            if ((nRetCode = HTTPIntrnSend(pHTTPSession, HttpParamOpq.pParam, &HttpParamOpq.nLength)) !=
                HTTP_CLIENT_SUCCESS) {
                break;
            }
            // Set the counters
