...
namespace {
    ...
    const ByteVector extendedContentDescriptionGuid("\x40\xA4\xD0\xD2\x07\xE3\xD2\x11\x97\xF0\x00\xA0\xC9\x5E\xA8\x50",
                                                    16);
    ...
    const ByteVector extendedContentEncryptionGuid("\x14\xE6\x8A\x29\x22\x26 \x17\x4C\xB9\x35\xDA\xE0\x7E\xE9\x28\x9C",
                                                   16);
    const ByteVector advancedContentEncryptionGuid("\xB6\x9B\x07\x7A\xA4\xDA\x12\x4E\xA5\xCA\x91\xD3\x8D\xC1\x1A\x8D",
                                                   16);
}
...
ByteVector ASF::File::FilePrivate::HeaderExtensionObject::render(ASF::File *file) {
    ...
    data = ByteVector("\x11\xD2\xD3\xAB\xBA\xA9\xcf\x11\x8E\xE6\x00\xC0\x0C\x20\x53\x65\x06\x00", 18) +
           ByteVector::fromUInt(data.size(), false) + data;
    return BaseObject::render(file);
}
...
bool ASF::File::save() {
    ...
    for (AttributeListMap::ConstIterator it = allAttributes.begin(); it != allAttributes.end(); ++it) {
        ...
        for (AttributeList::ConstIterator jt = attributes.begin(); jt != attributes.end(); ++jt) {
            ...
            if (!inExtendedContentDescriptionObject && !guid && !largeValue && attribute.language() == 0 &&
                attribute.stream() == 0) {
                d->extendedContentDescriptionObject->attributeData.append(attribute.render(name));
                inExtendedContentDescriptionObject = true;
            } else if (!inMetadataObject && !guid && !largeValue && attribute.language() == 0 &&
                       attribute.stream() != 0) {
